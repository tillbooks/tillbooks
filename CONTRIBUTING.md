...
The commands above cover the core: the engine, the MCP server, and the CLI. If you touch the Studio
under `app/`, install its dependencies and run the full gate, which builds the Studio and runs its
tests on top of `npm run check`:

```bash
npm run setup          # installs app/ dependencies too
npm run gate           # npm run check, plus the Studio build and tests
```
...
`develop` is the integration branch. Fork the repo, work on a short-lived branch, and open a pull
request into `develop`. Maintainers cut releases from there.
...
Before you open the PR: `npm run check` passes (run `npm run gate` as well if you touched the
Studio), and you have actually run the thing.
