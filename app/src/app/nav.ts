...
/**
 * A rail PARENT: a heading that groups sibling surfaces and is NOT itself a destination.
 *
 * A parent has no path (it is a label, not a link: there is no "MWST overview" screen), which is the
 * property that keeps the rail's link count equal to the number of destinations. Its own `navId` is
 * the stable key a per-user collapse flag attaches to (A1 needs collapsible parents). The model
 * carries this level whether or not the rail renders nesting today, so personalization can grow into
...
export const NAV_TREE: readonly NavTreeGroup[] = [
  ...
  },
  {
    ...
    items: [
      ...
      leaf('files'),
      {
        ...
        // landing) leading. A heading, never a link: it has no destination of its own.
        navId: 'lager-group',
  ...
  },
  {
    ...
    items: [
      ...
      leaf('fx'),
      {
        // The MWST parent: a heading, not a link (it has no path). The `/vat` route is untouched;
        // only the child labels dropped the "MWST-" prefix the parent already carries.
        navId: 'mwst-group',
  ...
  },
  {
    ...
    items: [
      ...
      leaf('environments'),
      leaf('onboarding'),
      // M00: the pre-workspace first-run door (create / restore / adopt). The spec (§6) mounts it as
      // a pre-workspace route; the D46 UX pass owns intercepting the unprovisioned state to render it
      // in place of an empty Setup. It carries a stable route here so it is reachable meanwhile.
      leaf('first-run'),
    ],
  },
];
