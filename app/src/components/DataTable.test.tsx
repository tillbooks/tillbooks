...
describe('DataTable, the stylesheet honours the layout law', () => {
  ...
  it('gives an OVERFLOWING frame its own horizontal scroll with a reserved gutter', () => {
    // K-19: `scroll`, not `auto`, so the thin bar is drawn at rest and the cut is visible.
  ...
  it('reads the density tokens for row height, padding and numeric type', () => {
    // K-20: the row is its own token (36/32), no longer the control height.
    expect(css).toContain('var(--t-row-h)');
    expect(css).toContain('var(--t-row-head-h)');
    expect(css).toContain('var(--t-cell-pad-x)');
