 ...
 *
 * Focus lands on the first FIELD (K-31, D137), the first invalid one when the form reopens with
 * errors, or on the dialog itself (named by its title) when there is no field. It used to land on the
 * close X, because the X is the first focusable in the DOM, so the first Space in any drawer closed
 * it. A caller names another target with `initialFocus`.
 *
 * `width="wide"` (K-30) is the 720px drawer for a form laid out as a grid (the journal composer);
 * below that viewport both widths are a full-width sheet.
 */
import { useId, useRef, type ReactNode } from 'react';

import { firstInvalidOrField, useFocusTrap, type InitialFocus } from './useFocusTrap';
...
export interface DetailDrawerProps {
  ...
  trapActive?: boolean;
  /**
   * The drawer's width (K-30). `default` is 480px, one column of fields. `wide` is 720px, for a form
   * laid out as a grid such as the journal composer. Below either width the drawer is a full-width
   * sheet.
   */
  width?: DrawerWidth;
  /**
   * Where focus lands on open (K-31). Defaults to the first invalid field, else the first field, else
   * the dialog itself: never the close X. Pass a ref, a selector inside the drawer, or a function.
   */
  initialFocus?: InitialFocus;
}

/** The two drawer widths (K-30). */
...
function DrawerPanel({
  ...
  trapActive = true,
  width = 'default',
  initialFocus = firstInvalidOrField,
}: Omit<DetailDrawerProps, 'open'>) {
  const titleId = useId();
  const panelRef = useRef<HTMLDivElement>(null);

  useFocusTrap(panelRef, { onEscape: onClose, active: trapActive, initialFocus });
