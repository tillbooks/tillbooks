...
describe('FeedbackDialog, the overlay tier (K-28)', () => {
  it('is portaled to document.body, outside the tree that opened it', async () => {
    const { container, dialog } = await openDialog();
    expect(container.contains(dialog)).toBe(false);
    // The overlay is a direct child of the body, so its z-index competes at the root, not inside a
    // drawer's stacking context.
    expect(dialog.parentElement?.parentElement).toBe(document.body);
  });

  it('sits on the palette tier over the shared scrim, with one overlay shadow', () => {
    const css = readFileSync(resolve(process.cwd(), 'src/components/FeedbackDialog.css'), 'utf8');
    expect(css).toMatch(/\.fb-overlay\s*\{[^}]*z-index:\s*var\(--t-z-palette\)/);
    expect(css).toMatch(/\.fb-overlay\s*\{[^}]*background:\s*var\(--t-scrim\)/);
    expect(css).toMatch(/\.fb-dialog\s*\{[^}]*box-shadow:\s*var\(--t-shadow-overlay\)/);
  });
});
...
describe('FeedbackDialog, the design gates', () => {
  ...
  it.each(['light', 'dark'] as const)('has no axe violations in the %s theme, at de-CH lengths', async (theme) => {
    // The dialog is portaled to document.body (K-28), so the render container no longer holds it:
    // axe reads the body, or it would pass vacuously over an empty container.
    await openDialog({ theme, request: { diagnostic: CRASH_ENTRY } });
    await fillForm();
