...
describe('Modal, the overlay tier (K-28)', () => {
  it('sits on the overlay tier over the shared scrim, one shadow and the dialog radius', () => {
    const css = readFileSync(resolve(process.cwd(), 'src/components/Modal.css'), 'utf8');
    expect(css).toMatch(/\.modal-scrim\s*\{[^}]*z-index:\s*var\(--t-z-overlay\)/);
    expect(css).toMatch(/\.modal-scrim\s*\{[^}]*background:\s*var\(--t-scrim\)/);
    expect(css).toMatch(/\.modal\s*\{[^}]*box-shadow:\s*var\(--t-shadow-overlay\)/);
    expect(css).toMatch(/\.modal\s*\{[^}]*border-radius:\s*var\(--t-radius-lg\)/);
