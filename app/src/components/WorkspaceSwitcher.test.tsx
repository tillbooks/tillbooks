 ...
 * error fall back to the RAW workspace id on the trigger, and the menu always keeps the
 * new-workspace way out.
 */
...
describe('WorkspaceSwitcher: rendering states', () => {
  ...
  it('falls back to the raw id while the list is loading', async () => {
    const { transport } = renderSwitcher({
      routes: { list_workspaces: () => new Promise<RestResponse>(() => {}) },
    });
    // The raw id is what the trigger shows before ANY list read, so asserting it straight after
    // render would pass over a switcher that never asked for the names at all. That is a different
    // defect (a trigger permanently stuck on the id), and this test is not about it.
    await transport.started('list_workspaces');
    expect(screen.getByRole('button', { name: 'Arbeitsbereich wechseln: ws_1' })).toHaveTextContent('ws_1');
  });
  ...
  it('falls back to the raw id when the current id is unknown to the list', async () => {
    renderSwitcher({ initialId: 'ws_unknown' });
    expect(await screen.findByRole('button', { name: 'Arbeitsbereich wechseln: ws_unknown' })).toBeInTheDocument();
  });
  ...
  it('on a load error the trigger keeps the raw id and the menu still offers the way to Setup', async () => {
    renderSwitcher({ routes: { list_workspaces: { status: 422, body: { ok: false, error: 'transport_error' } } } });
    const trigger = await screen.findByRole('button', { name: 'Arbeitsbereich wechseln: ws_1' });
    await userEvent.click(trigger);
