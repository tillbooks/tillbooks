 ...
 *
 * ONE VERB, THE WORDS A PERSON USES FOR IT (K-42, D137). The Journal's header button says "Neue
 * Buchung", the Übersicht door says "Buchen", and the palette row said "Buchung erfassen", so typing
 * the words on the button found nothing but the ask row. A command may carry `aliasKey`: a locale key
 * whose value is a comma-separated list of the other names the product uses for the same verb (the
 * surface header's primary label first). The palette ranks a verb by its best hit over the label and
 * every alias (`verbRank`), and `command-source.test.ts` holds each surface header's primary label to
 * resolving its verb, so a relabelled button cannot silently fall out of the palette again.
 */
import generated from './command-source.generated.json';
import exposed from './command-source.exposed.json';
import { NAV_ITEMS, SURFACE_CATALOG, type NavIconName } from '../app/nav';
...
interface ExposedEntry {
  ...
  labelKey: string;
  /** K-42: a locale key holding a comma-separated list of the product's other names for the verb. */
  aliasKey?: string;
  targetRoute?: string;
...
export interface NavCommand {
  ...
  path: string;
  labelKey: string;
  /** The surface's own rail glyph, for the palette row (K-43). */
  icon: NavIconName;
}
...
export interface VerbCommand {
  ...
  name: string;
  labelKey: string;
  /** K-42: the locale key of the verb's other names (comma-separated), when it has any. */
  aliasKey?: string;
...
export const NAV_COMMANDS: readonly NavCommand[] = NAV_ITEMS.map((item) => ({
  ...
  // A nested rail leaf drops its glyph in the rail; the palette row always shows the surface's own.
  icon: item.icon ?? SURFACE_CATALOG.find((surface) => surface.path === item.path)?.icon ?? 'overview',
}));
...
export const VERB_COMMANDS: readonly VerbCommand[] = EXPOSED.flatMap((e) => {
  ...
  if (e.aliasKey !== undefined) cmd.aliasKey = e.aliasKey;
  return [cmd];
});
...
/**
 * K-42 (D137): the scattered-subsequence tier only makes sense for a short, typed-in-a-hurry query
 * ("bchg" for Buchung). A question or a phrase ("wie viele offene rechnungen habe ich") contains the
 * letters of almost any glossary word in order, and the tier then promoted "Berichtigungsabrechnung"
 * above the ask row. From three words or sixteen characters, a subsequence is not a match.
 */
export const SUBSEQUENCE_MAX_WORDS = 2;
export const SUBSEQUENCE_MAX_LENGTH = 15;

function subsequenceAllowed(q: string): boolean {
  return q.length <= SUBSEQUENCE_MAX_LENGTH && q.split(/\s+/).filter(Boolean).length <= SUBSEQUENCE_MAX_WORDS;
}
...
export function rankScore(query: string, text: string): number {
  ...
  return subsequenceAllowed(q) && isSubsequence(q, t) ? RANK.subsequence : RANK.none;
}

/**
 * The words a verb answers to: its label and, when it has an alias key, every alias (K-42). `t` is the
 * caller's translate function, so the list follows the active locale.
 */
export function verbNames(cmd: Pick<VerbCommand, 'labelKey' | 'aliasKey'>, t: (key: string) => string): string[] {
  const names = [t(cmd.labelKey)];
  if (cmd.aliasKey !== undefined) {
    for (const alias of t(cmd.aliasKey).split(',')) {
...
/** A verb's rank for `query`: the best tier over its label and its aliases (K-42). */
export function verbRank(query: string, cmd: Pick<VerbCommand, 'labelKey' | 'aliasKey'>, t: (key: string) => string): number {
  return Math.max(...verbNames(cmd, t).map((name) => rankScore(query, name)));
}

/**
 * The palette auto-selects a row only when it matched ABOVE the subsequence tier (K-42): anything
 * weaker is a guess, and a guess must never take Enter from the ask row.
 */
export const AUTO_SELECT_MIN = RANK.subsequence + 1;
