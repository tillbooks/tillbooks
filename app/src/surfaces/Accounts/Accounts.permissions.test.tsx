...
describe('Kontenplan, the A24 padlock (F5)', () => {
  it('renders every write affordance for an actor holding manage_chart', async () => {
    ...
    // K-21: the account name is the edit opener (named "Konto <nr> <name> bearbeiten").
    expect(screen.getAllByRole('button', { name: /^Konto .* bearbeiten$/ }).length).toBeGreaterThan(0);
  ...
  it('HIDES the create, edit and row overflows without it, while the chart still reads', async () => {
    ...
    expect(screen.queryAllByRole('button', { name: /^Konto .* bearbeiten$/ })).toHaveLength(0);
