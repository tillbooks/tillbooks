...
import { ArchiveGlyph, TagGlyph } from './glyphs';
import {
  GROUP_ORDER,
  groupOf,
  idemKey,
    ...
    <div className="accounts">
      ...
      {!hasVisibleAccounts && searching ? (
        <EmptyState
          ...
          action={{ label: t('account.clearSearch'), onClick: () => setSearch('') }}
        />
      ) : !hasVisibleAccounts ? (
        <EmptyState
...
function AccountRow({ account, canManage, onEdit, onArchive, onDelete }: AccountRowProps) {
  ...
  return (
    <li className={`acc-row${archived ? ' acc-row-archived' : ''}`}>
      ...
      {/* D15/C2: the primary action stays directly reachable and everything else, above all the
          destructive Delete, drops one level into the per-row overflow menu. The Archive-XOR-Delete
          exclusivity is unchanged, so the menu offers exactly one of the two and the operator still
          cannot choose wrong. */}
      {canManage && (
