...
describe('Adjustments, the reason-coded adjustment ledger', () => {
  ...
  it('posts a new adjustment through inventory_adjust', async () => {
    ...
    await userEvent.click(screen.getByRole('combobox', { name: de.adjustments.form.item }));
    await userEvent.click(screen.getByRole('option', { name: 'Widget' }));
    await userEvent.click(screen.getByRole('combobox', { name: de.adjustments.form.location }));
    await userEvent.click(screen.getByRole('option', { name: 'Lager A' }));
    await userEvent.type(screen.getByLabelText(de.adjustments.form.qty), '-7');
    await userEvent.click(screen.getByRole('combobox', { name: de.adjustments.form.reason }));
    await userEvent.click(screen.getByRole('option', { name: 'SCHWUND · Schwund' }));
  ...
  it('reverses an adjustment through inventory_adjust_reverse', async () => {
    ...
    await screen.findByText('Widget');
    // K-21: Stornieren sits in the row's overflow, never a button on the row.
    await userEvent.click(screen.getByRole('button', { name: /^Aktionen für die Korrektur von Widget/ }));
    await userEvent.click(screen.getByRole('menuitem', { name: de.adjustments.reverse }));
    await userEvent.click(screen.getByRole('combobox', { name: de.adjustments.reverseForm.reason }));
    await userEvent.click(screen.getByRole('option', { name: 'SCHWUND · Schwund' }));
