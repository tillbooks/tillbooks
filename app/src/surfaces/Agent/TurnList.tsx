...
/**
 * A call's outcome as one of the shared Status kinds (K-22, D137): the icon set's glyph plus the
 * word, never a text dingbat. A refusal and a failure are `danger`, a pending draft is `pending`, a
 * read is `neutral` and an executed write is `success`.
 */
function statusOf(call: AgentCall): { kind: StatusKind; key: string } {
  if (call.mode === 'deny') return { kind: 'danger', key: 'verweigert' };
  if (!call.ok) return { kind: 'danger', key: 'fehlgeschlagen' };
  if (call.mode === 'draft') return { kind: 'pending', key: 'entwurf' };
  if (call.kind === 'read') return { kind: 'neutral', key: 'gelesen' };
  return { kind: 'success', key: 'ausgefuehrt' };
}
...
function TraceRow({ call }: { call: AgentCall }) {
  ...
  return (
    <li className="trace-row" data-verb={call.verb}>
      <div className="trace-row-line">
        <span className="trace-status" data-status={status.key}>
          <Status kind={status.kind} label={t(`agent.trace.status.${status.key}`)} />
        </span>
