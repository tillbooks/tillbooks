...
const URGENCY_GLYPH: Record<Urgency, string> = { overdue: '!', due: '◔', open: '○' };
    ...
    <section className="att" aria-labelledby="att-title">
      {header(
        <>
          ...
          <button
            type="button"
            className="btn btn--ghost btn--sm att-refresh"
      ...
      <section className="att-work" aria-labelledby="att-work-title">
        ...
        {shownItems.length === 0 ? (
          openArea !== null ? (
            ...
            <EmptyState title={t('attention.empty.title')} hint={t('attention.empty.hint')} />
          ) : (
            ...
            {shownItems.map((item, index) => {
              ...
              return (
                <li key={rowKey} className="att-row" aria-busy={busyKey === rowKey ? 'true' : undefined}>
                  ...
                  >
                    <span className={`att-urgency att-urgency--${item.urgency}`}>
                      <span aria-hidden="true">{URGENCY_GLYPH[item.urgency]}</span>
                      <span className="att-urgency-label">{t(`attention.urgency.${item.urgency}`)}</span>
                    </span>
                    ...
                    {item.amountMinor !== undefined && item.currency !== undefined && (
                      <span className="att-amount">{formatMoney(item.amountMinor, item.currency)}</span>
                    )}
                  </button>
            ...
            {summary.queues.map((q) => (
              <li key={q.queueId}>
                ...
                >
                  ...
                  <span className={`att-urgency att-urgency--${q.topUrgency}`} aria-hidden="true">
                    {URGENCY_GLYPH[q.topUrgency]}
                  </span>
                  <span className="att-index-count">{q.count}</span>
                  <span className="att-index-chevron" aria-hidden="true">
                    ›
                  </span>
                </button>
              </li>
