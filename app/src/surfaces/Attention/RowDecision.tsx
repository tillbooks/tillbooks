 ...
 * reason field, never in a modal. The accent budget holds: the primary carries `btn--primary` only
 * on the selected row.
 *
...
export interface RowDecisionProps {
  item: DecisionItem;
  /** True on the ONE selected row: its primary carries the accent, every other row's stays neutral. */
  selected: boolean;
...
export function RowDecision({ item, selected, busy, onAct, onOpen }: RowDecisionProps) {
  ...
  const button = (option: DecisionOption): ReactNode => {
    ...
    const primary = option.role === 'primary';
    return (
      <button
        key={option.id}
        type="button"
        className={primary && selected ? 'btn btn--primary btn--sm' : 'btn btn--secondary btn--sm'}
        data-option={option.id}
        disabled={busy}
  ...
  return (
    <div className="att-decide" data-queue={item.queueId}>
      ...
      {confirming !== null ? (
        <div className="att-decide-confirm" role="group" aria-label={t(confirming.labelKey)}>
          ...
          <input
            id={reasonFieldId}
            className="att-decide-reason-field"
            type="text"
            value={reason}
