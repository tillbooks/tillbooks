...
describe('Automations, the load states', () => {
  it('shows a loading skeleton while the rule list is in flight', async () => {
    ...
    // The surface renders row skeletons in the final list shape, so there are SEVERAL `status`
    // nodes rather than one. Each must announce itself as busy.
...
describe('Automations, the pickers are fed by the ENGINE', () => {
  it('offers exactly the trigger events and action verbs the catalogue carried', async () => {
    ...
    const trigger = screen.getByLabelText(de.automation.form.trigger);
    expect([...within(trigger as HTMLSelectElement).getAllByRole('option')].map((o) => o.getAttribute('value'))).toEqual(
      CATALOGUE.events.map((e) => e.event),
    );

    const action = screen.getByLabelText(de.automation.form.action);
    expect([...within(action as HTMLSelectElement).getAllByRole('option')].map((o) => o.getAttribute('value'))).toEqual(
      [...CATALOGUE.actions],
    );
  });
...
describe('Automations, the off switch', () => {
  it('NEVER disables Stoppen for a viewer, and disables every other write control', async () => {
    ...
    const stop = await screen.findByRole('button', { name: de.automation.action.disable });
    await waitFor(() => expect(stop).toBeEnabled());
    expect(stop).not.toHaveAttribute('aria-disabled', 'true');

    expect(screen.getByRole('button', { name: de.automation.rules.create })).toBeDisabled();
    expect(screen.getByRole('button', { name: de.automation.action.archive })).toBeDisabled();
  ...
  it('disables Aktivieren for a viewer while Stoppen stays live on a STOPPED rule', async () => {
    ...
    const enable = await screen.findByRole('button', { name: de.automation.action.enable });
    await waitFor(() => expect(enable).toBeDisabled());
    // And the stop button is correctly absent on an already-stopped rule rather than merely enabled.
    expect(screen.queryByRole('button', { name: de.automation.action.disable })).toBeNull();
  });
  ...
  it('enables every control for an owner, so the gate above is measuring the ROLE', async () => {
    ...
    const stop = await screen.findByRole('button', { name: de.automation.action.disable });
    await waitFor(() => expect(stop).toBeEnabled());
    expect(screen.getByRole('button', { name: de.automation.rules.create })).toBeEnabled();
    expect(screen.getByRole('button', { name: de.automation.action.archive })).toBeEnabled();
  });
...
describe('Automations, the Verlauf', () => {
  it('renders a firing with its rule, its outcome and the identity it ran as', async () => {
    ...
    // WHOSE rights it ran with is the question this whole screen exists to answer.
