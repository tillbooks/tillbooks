...
export function Automations() {
  ...
  const ruleColumns: DataTableColumn<RuleDto>[] = [
    ...
    { key: 'trigger', header: t('automation.col.trigger'), render: (rule) => rule.trigger.event },
    { key: 'action', header: t('automation.col.action'), render: (rule) => rule.action.tool },
    {
      key: 'status',
      header: t('automation.col.status'),
      render: (rule) => (
        <>
          {/* Glyph AND text: a status must never be carried by shape or colour alone. */}
          <span aria-hidden="true">{rule.enabled ? '●' : '○'}</span>{' '}
          {rule.archived
            ? t('automation.status.archived')
            : rule.enabled
              ? t('automation.status.enabled')
    ...
    {
      key: 'controls',
      header: t('automation.col.controls'),
      render: (rule) => (
        <div className="automations__controls">
          {/*
            DEAKTIVIEREN IS NEVER DISABLED. Every other control here is gated on `manage_automations`,
            because the canon forbids offering a control that will always reject. The engine
            deliberately leaves the stop button ungated, so greying it out would be this screen lying
            about what the person may do, on the one control that matters most when a rule is writing
            things it should not.
          */}
          {!rule.archived && rule.enabled ? (
            <button type="button" disabled={busy} onClick={() => void runRuleAction('disable_automation_rule', rule.ruleId)}>
              {t('automation.action.disable')}
            </button>
          ) : null}
          {!rule.archived && !rule.enabled ? (
            <button
              ...
              onClick={() => void runRuleAction('enable_automation_rule', rule.ruleId)}
            >
              {t('automation.action.enable')}
            </button>
          ) : null}
          {!rule.archived ? (
            <button
              type="button"
              disabled={!canManage || busy}
              onClick={() => void runRuleAction('archive_automation_rule', rule.ruleId)}
            >
  ...
  const runColumns: DataTableColumn<RunDto>[] = [
    { key: 'rule', header: t('automation.col.rule'), render: (run) => run.ruleName ?? run.ruleId },
    { key: 'trigger', header: t('automation.col.trigger'), render: (run) => run.event },
    {
    ...
    { key: 'actor', header: t('automation.col.actor'), render: (run) => run.actor },
  ...
  const rulesPanel = (
    <div className="automations__panel">
      ...
      <DataTable
        ...
        emptyState={<EmptyState title={t('automation.empty.title')} hint={t('automation.empty.hint')} />}
      />
    </div>
  );
  ...
  const runsPanel = (
    <div className="automations__panel">
      <DataTable
        ...
        emptyState={<EmptyState title={t('automation.emptyRuns.title')} hint={t('automation.emptyRuns.hint')} />}
      />
    </div>
  );
    ...
    <section className="automations" aria-labelledby="automations-title">
      ...
      */}
      ...
      {state.kind === 'loading' ? (
        ...
        // the answer arrives.
        <div className="automations__skeletons">
          <Skeleton />
          <Skeleton />
          <Skeleton />
        </div>
