 ...
 * editor's Währung control is the shared `<Select>` over a local list, and the only FX read on this
 ...
 * portaled to <body>, so the option is queried from `screen` by its data-value, not from the trigger.
 */
async function chooseOption(combo: HTMLElement, value: string): Promise<void> {
...
describe('the BankAccountEditor drawer', () => {
  ...
  it('opens the recorded account with its IBAN grouped for reading', async () => {
    ...
    expect(within(drawer).getByLabelText('Währung')).toHaveTextContent('EUR');
  });
  ...
  it('offers only asset accounts in the picker, never 9100 Eröffnungsbilanz', async () => {
    ...
    // The shared <Select> paints its list only once open, portaled to <body>: open it, then read the
    // options from `screen`, stripping the selected row's ✓ from the text.
    await userEvent.click(picker);
    const labels = screen.getAllByRole('option').map((option) => option.textContent?.replace('✓', '').trim());
