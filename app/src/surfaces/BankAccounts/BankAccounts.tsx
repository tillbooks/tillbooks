...
export function BankAccounts() {
   ...
   */
  const rowItems = (account: BankAccount) => [
  ...
  const renderOpening = (account: BankAccount) => {
    ...
    return (
      <Link to="/journal" className="bank-money">
  ...
  const columns: DataTableColumn<BankAccount>[] = [
      ...
      render: (account) => (
        <>
          {canManage ? (
            <button
              type="button"
              className="bank-row-name"
              onClick={() => setParam('account', account.id)}
            >
              {account.name}
            </button>
          ) : (
          ...
          {account.archived && <span className="bank-chip">{t('bank.status.archived')}</span>}
        </>
      ),
    },
    {
    ...
    {
      key: 'actions',
      header: t('bank.rowActions'),
      headerHidden: true,
      align: 'end',
      render: (account) => {
        const items = rowItems(account);
        return items.length > 0 ? (
          <OverflowMenu label={t('bank.rowActionsFor', { name: account.name })} items={items} />
        ) : null;
      },
    },
  ];
