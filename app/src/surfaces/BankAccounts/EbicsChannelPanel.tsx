      ...
      {canPay ? (
        connectOpen ? (
          ...
          >
            ...
            <input id={`${hostFieldsId}-url`} placeholder="https://ebics.bank.example" value={form.url} onChange={(e) => setForm({ ...form, url: e.target.value })} />
            <label htmlFor={`${hostFieldsId}-hostId`}>{t('channel.wizard.hostId')}</label>
            <input id={`${hostFieldsId}-hostId`} placeholder="Host-ID" value={form.hostId} onChange={(e) => setForm({ ...form, hostId: e.target.value })} />
            <label htmlFor={`${hostFieldsId}-partnerId`}>{t('channel.wizard.partnerId')}</label>
            <input id={`${hostFieldsId}-partnerId`} placeholder="Partner-ID" value={form.partnerId} onChange={(e) => setForm({ ...form, partnerId: e.target.value })} />
            <label htmlFor={`${hostFieldsId}-userId`}>{t('channel.wizard.userId')}</label>
            <input id={`${hostFieldsId}-userId`} placeholder="User-ID" value={form.userId} onChange={(e) => setForm({ ...form, userId: e.target.value })} />
            <label htmlFor={`${hostFieldsId}-routeAccount`}>{t('channel.routeAccountLabel')}</label>
            <select id={`${hostFieldsId}-routeAccount`} value={form.bankAccountId} onChange={(e) => setForm({ ...form, bankAccountId: e.target.value })}>
              <option value="">{t('channel.routeAccount')}</option>
              {accounts.map((a) => (
        ...
        (managedOpen ? (
          ...
          >
            ...
            <select
              ...
              onChange={(e) => setManagedForm({ ...managedForm, bankAccountId: e.target.value })}
            >
              <option value="">{t('channel.routeAccount')}</option>
              {accounts.map((a) => (
                <option key={a.id} value={a.id}>
          ...
          <button type="button" className="btn btn--secondary" onClick={() => setManagedOpen(true)}>
            {t('channel.managed.connect')}
          </button>
        ))}
...
function ScheduleControl({
  ...
  return (
    <div className="bank-channel-schedule">
      ...
      {!schedule.enabled && (
        <select
          aria-label={t('channel.schedule.cadence')}
          value={cadence}
          disabled={!canManage || busy}
          onChange={(e) => onCadenceChange(e.target.value)}
        >
          <option value="daily">{t('channel.schedule.cadenceOption.daily')}</option>
          <option value="weekly">{t('channel.schedule.cadenceOption.weekly')}</option>
          <option value="monthly">{t('channel.schedule.cadenceOption.monthly')}</option>
        </select>
      )}
