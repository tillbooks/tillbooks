 ...
 * Rappen through `formatMoney` with the outgoing sign, `10.07.2026` is `2026-07-10` through
...
describe('the Kreditoren list, in all five states', () => {
  ...
  it('POPULATED: renders the recording with the sign carrying the direction and Swiss formats', async () => {
    renderBills();
    // The reference is the row's identity, the date renders dd.mm.yyyy, the money renders negative:
    // every figure on this screen is money going OUT, and the SIGN carries that (DESIGN.md).
    expect(await screen.findByRole('button', { name: 'LG-2026-0093' })).toBeInTheDocument();
    ...
    // The header total is the recorded base open figure, labelled as open.
    expect(screen.getByText("Offen CHF -3'783.50")).toBeInTheDocument();
  });

  it('shows every recorded status word, and the overdue chip only on an overdue open row', async () => {
    renderBills();
    ...
    expect(screen.getByText(`${overdue.daysOverdue} Tage`)).toBeInTheDocument();
  });
  ...
  it('renders the band with the SIGNED difference from the mismatch recording', async () => {
    ...
    await screen.findByText('LG-2026-0093');
    expect(screen.getByText(/weichen um CHF -12\.50 ab/)).toBeInTheDocument();
  ...
  it('offers Zahlung erfassen only on a posted row with an open amount', async () => {
    ...
    await userEvent.click(screen.getByRole('button', { name: 'Aktionen für LG-2026-0031' }));
    const menu = await screen.findByRole('menu');
    expect(within(menu).queryByRole('menuitem', { name: /Zahlung erfassen/ })).not.toBeInTheDocument();
  });
...
describe('the BillEditor drawer', () => {
  ...
  it('renders the cost-centre picker when centres exist, and sends the chosen id', async () => {
    ...
    await userEvent.selectOptions(within(dialog).getByLabelText('Lieferant'), 'v1');
    await userEvent.type(dialog.querySelector('#bill-amount') as HTMLElement, '1081');
    await userEvent.selectOptions(within(dialog).getByLabelText('Aufwand- oder Aktivkonto'), account('6500').id);
    await userEvent.selectOptions(within(dialog).getByLabelText('Kostenstelle'), 'cc1');
  ...
  it('renders the project picker when projects exist, and sends the chosen id (B03 cost dimension)', async () => {
    ...
    await userEvent.selectOptions(within(dialog).getByLabelText('Aufwand- oder Aktivkonto'), account('6500').id);
    await userEvent.selectOptions(within(dialog).getByLabelText('Projekt'), 'p1');
    await userEvent.click(within(dialog).getByRole('button', { name: 'Buchen' }));
  ...
  it('renders the rejection copy for a period lock, and every typed value survives it', async () => {
    ...
    expect(within(dialog).getByLabelText('Lieferant')).toHaveValue('v1');
  });
