...
export function Capture() {
  ...
  const columns: DataTableColumn<CaptureRow>[] = useMemo(
    () => [
      ...
      },
      {
        ...
        render: (row) => (
          <span className={`capture__status capture__status--${row.status}`}>
  ...
  const dropAction = canDrop ? (
    <label className="capture__drop">
      <span>{t('capture.action.drop')}</span>
      <input
        type="file"
        aria-label={t('capture.action.drop')}
        disabled={busy}
        onChange={(e) => {
  ...
  return (
    <section className="capture" aria-labelledby="capture-title">
      ...
      <DataTable
        ...
        rowClassName={(row) =>
          [row.id === selectedId ? 'capture__row--active' : '', row.id === justCommittedId ? COMMIT_TARGET_CLASS : '']
            .filter(Boolean)
            .join(' ') || undefined
        }
        emptyState={<EmptyState title={t('capture.empty.queue')} hint={t('capture.action.drop')} />}
      />
      ...
      {detail !== null ? (
        <DetailDrawer
          ...
          headerExtra={
            <span className={`capture__status capture__status--${detail.capture.status}`}>
                ...
                <tbody>
                  {liveFields.map((f) => (
                    <tr key={f.key}>
                      ...
                      <td className="capture__val">{renderValue(f)}</td>
                      <td>
          ...
          {detail.capture.status === 'needs_review' ? (
              ...
              {target === 'vendor_bill' && (
                <div className="capture__pickers">
                  ...
                  {taxCodes !== null && taxCodes.length > 0 && (
                    <label className="capture__picker" htmlFor="capture-tax">
                      ...
                      <select
                        ...
                        onChange={(e) => {
                          ...
                          setTaxChoice(e.target.value);
                        }}
                      >
                        <option value="">{t('capture.pick.taxChoose')}</option>
                        <option value={TAX_NONE}>{t('capture.pick.taxNone')}</option>
                        {taxCodes.map((c) => (
                          <option key={c.code} value={c.code}>
                            {c.label !== null && c.label !== '' ? `${c.code}, ${c.label}` : c.code}
                          </option>
                        ))}
                      </select>
                    </label>
            ...
            <p className="capture__committed" role="status">
              ...
              {detail.capture.targetKind === 'vendor_bill' && detail.capture.targetId !== null ? (
                <>
                  {' '}
                  <Link to={`/bills?bill=${encodeURIComponent(detail.capture.targetId)}`}>{t('capture.committed.openBill')}</Link>
                </>
