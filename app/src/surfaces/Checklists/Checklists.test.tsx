...
describe('Checklists list', () => {
  ...
  it('start: the dialog proposes the last ended period, calls checklist_start and lands on the run; a refusal stays inline', async () => {
    ...
    // The last ENDED period is pre-filled: with today far past 2026, that is the latest option. The
    // trigger shows its title (e.g. "Q2/2026"), so a quarter is pre-selected, never an empty picker.
    const select = within(dialog).getByLabelText(de.checklists.start.period.vat_period);
    expect(select).toHaveTextContent(/Q[1-4]\/\d{4}/);
...
describe('Checklists, the guided close (leg 2)', () => {
  ...
  it('the Start dialog offers the month and the year with locally derived ended periods and sends the picked template', async () => {
    ...
    const months = within(dialog).getByLabelText(de.checklists.start.period.month_close);
    fireEvent.click(months); // open the portaled listbox
    expect(screen.getAllByRole('option').length).toBe(6);
    expect(
      screen.getAllByRole('option').find((o) => o.getAttribute('aria-selected') === 'true')?.getAttribute('data-value'),
    ).toMatch(/^\d{4}-\d{2}$/);
    fireEvent.click(months); // close it again (re-clicking the trigger, not Escape, so the Modal stays open)
    ...
    const years = within(dialog).getByLabelText(de.checklists.start.period.year_close);
