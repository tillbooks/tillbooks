      ...
      <fieldset className="chk-choice">
        ...
        {options.map((o) => (
          <label key={o.id} className="chk-choice-option">
            <input type="radio" name={name} value={o.id} checked={picked === o.id} disabled={disabled} onChange={() => setPicked(o.id)} />
            ...
            {totalMinor !== null && (
              <tfoot>
                <tr>
                  ...
                  <td className="t-num">{money(totalMinor)}</td>
                </tr>
              </tfoot>
            )}
          </table>
        ...
        <dl className="chk-figures">
          {figures.map(([k, v]) => (
            <div key={k} className="chk-figure">
              ...
              <dd className={isMinorKey(k) ? 't-num' : undefined}>{isMinorKey(k) ? money(v) : String(v)}</dd>
            </div>
          ))}
        </dl>
      )}
            ...
            <tbody>
              {settlements.map((s) => (
                <tr key={s.label} data-period={s.label}>
                  ...
                  <td>
                    {!s.filed ? (
                      <Link to="/checklisten">{t('checklists.settlement.notFiled')}</Link>
                    ) : s.nothingToSettle ? (
                  ...
                  <td className="t-num">{money(s.netMinor)}</td>
  ...
  return (
    <div className="chk-body chk-body--validation" data-result={v.result}>
      <p className="chk-validation-word" data-result={v.result}>
        <span aria-hidden="true">{v.result === 'pass' ? '✓' : v.result === 'unavailable' ? '?' : '!'}</span> {word}
      ...
      {figures.length > 0 && (
        <dl className="chk-figures">
          {figures.map(([k, val]) => (
            <div key={k} className="chk-figure">
              <dt>{knownKey(`checklists.figure.${k}`) ? t(`checklists.figure.${k}`) : k}</dt>
              <dd className={isMinorKey(k) ? 't-num' : undefined}>{isMinorKey(k) ? money(val) : String(val)}</dd>
            </div>
          ))}
        </dl>
      )}
      ...
      {state.status === 'loaded' && (
        <dl className="chk-figures">
          <div className="chk-figure">
            ...
            <dd className="t-num">{formatMoney(state.aktivenMinor, state.currency)}</dd>
          </div>
          <div className="chk-figure">
            <dt>{t('checklists.statements.passiven')}</dt>
            <dd className="t-num">{formatMoney(state.passivenMinor, state.currency)}</dd>
          </div>
          <div className="chk-figure">
            <dt>{t('checklists.statements.reingewinn')}</dt>
            <dd className="t-num">{formatMoney(state.reingewinnMinor, state.currency)}</dd>
          </div>
        </dl>
      )}
      <p className="chk-provenance">
