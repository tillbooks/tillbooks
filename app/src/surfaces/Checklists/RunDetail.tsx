  ...
  const titleAside = (item: ItemView): ReactNode => {
    ...
    return (
      <Tooltip content={t('checklists.checkKey', { key })}>
        {/* An info mark from the icon language (K-22: no ⓘ dingbat), dim because it is a control. */}
    ...
    <div className="chk-detail">
      ...
      {autostart !== null && (
        <p className="chk-detail-provenance" data-testid="chk-provenance">
          {t('checklists.detail.autostarted', { rule: t(`checklists.template.${autostart}`), date: run.createdAt === null ? '' : formatDate(run.createdAt.slice(0, 10)) })} <Link className="link-inline" to="/automations">{t('checklists.detail.automations')}</Link>
        </p>
      )}
