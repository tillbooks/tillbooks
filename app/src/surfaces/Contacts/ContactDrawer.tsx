 ...
 * primitive was grounded in: the bespoke drawer had neither a focus trap nor Escape. The tab strip
 * stays HAND-ROLLED on purpose: its panels load their own data on open (the timeline, the tasks, the
 * two portal panels), and the shared `Tabs` primitive keeps every panel mounted, which would fire all
 * of those reads at once and change the surface's network behaviour. The C3 `Provenance` line is not
...
export function ContactDrawer({
  ...
  return (
    <DetailDrawer
      ...
      footer={footer}
    >
      <DrawerTabs tab={tab} onSelect={setTab} />

      <div className="ct-drawer-tabhost">
        {tab === 'master' && (
        ...
        {tab === 'portal' && (
          <div role="tabpanel" aria-labelledby="ct-tab-portal" className="ct-tabpanel">
...
/** The tablist. Arrow keys move between tabs, which is what makes a tablist a tablist. */
function DrawerTabs({ tab, onSelect }: { tab: Tab; onSelect: (t: Tab) => void }) {
  const t = useT();
  ...
  function onKeyDown(event: React.KeyboardEvent) {
    ...
    const target = TABS[(next + TABS.length) % TABS.length] as Tab;
    onSelect(target);
    refs.current[target]?.focus();
  }

  return (
    <div className="ct-tabs" role="tablist" aria-label={t('contact.tabs')} onKeyDown={onKeyDown}>
      {TABS.map((id) => (
        <button
          key={id}
          type="button"
          ...
          tabIndex={tab === id ? 0 : -1}
          className={`ct-tab${tab === id ? ' ct-tab-active' : ''}`}
          onClick={() => onSelect(id)}
        >
          {t(`contact.tab.${id}`)}
        </button>
...
function MasterTab({
  ...
  return (
    <div role="tabpanel" aria-labelledby="ct-tab-master" className="ct-tabpanel">
...
/** ○ Offen · ◐ In Arbeit · ✓ Erledigt · ✕ Abgebrochen: glyph AND label, never colour alone (WCAG 1.4.1). */
const TASK_STATUS_GLYPH: Record<string, string> = { open: '○', doing: '◐', done: '✓', cancelled: '✕' };
        ...
        >
          {RECURRENCE_PRESETS.map((p) => (
            <option key={p.key} value={p.key}>
              {t(`contact.tasks.recurrence.preset.${p.key}`)}
            </option>
          ))}
          <option value="custom">{t('contact.tasks.recurrence.preset.custom')}</option>
        </select>
      </label>
      {showAdvanced && (
        <>
  ...
  return (
    <section className="ct-subsection ct-tasks">
      ...
      {loading ? (
        ...
        <ErrorBanner message={t('contact.tasks.loadFailed')} onRetry={() => void load()} />
      ) : tasks.length === 0 ? (
            ...
            return (
              <li key={task.id} className="ct-task-row">
                <div className="ct-task-main">
                  <span className="ct-task-status">
                    {/* glyph AND label: a status is never signalled by glyph or colour alone (WCAG 1.4.1). */}
                    <span aria-hidden="true">{TASK_STATUS_GLYPH[task.status] ?? '○'}</span>
                    <span className="ct-task-status-label">{t(`contact.tasks.status.${task.status}`)}</span>
                  </span>
                  <span className="ct-task-title">{task.title}</span>
                  {task.recurrenceRule !== null && (
                    <span className="ct-task-badge" title={task.recurrenceRule}>
                      <span aria-hidden="true">↻</span> {t('contact.tasks.recurring.badge')}
                    </span>
                  )}
                </div>
                ...
                {live && (
                  <div className="ct-task-actions">
                    ...
                    >
                      <span aria-hidden="true">✓</span> {t('contact.tasks.action.complete')}
                    </button>
                    ...
                    {canWrite && (
                      <button type="button" className="btn btn--ghost btn--sm" onClick={() => cancel(task.id)}>
                        {t('contact.tasks.action.cancel')}
                      </button>
                    )}
                  </div>
                )}
...
function TimelineTab({ contact, workspaceId }: { contact: Contact; workspaceId: string }) {
  ...
  return (
    <div role="tabpanel" aria-labelledby="ct-tab-timeline" className="ct-tabpanel">
          ...
          <ul className="ct-timeline">
            {activities.map((a) => (
              <li key={a.id} className="ct-timeline-row">
                ...
                <span className="ct-timeline-when t-num">{formatDay(a.occurredAt)}</span>
              </li>
            ))}
          </ul>
        )}
...
/** The status glyph+label pairs (spec §6): ● Aktiv · ◐ Entwurf · ⊘ Widerrufen · ▼ Abgelaufen. */
const PORTAL_STATUS_GLYPH: Record<PortalGrant['status'], string> = {
  active: '●',
  draft: '◐',
  revoked: '⊘',
  expired: '▼',
    ...
    <div className="ct-portal-section">
      ...
      {oneTimeLink !== null && (
        <section className="ct-subsection">
          ...
          <div className="ct-field-row">
            <input className="ct-input" readOnly value={oneTimeLink} aria-label={t('portal.link.once')} />
            <button
              type="button"
              className="btn btn--secondary btn--sm"
          ...
          <ul className="ct-portal-list">
            {grants.map((g) => (
              <li key={g.id} className="ct-portal-row">
                <span className="ct-portal-status">
                  {/* glyph AND label, never colour-only (WCAG 2.2 AA). */}
                  <span aria-hidden="true">{PORTAL_STATUS_GLYPH[g.status]}</span>
                  <span className="ct-portal-status-label">{t(`portal.status.${g.status}`)}</span>
                </span>
                <span className="ct-portal-expiry t-num">{formatDay(g.expiresAt)}</span>
...
/**
 * de-CH day formatting (P11): `31.12.2026`. An ISO day or a full ISO timestamp both resolve; an
 * unparseable value is shown verbatim rather than as "Invalid Date", because the raw string is at
 * least true.
 */
...
function FinanceTab() {
  const t = useT();
  return (
    <div role="tabpanel" aria-labelledby="ct-tab-finance" className="ct-tabpanel">
      <p className="ct-field-hint">{t('contact.finance.hint')}</p>
    </div>
  );
}
  ...
  return (
    <div className="ct-confirm">
      ...
      <label className="ct-field-inner">
        ...
        <input className="ct-input" value={typed} onChange={(event) => setTyped(event.target.value)} />
      </label>
