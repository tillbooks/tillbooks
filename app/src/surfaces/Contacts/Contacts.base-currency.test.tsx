...
describe('Contacts, the base currency is a setting and not a synonym for CHF', () => {
  ...
  it('EMPTY: the first contact of an empty book is created in the base currency, not CHF', async () => {
    ...
    expect(within(dialog).getByRole('combobox', { name: 'Default currency' })).toHaveTextContent('EUR');
  });
  ...
  it('ERROR: a failed profile read falls back to CHF, and does not take the list down with it', async () => {
    ...
    const dialog = await screen.findByRole('dialog');
    expect(within(dialog).getByRole('combobox', { name: 'Default currency' })).toHaveTextContent('CHF');
  ...
  it('SUCCESS/edit: keeps a contact on its OWN currency, which is not the workspace default', async () => {
    ...
    // K-21: Edit sits in the row's one overflow.
    await userEvent.click(await screen.findByRole('button', { name: /^More actions for contact/ }));
    await userEvent.click(screen.getByRole('menuitem', { name: 'Edit' }));
    const dialog = await screen.findByRole('dialog');
    expect(within(dialog).getByRole('combobox', { name: 'Default currency' })).toHaveTextContent('USD');
  ...
  it('offers only currencies the engine accepts, so no pick is a guaranteed rejection', async () => {
    ...
    await userEvent.click(await screen.findByRole('button', { name: 'New contact' }));
    const dialog = await screen.findByRole('dialog');
    // Open the portaled listbox and read its option labels (the check glyph is aria-hidden and lives
    // outside the label span, so it never joins the compared text).
    await userEvent.click(within(dialog).getByRole('combobox', { name: 'Default currency' }));
    const options = screen.getAllByRole('option');
    expect(options.map((o) => o.querySelector('.select-option-label')?.textContent)).toEqual(['CHF', 'EUR', 'USD']);
  });
});
