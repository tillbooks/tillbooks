...
const happy = (): Canned => ({
  ...
  tasks_list: ok({ tasks: [] }),
  // E00: the Stammdaten tab now mounts the shared LinkedFiles panel, which reads on open. A contact
  // with no attached files renders the empty state (not an error), so these C00 tests keep a single
  // alert to assert on rather than colliding with a spurious files transport banner.
...
/** Open the detail drawer for one contact by clicking its row, the opener since K-21. */
...
describe('Contacts, the C00 kind axis', () => {
  ...
  it('filters by kind through the engine rather than in the browser', async () => {
    ...
    await userEvent.click(screen.getByRole('option', { name: 'Person' }));
...
describe('ContactDrawer, the Verlauf tab (OP5)', () => {
  it('shows an empty timeline with a way out, then logs an activity', async () => {
    ...
    await userEvent.click(within(drawer).getByRole('combobox', { name: 'Art' }));
    await userEvent.click(screen.getByRole('option', { name: 'Anruf' }));
...
describe('Contacts, the employer picker (US-C00.1)', () => {
  it('offers companies only, so the form cannot create the rejection it would then show', async () => {
    ...
    // the options from the named listbox, stripping the selected row's ✓.
    await userEvent.click(within(dialog).getByRole('combobox', { name: 'Arbeitgeber' }));
    const labels = within(screen.getByRole('listbox', { name: 'Arbeitgeber' }))
      .getAllByRole('option')
  ...
  it('sends the kind and the employer link on create', async () => {
    ...
    await chooseOption('Arbeitgeber', 'k1');
