...
describe('ContactEditor', () => {
  it('creates a contact and calls create_contact with structured fields', async () => {
    ...
    await userEvent.click(within(dialog).getByRole('combobox', { name: 'Rolle' }));
  ...
  it('K-31: opens on its first field, and an empty save marks the name inline and takes the focus there', async () => {
    const createSpy = vi.fn<CannedHandler>(() => ok({ contactId: 'new1' }));
    renderContacts({ ...happyCanned(), create_contact: createSpy });
    await screen.findByText('Muster AG');
    ...
    await userEvent.click(screen.getByRole('button', { name: 'Neuer Kontakt' }));
    const dialog = await screen.findByRole('dialog');
    // The first key press in a fresh drawer never lands on its close X.
    await waitFor(() => expect(document.activeElement).not.toBe(within(dialog).getByRole('button', { name: 'Schliessen' })));
    ...
    expect(within(dialog).getByText('Name fehlt.')).toHaveClass('field-error');
    await waitFor(() => expect(document.activeElement).toBe(name));
    // The field says it, so no banner and no write: the engine is never asked to refuse it.
    expect(dialog.querySelector('.error-banner')).toBeNull();
    expect(createSpy).not.toHaveBeenCalled();

    // Typing clears the mark, and the save goes through.
    await userEvent.type(name, 'Neue AG');
  ...
  it('edits a contact and calls update_contact with a patch', async () => {
    ...
    const row = (await screen.findByText('Muster AG')).closest('tr') as HTMLElement;
    // K-21: Bearbeiten sits in the row's one overflow, beside no other button.
    await userEvent.click(within(row).getByRole('button', { name: 'Weitere Aktionen für Kontakt Muster AG' }));
