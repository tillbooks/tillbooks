...
const STATUS_GLYPH: Record<GrantStatus, string> = {
  active: '●',
  draft: '◐',
  revoked: '⊘',
  expired: '▼',
      ...
      {oneTimeLink !== null && (
        <div className="ct-subsection">
          <div className="ct-field-row">
            <input className="ct-input" readOnly value={oneTimeLink} aria-label={t('vendorPortal.notice.token_once')} />
            <button
              type="button"
      ...
      {canManage && (
        <div className="ct-subsection">
          <label className="ct-field-inner">
            ...
            <input type="date" className="ct-input" value={expiresAt} onChange={(e) => setExpiresAt(e.target.value)} />
          </label>
        ...
        <ul className="ct-portal-grants">
          {grants.map((g) => (
            <li key={g.id} className="ct-portal-grant">
              <span className="ct-grant-status">
                <span aria-hidden="true">{STATUS_GLYPH[g.status]}</span>
                <span>{t(`vendorPortal.status.${g.status}`)}</span>
              </span>
              <span className="ct-grant-expiry">{formatDate(g.expiresAt)}</span>
          ...
          <ul className="ct-portal-preview">
            {pos.map((p) => (
              <li key={p.id}>
                ...
                <span>{formatMoney(p.totalRappen, p.currency)}</span>
              </li>
            ))}
          </ul>
        )}
          ...
          <ul className="ct-portal-preview">
            {advices.map((a) => (
              <li key={a.id}>
                ...
                <span>{formatMoney(a.totalRappen, a.currency)}</span>
              </li>
            ))}
          </ul>
        )}
