...
describe('Korrespondenz, the queue', () => {
  it('groups threads into the three buckets, direction as glyph AND label, draft badge on drafted', async () => {
    ...
    renderMail(canned);
    expect(await screen.findByRole('row', { name: `${de.mail.action.open}: Terminverschiebung` })).toBeInTheDocument();
  ...
  it('opens a thread and renders bodies, the stale warning in words, and message_moved per message', async () => {
    ...
    renderMail(canned);
    await userEvent.click(await screen.findByRole('row', { name: `${de.mail.action.open}: Terminverschiebung` }));
  ...
  it('has no axe violations on the loaded queue', async () => {
    ...
    await screen.findByRole('row', { name: `${de.mail.action.open}: Terminverschiebung` });
