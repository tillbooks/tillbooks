...
const DIRECTION_GLYPH: Record<string, string> = { inbound: '←', outbound: '→' };
...
export function Correspondence() {
    ...
    >
      ...
      <div className="mail-connect-fields">
        <label className="mail-field">
          ...
          <select value={connect.adapter} onChange={(e) => setConnect({ ...connect, adapter: e.target.value })}>
            <option value="apple_mail">{t('mail.connect.adapter_apple_mail')}</option>
            <option value="thunderbird">{t('mail.connect.adapter_thunderbird')}</option>
          </select>
        </label>
        <label className="mail-field">
          <span>{t('mail.connect.storePath')}</span>
          <input type="text" value={connect.storePath} onChange={(e) => setConnect({ ...connect, storePath: e.target.value })} required />
        </label>
        <label className="mail-field">
          <span>{t('mail.connect.address')}</span>
          <input type="email" value={connect.address} onChange={(e) => setConnect({ ...connect, address: e.target.value })} required />
        </label>
      </div>
  ...
  const columns: DataTableColumn<ThreadItem>[] = [
    {
      ...
      render: (thread) => (
        <span className="mail-direction">
          <span aria-hidden="true">{DIRECTION_GLYPH[thread.lastDirection ?? 'inbound'] ?? '←'}</span>{' '}
      ...
      render: (thread) => (
        <span className="mail-subject-cell">
          <button
            ...
            aria-label={`${t('mail.action.open')}: ${thread.subject ?? t('mail.thread.noSubject')}`}
          ...
          {thread.draftReady && (
            <span className="mail-badge">
              <span aria-hidden="true">✎</span> {t('mail.draft.badge')}
            </span>
          )}
        </span>
      ),
      ...
      ? [
          {
            ...
            render: (thread: ThreadItem) => (
              <label className="mail-label-control">
                <select
                  aria-label={t('mail.field.label')}
                  value={labels[thread.id] ?? ''}
                  onChange={(e) => void setLabel(thread.id, e.target.value)}
                >
                  <option value="">{t('mail.field.none')}</option>
                  {labelField.options.map((option) => (
  ...
  return (
    <section className="mail" aria-labelledby="mail-title">
      ...
      {failed && <ErrorBanner message={t('mail.error.transport')} onRetry={() => void load()} />}

      {connecting && canWrite && connectPanel}

      {loading ? (
        <div role="status" aria-busy="true" aria-live="polite">
          <span className="mail-sr">{t('mail.loading')}</span>
          <Skeleton rows={4} />
        </div>
            ...
            {messages.map((message) => (
              <article key={message.id} className="mail-message">
                <header className="mail-message-head">
                  <span className="mail-direction">
                    <span aria-hidden="true">{DIRECTION_GLYPH[message.direction] ?? '←'}</span>{' '}
                  ...
                  <>
                    {message.stale === true && (
                      <p className="mail-message-issue">
                        <span aria-hidden="true">⚠</span> {t('mail.msg.stale')}
                      </p>
                    )}
        ...
        <>
          ...
          {canSeeBooks && (
            <p className="mail-draft-grounding">
              <span aria-hidden="true">{latest.grounded ? '▣' : '○'}</span>{' '}
              {latest.grounded ? t('draft.grounded.on') : t('draft.grounded.off')}
