...
export function CustomFieldRow({
  ...
  function control() {
    switch (def.type) {
      ...
      case 'select':
        // Single-choice: the shared <Select> (a styled combobox), so the open list is the Brass
        // popover and not the OS-painted native menu.
        return (
          <Select
            id={inputId}
            value={typeof value === 'string' ? value : ''}
      ...
      case 'multiselect':
        // Left native on purpose: the shared <Select> is single-choice, and a <select multiple> is a
        // different control (multiple concurrent values). Migrating it would change its behaviour.
        return (
          <select className="field custom-field-multi"
            {...common}
            multiple
