...
describe('Customization, the entity picker', () => {
  it('offers every registered kind, each with a translated label', async () => {
    ...
    const picker = await screen.findByLabelText(de.customization.entityKind);
    await userEvent.click(picker);
    // Options portal to <body> and carry their value on `data-value`, not a native `value` attribute.
    ...
    for (const value of offered) {
      expect(screen.getByRole('option', { name: (de.customization.entity as Record<string, string>)[value!] })).toBeInTheDocument();
    }
  });
  ...
  it('re-reads the engine for the newly chosen kind rather than filtering what it already had', async () => {
    ...
    await user.click(screen.getByLabelText(de.customization.entityKind));
    await user.click(screen.getAllByRole('option').find((o) => o.getAttribute('data-value') === 'journal_entry')!);
