...
export function Customization() {
  ...
  const openDraft = () => setDraft({ ...EMPTY_DRAFT, entityKind });

  const panelBody = (content: ReactNode): ReactNode => {
    if (state.kind === 'loading') {
      // K-34: the shared skeleton in the list's row height, never a bare spinner.
      return <Skeleton rows={3} height={36} />;
    }
    if (state.kind === 'error') {
      return <ErrorBanner error={state.error} context="read" onRetry={() => void load()} />;
    }
    return content;
  };
  ...
  const fieldsPanel = (
    <div className="customization__panel">
      {panelBody(
        <>
          {/* K-08: "Feld definieren" is the header's one primary (at its natural width, never a bar
              across the panel); the draft form opens here. */}
          {canManageFields ? (
            draft === null ? null : (
              <form
                ...
                <label htmlFor={typeSelectId}>
                  ...
                  <Select
                    ...
                    options={FIELD_TYPES.map((type) => ({
                      value: type,
                      label: t(`customization.fields.type.${type}`),
                    }))}
                    ariaLabel={t('customization.fields.typeLabel')}
                  />
                </label>
            ...
            <div className="customization__drafts">
              {draftDefs.map((def) => (
                <p key={def.fieldDefId} className="customization__draft-banner">
                  {/* K-22: the shared Status (a clock plus the words), never the ◷ dingbat. */}
                  <Status kind="pending" label={labelFor(def, locale)} /> {t('customization.fields.draft_banner')}
                  {/*
                  ...
                  */}
                  {canManageFields ? (
                    <span className="customization__draft-actions">
                      {/* Approving a schema change is not a money write: a secondary, never the accent. */}
                      <button
                        type="button"
                        className="btn btn--secondary btn--sm"
                        disabled={busy}
          ...
          {liveDefs.length === 0 && draftDefs.length === 0 ? (
            <EmptyState
              ...
              action={canManageFields ? { label: t('customization.fields.create'), onClick: openDraft } : undefined}
            />
          ) : liveDefs.length > 0 ? (
            <DataTable
  ...
  return (
    <section className="customization" aria-labelledby={titleId}>
      <SurfaceHeader
        ...
        actions={
          tab === 'fields' && canManageFields && draft === null && state.kind === 'ok' ? (
      ...
      <label className="customization__kind" htmlFor={kindSelectId}>
        ...
        <Select
          ...
          options={ENTITY_KINDS.map((kind) => ({
            value: kind,
            label: t(`customization.entity.${kind}`),
          }))}
          ariaLabel={t('customization.entityKind')}
        />
      </label>
