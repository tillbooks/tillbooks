...
describe('CycleCounts, the stocktake surface', () => {
  ...
  it('commit opens a confirm and fires inventory_stocktake_commit ONLY after the operator confirms (C4)', async () => {
    ...
    await screen.findByText('Widget');
    // The commit writes quantity movements with no ledger reach, so it is never the money tint (K-08,
    // the C2 money critic, F5): secondary beside the header's primary, primary in its confirm.
    const opener = screen.getByRole('button', { name: de.cycleCounts.commit });
    ...
    const confirmCommit = within(dialog).getByRole('button', { name: de.cycleCounts.commit });
