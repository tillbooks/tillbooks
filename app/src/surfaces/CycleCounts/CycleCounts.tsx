...
/** A session's and a line's state as the one `Status` word (K-22), never a bordered badge. */
const SESSION_KIND: Record<string, StatusKind> = { open: 'neutral', review: 'warn', committed: 'success', cancelled: 'inactive' };
const LINE_KIND: Record<string, StatusKind> = { pending: 'neutral', counted: 'pending', review_required: 'warn', approved: 'success' };
...
export function CycleCounts() {
  ...
  // K-34: the list's own skeleton, never before 200ms and never for less than 300ms once shown.
  ...
  const lineColumns = useMemo<DataTableColumn<Line>[]>(() => {
    ...
    cols.push({
      ...
      render: (l) => <Status kind={LINE_KIND[l.status] ?? 'neutral'} label={t(`cycleCounts.lineStatus.${l.status}`)} />,
    });
    if (isOpenLike) {
      cols.push({
        key: 'actions',
        header: t('cycleCounts.col.actions'),
        // K-21: the count's own commit stays beside its field; Nachzählen sits behind the overflow.
        render: (l) => (
          <span className="cc-row-actions">
            <button type="button" className="btn btn--secondary btn--sm" disabled={!canWrite} onClick={() => void saveCount(l)}>
              {t('cycleCounts.saveCount')}
            </button>
            {l.countedQty !== null && canWrite && (
              <OverflowMenu
                quiet
                label={t('cycleCounts.lineActions', { item: l.itemName ?? l.itemId })}
                items={[{ key: 'recount', label: t('cycleCounts.recount'), onSelect: () => void requestRecount(l) }]}
              />
            )}
          </span>
        ),
  ...
  return (
    <div className="cc">
      ...
      {failed && (
        <ErrorBanner context="read" message={t('cycleCounts.error.transport')} onRetry={() => void loadList()} />
      )}

      {showSkeleton ? (
        <DataTable columns={[]} rows={[]} rowKey={() => ''} loading skeletonRows={4} />
      ) : sessions.length === 0 ? (
        <EmptyState
          title={t('cycleCounts.empty.title')}
          hint={t('cycleCounts.empty.hint')}
          {...(canWrite
            ? {
                action: {
                  label: t('cycleCounts.new'),
                  onClick: () => {
                    setWriteError(null);
                    setCreateDraft(emptyCreate());
                    setCreateDrawer(true);
        ...
        <div className="cc-split">
          <section className="cc-pane" aria-label={t('cycleCounts.listLabel')}>
            <ul className="cc-list">
              {sessions.map((s) => (
                <li key={s.id}>
                  {/* A list row: the open session is current (K-17, K-22), the pill, announced. */}
                  <button
                    type="button"
                  ...
                  >
                    <span className="cc-li-main">
                      ...
                      <Status kind={SESSION_KIND[s.status] ?? 'neutral'} label={t(`cycleCounts.status.${s.status}`)} />
                    </span>
                    <span className="cc-li-sub">
                      {formatCalendar(s.freezeAt)} · {s.progressPct}% · {t('cycleCounts.lineCount', { n: s.totalLines })}
                    </span>
                  </button>
                </li>
              ))}
                ...
                <div className="cc-detail-head">
                  <div>
                    <h2 className="cc-detail-title">
                      {t(`cycleCounts.type.${detail.session.type}`)}{' '}
                      <Status kind={SESSION_KIND[detail.session.status] ?? 'neutral'} label={t(`cycleCounts.status.${detail.session.status}`)} />
                    </h2>
                    <p className="cc-muted">
                      {t('cycleCounts.freeze')}: {formatCalendar(detail.session.freezeAt)} · {t('cycleCounts.progress')}: {detail.session.progressPct}%
                    </p>
                  </div>
                  <div className="cc-actions">
                    ...
                    {/* The commit writes quantity movements and has NO ledger reach
                        (`inventory_stocktake_commit`), so it is not the money tint (K-08, the C2 money
                        critic, F5). The header holds the one primary, so this is secondary; the
                        consequence confirm it opens carries the primary. */}
                    <button type="button" className="btn btn--secondary" disabled={!canWrite || !isOpenLike} onClick={() => setPendingWrite('commit')}>
      ...
      >
        ...
        <div className="cc-field">
          ...
          <Select
            ...
            onChange={(value) => setCreateDraft({ ...createDraft, type: value as SessionType })}
            options={SESSION_TYPES.map((ty) => ({ value: ty, label: t(`cycleCounts.type.${ty}`) }))}
            ariaLabel={t('cycleCounts.form.type')}
          />
        </div>
        <div className="cc-field">
