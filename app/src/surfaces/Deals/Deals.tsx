...
/** ● Offen · ✓ Gewonnen · ✕ Verloren: glyph AND label, never colour alone. */
const STATUS_GLYPH: Record<string, string> = { open: '●', won: '✓', lost: '✕' };
...
export function Deals() {
    ...
    return (
      <li key={deal.id} className="deals-card">
        <button type="button" className="deals-card-main" onClick={() => setOpenId(openId === deal.id ? null : deal.id)}>
          ...
          <span className="deals-card-contact">{contacts.get(deal.contactId) ?? deal.contactId}</span>
          <span className="deals-card-meta">
            <span className="deals-money">{formatMoney(deal.valueMinor, deal.currency)}</span>
            <span>{deal.probability}%</span>
            {deal.status !== 'open' && (
              <span className="deals-status">
                <span aria-hidden="true">{STATUS_GLYPH[deal.status]}</span> {t(`deals.status.${deal.status}`)}
              </span>
            )}
          </span>
        </button>
        {live && canWrite && (
          <label className="deals-move">
            <span>{t('deals.action.move')}</span>
            <select className="field" value={deal.stageId} onChange={(e) => void pickStage(deal, e.target.value)}>
        ...
        {lostFor === deal.id && (
          ...
          >
            ...
            <div className="deals-form-actions">
              ...
              <button type="button" className="btn btn--secondary btn--sm" onClick={() => setLostFor(null)}>
  ...
  const dealMatches = (deal: DealView): boolean => {
    ...
    if (q === '') return true;
    const contactName = (contacts.get(deal.contactId) ?? deal.contactId).toLowerCase();
    return deal.title.toLowerCase().includes(q) || contactName.includes(q);
  };
    ...
    return (
      <section key={stage.id} className="deals-column" aria-labelledby={`deals-col-${stage.id}`}>
        <h2 id={`deals-col-${stage.id}`} className="deals-column-title">
          {stage.name}
          <span className="deals-count">{cards.length}</span>
        </h2>
        {cards.length === 0 ? (
      ...
      >
        ...
        {views.length > 0 && (
          <label className="deals-picker">
            ...
            <select className="field" value={viewId} onChange={(e) => setViewId(e.target.value)}>
              <option value="">{t('deals.view.all')}</option>
              {views.map((view) => (
      ...
      {creating && canWrite && (
        ...
        >
          <label className="deals-field">
            ...
            <select className="field" value={draft.contactId} onChange={(e) => setDraft({ ...draft, contactId: e.target.value })} required>
              <option value="">{t('deals.field.contactPick')}</option>
          ...
          <ul className="deals-settings-stages">
            {board.stages.map((stage) => (
              <li key={stage.id}>
                ...
                <span className="deals-settings-stage-meta">
      ...
      <div className="deals-workspace">
        ...
        {selected !== undefined && (
          ...
          <header className="deals-drawer-head">
            ...
            <button type="button" className="btn btn--secondary btn--icon btn--sm" aria-label={t('deals.drawer.close')} onClick={() => setOpenId(null)}>
              <span aria-hidden="true">✕</span>
            </button>
          </header>
          <dl className="deals-drawer-facts">
            <dt>{t('deals.field.contact')}</dt>
            <dd>{contacts.get(selected.contactId) ?? selected.contactId}</dd>
            <dt>{t('deals.field.value')}</dt>
            <dd className="deals-money">
              {formatMoney(selected.valueMinor, selected.currency)}
              {board !== null && selected.currency !== board.baseCurrency && (
                <span className="deals-base"> ({formatMoney(selected.valueBaseMinor, board.baseCurrency)})</span>
              )}
            </dd>
            <dt>{t('deals.field.weighted')}</dt>
            <dd className="deals-money">{board === null ? '-' : formatMoney(selected.weightedMinor, board.baseCurrency)}</dd>
            <dt>{t('deals.field.status')}</dt>
            <dd>
              <span aria-hidden="true">{STATUS_GLYPH[selected.status] ?? '●'}</span> {t(`deals.status.${selected.status}`)}
              {selected.lostReason !== null && ` (${selected.lostReason})`}
            </dd>
          ...
          {canWrite && (
            ...
            >
              ...
              <label className="deals-field">
                ...
                <select className="field" value={activity.kind} onChange={(e) => setActivity({ ...activity, kind: e.target.value })}>
                  {ACTIVITY_KINDS.map((kind) => (
                    <option key={kind} value={kind}>
                      {t(`deals.activity.kinds.${kind}`)}
                    </option>
                  ))}
                </select>
              </label>
