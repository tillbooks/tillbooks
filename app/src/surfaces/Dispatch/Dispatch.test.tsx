...
describe('Dispatch', () => {
  ...
  it('SUCCESS (Textbausteine): lede, tabs and the saved slot prefill render; the preview resolves', async () => {
    ...
    // K-11: the kind is a Segmented value, a radio in a named group, the chosen one checked.
    expect(screen.getByRole('radio', { name: de.dispatch.kind.invoice })).toHaveAttribute('aria-checked', 'true');
  ...
  it('PROTOKOLL: rows render outcome as glyph + label with a document link; empty state names the first action', async () => {
    ...
    await user.click(screen.getByLabelText(de.dispatch.log.filterLabel));
    await user.click(screen.getAllByRole('option').find((o) => o.getAttribute('data-value') === 'failed')!);
