...
/** Outcome renders as GLYPH + TEXT, never colour alone (§10.6). */
const OUTCOME_GLYPH: Record<Outcome, string> = { sent: '✓', degraded: '◌', failed: '✕', artifact_created: '▤' };
...
export function Dispatch() {
  ...
  const textsTab = (
    <div className="dispatch-body">
      <div className="dispatch-main">
        <div className="dispatch-kinds" role="tablist" aria-label={t('dispatch.tab.texts')}>
          {KINDS.map((k) => (
            <button
              key={k}
              type="button"
              ...
              className={`dispatch-tab${kind === k ? ' dispatch-tab--active' : ''}`}
              onClick={() => {
                setKind(k);
                setSaved(false);
              }}
            >
              {t(`dispatch.kind.${KIND_KEY[k]}`)}
            </button>
          ))}
        </div>
        <div className="dispatch-locales" role="tablist" aria-label={t('dispatch.texts.localeTabs')}>
          {LOCALES.map((l) => (
            <button
              key={l}
              type="button"
              ...
              className={`dispatch-tab${locale === l ? ' dispatch-tab--active' : ''}`}
              onClick={() => {
                setLocale(l);
                setSaved(false);
              }}
  ...
  const logColumns: DataTableColumn<LogRow>[] = [
    { key: 'date', header: t('dispatch.log.date'), render: (row) => formatDay(row.sentAt) },
    ...
    { key: 'channel', header: t('dispatch.log.channel'), render: (row) => t(`dispatch.log.channelName.${row.channel}`) },
    {
      ...
      render: (row) => (
        <>
          {/* Glyph + text, never colour alone; the glyph is decoration for AT. */}
          <span aria-hidden="true">{OUTCOME_GLYPH[row.outcome]}</span>{' '}
          {t(`dispatch.log.outcome.${OUTCOME_KEY[row.outcome]}`)}
    ...
    },
    {
      ...
      render: (row) =>
        row.documentId !== null ? (
          <Link to={`/documents/${row.documentId}`}>{t('dispatch.log.open')}</Link>
        ) : row.dunningRunId !== null ? (
          <Link to="/dunning">{t('dispatch.log.openRun')}</Link>
        ) : null,
    },
  ];
  ...
  const logTab = (
    <div className="dispatch-main">
      <label className="dispatch-filter">
        <span>{t('dispatch.log.filterLabel')}</span>
        <select value={outcomeFilter} onChange={(e) => setOutcomeFilter(e.target.value as '' | Outcome)}>
          <option value="">{t('dispatch.log.filterAll')}</option>
          {OUTCOMES.map((o) => (
            <option key={o} value={o}>
              {t(`dispatch.log.outcome.${OUTCOME_KEY[o]}`)}
            </option>
          ))}
        </select>
      </label>
