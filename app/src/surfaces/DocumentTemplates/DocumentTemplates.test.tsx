...
describe('DocumentTemplates', () => {
  ...
  it('PERMISSION-DENIED (write): without the capability the controls are PRE-disabled with the reason beside them, list stays readable', async () => {
    ...
    // K-21: the row's verbs sit behind its one overflow, disabled the ARIA way.
    await userEvent.click(screen.getByRole('button', { name: de.docTemplate.rowActions.replace('{name}', 'Briefpapier Standard') }));
    expect(await screen.findByRole('menuitem', { name: de.docTemplate.archive })).toHaveAttribute('aria-disabled', 'true');
  });
  ...
  it('SET DEFAULT: a non-default row offers Als Standard festlegen and drives the verb', async () => {
    ...
    const user = userEvent.setup();
    render(tree(canned, asked));
    await user.click(await screen.findByRole('button', { name: de.docTemplate.rowActions.replace('{name}', 'Zweite') }));
    await user.click(await screen.findByRole('menuitem', { name: de.docTemplate.setDefault }));
    await waitFor(() => {
