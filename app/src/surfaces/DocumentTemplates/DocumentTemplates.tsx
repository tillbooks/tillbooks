...
export function DocumentTemplates() {
  ...
  const columns: DataTableColumn<Template>[] = [
    { key: 'name', header: t('docTemplate.name'), render: (template) => template.name },
    {
      ...
      render: (template) =>
        // K-22: the shared Status (glyph plus word), never a ★ dingbat or a bold-only word.
        template.isDefault ? (
          <Status kind="success" label={t('docTemplate.default')} />
        ) : template.archived ? (
          <Status kind="inactive" label={t('docTemplate.archived')} />
        ) : null,
    },
  ];

  // K-21: the row opens the editor; its other verbs sit behind ONE overflow, archive last.
  const rowActions = (template: Template): OverflowMenuItem[] => {
    if (template.archived) return [];
    const items: OverflowMenuItem[] = [];
    if (!template.isDefault) {
      items.push({
        key: 'default',
        label: t('docTemplate.setDefault'),
        disabled: !canManage || busy,
        onSelect: () => void setDefault(template.templateId),
      });
    }
    items.push({
      key: 'archive',
      label: t('docTemplate.archive'),
      danger: true,
      disabled: !canManage || busy,
      onSelect: () => void archive(template.templateId),
    });
    return items;
  };
  ...
  const list = (
    <DataTable
      ...
      rowActionsLabel={(template) => t('docTemplate.rowActions', { name: template.name })}
      emptyState={<></>}
    />
  );
  ...
  const editorPanel = editor !== null && (
    ...
    >
      <label className="doctpl-field">
        ...
        <input className="field" type="text" value={editor.name} onChange={(e) => setEditor({ ...editor, name: e.target.value })} />
      </label>
      ...
      <fieldset className="doctpl-field">
        ...
        {/* K-13: the one FileDrop (the real input stays the keyboard target), never the browser's
            own English "Choose File" control. */}
        <FileDrop
          variant="button"
          accept="image/*"
          label={t('docTemplate.uploadLogo')}
          onFiles={(files) => pickLogo(files[0])}
        />
      ...
      <fieldset className="doctpl-field">
        ...
        {/* K-11: which language's footer is being edited is one of four sibling values, so it is the
            shared Segmented control; K-38: the languages read as words, not locale codes. */}
        <Segmented
          label={t('docTemplate.footer')}
          options={LOCALES.map((locale) => ({ value: locale, label: t(`docTemplate.localeName.${locale}`) }))}
          value={footerLocale}
      ...
      <fieldset className="doctpl-field">
        ...
        <div className="doctpl-field">
          <span>{t('docTemplate.fixedLocale')}</span>
          <Select
            value={editor.fixedLocale}
            onChange={(val) => setEditor({ ...editor, fixedLocale: val })}
            options={LOCALES.map((locale) => ({ value: locale, label: t(`docTemplate.localeName.${locale}`) }))}
            ariaLabel={t('docTemplate.fixedLocale')}
          />
        </div>
      </fieldset>
      ...
      <div className="doctpl-actions">
        <button type="submit" className="btn btn--primary" disabled={!canManage || busy || editor.name.trim() === ''}>
          {t('docTemplate.save')}
        </button>
  ...
  // K-08: the header's one primary; while the editor is open its own save is, so "Neu" steps down.
  const newTemplate = (
    <button
      type="button"
      className={editor === null ? 'btn btn--primary' : 'btn btn--secondary'}
      disabled={!canManage || busy}
      onClick={openNew}
    >
      {t('docTemplate.new')}
  ...
  return (
    <section className="doctpl" aria-labelledby="doctpl-title">
      ...
      {/* K-11: the document kinds are views of this one surface, so they are the shared Tabs. */}
      <Tabs
        label={t('docTemplate.title')}
        tabs={KINDS.map((k) => ({ id: k, label: t(`docTemplate.kind.${KIND_KEY[k]}`) }))}
        activeId={kind}
        onChange={(id) => {
          setKind(id as (typeof KINDS)[number]);
          setEditor(null);
          setPreview(null);
        }}
      >
        ...
        {loading ? (
          <Skeleton rows={4} height={36} labelKey="docTemplate.loading" />
        ) : failed ? null : (
          <div className="doctpl-body">
            <div className="doctpl-main">
              {active.length === 0 && editor === null ? (
                <EmptyState
                  title={t('docTemplate.empty')}
                  hint={t('docTemplate.emptyHint')}
                  action={canManage ? { label: t('docTemplate.create'), onClick: openNew } : undefined}
                />
              ) : (
                list
              )}
