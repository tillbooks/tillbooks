...
describe('CreditNoteDialog (A13 §6)', () => {
  it('full arm sends mode full, and no lines or amount', async () => {
    ...
    // `create_credit_note` is always a draft, so the commit is the dialog's primary, never the money
    // tint (K-08, the C2 money critic, F5): the posting step is the later "Ausstellen" on the draft.
    const create = screen.getByRole('button', { name: 'Entwurf erstellen' });
    expect(create).toHaveClass('btn--primary');
    expect(create).not.toHaveClass('btn--accent');
