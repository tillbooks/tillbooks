...
/** The currency control is the shared Select (K-15): open it, then pick the option by its value. */
async function chooseCurrency(value: string) {
  await userEvent.click(await screen.findByRole('combobox', { name: 'Währung' }));
  const option = screen.getAllByRole('option').find((o) => o.getAttribute('data-value') === value);
  if (option === undefined) throw new Error(`the currency list offers no ${value}`);
  await userEvent.click(option);
}

/** The offered values, read from the open list and closed again so the test leaves no popup behind. */
async function currencyOptions(): Promise<{ values: string[]; labels: string[] }> {
  await userEvent.click(screen.getByRole('combobox', { name: 'Währung' }));
  const options = within(screen.getByRole('listbox')).getAllByRole('option');
  const read = {
    values: options.map((o) => o.getAttribute('data-value') ?? ''),
...
describe('CurrencyPicker, the five states', () => {
  ...
  it('EMPTY: with no recorded rates the picker still offers the base currency and EUR', async () => {
    ...
    await screen.findByRole('combobox', { name: 'Währung' });
    const { values, labels } = await currencyOptions();
    ...
    expect(labels.some((l) => l.includes('CHF (Buchwährung)'))).toBe(true);
  });
  ...
  it('LOADED: the recorded pairs become options, so a workspace bills what it has rates for', async () => {
    ...
    await screen.findByRole('combobox', { name: 'Währung' });
    await waitFor(async () => {
      const { values } = await currencyOptions();
      expect(values).toEqual(['CHF', 'EUR', 'USD', '__other__']);
    });
  });
...
describe('CurrencyPicker, the payment-part consequence (M13/ST3)', () => {
  ...
  it('a currency outside CHF/EUR warns at the control and does NOT block issuing', async () => {
    ...
    await userEvent.click(screen.getByRole('option', { name: 'Muster AG' }));
    await userEvent.type(screen.getByLabelText('Einzelpreis 1'), '150');
    await chooseCurrency('__other__');
...
describe('CurrencyPicker reaches the engine', () => {
  ...
  it('a saved EUR draft reopens in EUR, with the rate resolved for it', async () => {
    ...
    await waitFor(() => expect(screen.getByRole('combobox', { name: 'Währung' })).toHaveTextContent('EUR'));
