  ...
  return (
    <div className="documents-currency">
      <label className="documents-field">
        ...
        <select
          value={selectValue}
          aria-label={t('document.editor.currency')}
          onChange={(e) => onSelect(e.target.value)}
        >
          {options.map((code) => (
            <option key={code} value={code}>
              {code === baseCurrency ? t('invoice.currency.baseOption', { currency: code }) : code}
            </option>
          ))}
          <option value={OTHER}>{t('invoice.currency.other')}</option>
        </select>
      </label>
      ...
      {selectValue === OTHER && (
        <label className="documents-field">
          ...
          <input
            ...
            inputMode="text"
            maxLength={3}
            className="documents-currency-code"
...
function QrConsequenceLine({
  ...
  if (consequence.kind === 'no_iban') {
    return (
      <p className="documents-currency-note" role="note">
        {t('invoice.currency.qr.noIban')} <Link to="/setup">{t('invoice.qr.toSetup')}</Link>
      </p>
    );
  }
  ...
  if (fx.kind === 'needs_rate') {
    return (
      ...
      >
        ...
        <Link to="/setup">{t('invoice.fx.recordRate')}</Link>
      </div>
    );
  }
