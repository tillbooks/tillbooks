  ...
  return (
    <div className="documents-detail">
        ...
        >
          <span className="invoice-send-ways-out">
            ...
            {(sendNow.error.error === 'needs_email_config' || sendNow.error.error === 'needs_email_transport') && (
              <Link to="/setup">{t('invoice.send.toSetup')}</Link>
            )}
          </span>
        </ActionFeedback>
      )}
        ...
        actions={
          <>
            ...
            {actions.primary !== null ? (
              <button type="button" className="btn btn--accent" disabled={busy} onClick={() => onAction(actions.primary as DocumentAction)}>
      ...
      <p className="documents-detail-sub">
        ...
        <span className="t-num">{formatMoney(doc.totalMinor, doc.currency)}</span>
        <span aria-hidden="true"> · </span>
        ...
        <section className="documents-detail-fx panel" aria-label={t('invoice.fx.postedTitle')}>
          ...
          <dl className="documents-detail-fx-facts">
            <div>
              ...
              <dd className="t-num">{formatMoney(doc.totalMinor, doc.currency)}</dd>
            </div>
            <div>
              <dt>{t('invoice.fx.booked')}</dt>
              <dd className="t-num">{formatMoney(fx.totalBaseMinor, fx.baseCurrency)}</dd>
            </div>
            ...
            <div>
              ...
              <dd className="t-num">{formatMoney(doc.taxMinor, doc.currency)}</dd>
            </div>
            <div>
              <dt>{t('invoice.fx.vatBooked')}</dt>
              <dd className="t-num">{formatMoney(fx.baseTaxMinor, fx.baseCurrency)}</dd>
            </div>
            <div>
        ...
        (actionErr.error === 'period_locked' ? (
          <div className="error-banner panel" role="alert">
            ...
            <div>
              ...
              <Link to="/periods">{t('document.periodLockedLink')}</Link>
            </div>
          </div>
        ) : (
      ...
      <section className="documents-timeline panel" aria-label={t('document.detail.timeline')}>
        ...
        {doc.sourceDocumentId !== null && (
          <Link to={`/documents/${doc.sourceDocumentId}`} className="documents-timeline-link">
            {t('document.detail.source')}
          </Link>
        )}
        {/* A13: a Gutschrift links to the invoice it credits, one hop, no list scan. */}
        {doc.creditedDocumentId !== null && (
          <Link to={`/documents/${doc.creditedDocumentId}`} className="documents-timeline-link">
            {t('creditNote.reference')}
          </Link>
        )}
        {doc.postedEntryId !== null && (
          <Link to="/journal" className="documents-timeline-link">
          ...
          <tbody>
            {lines.map((l, i) => (
              <tr key={l.id ?? i}>
                ...
                <td className="documents-amount t-num">{formatMoney(l.unitPriceMinor, doc.currency)}</td>
                <td className="documents-amount t-num">{formatMoney(l.lineTotalMinor ?? 0, doc.currency)}</td>
              </tr>
            ))}
          </tbody>
        </table>
          ...
          <ul className="documents-timeline-list">
            {creditNotes.map((cn) => (
              <li key={cn.id} className="documents-timeline-item">
                <Link to={`/documents/${cn.id}`} className="documents-timeline-link">
                  {cn.number ?? t('document.draftNumber')}
                </Link>
                <span className="documents-timeline-meta">
                  {t(statusKey(cn.status))} · <span className="t-num">{formatMoney(cn.totalMinor, cn.currency)}</span>
                </span>
              </li>
            ))}
          </ul>
