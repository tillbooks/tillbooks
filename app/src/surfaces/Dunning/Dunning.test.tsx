...
describe('Dunning: the run history opens', () => {
  it('an earlier run can be opened from the history, and its letters come back with it', async () => {
    ...
    await userEvent.click(screen.getByRole('button', { name: '16.06.2026' }));
    await waitFor(() =>
    ...
    expect(screen.getByRole('button', { name: '16.06.2026' })).toHaveAttribute('aria-current', 'true');
  });
});
...
describe('Dunning: the policy editor', () => {
  ...
  it('a positive fee reveals the income-only account picker, and no tax-code picker exists (D69)', async () => {
    ...
    expect(
      within(accountSelect as HTMLElement).getByText('3600 Übrige betriebliche Erlöse'),
    ).toBeInTheDocument();
    expect(within(accountSelect as HTMLElement).queryByText(/Kassenbestand/)).not.toBeInTheDocument();
