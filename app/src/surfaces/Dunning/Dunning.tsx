 ...
 * open items.
 *
 * ONE PRIMARY ACTION, AND IT IS THE PROPOSE. "Mahnlauf vorschlagen" is the header's primary and the
 * whole entry to the flow; Ausstellen and Versenden are the commitments and live ON the run they
 * commit. Ausstellen books the Mahngebühr when the run carries a fee not yet on the ledger, and only
 * then is it the tinted `.btn--accent`, the one role that button keeps (K-08, D137: the commit of a
 * money write); its C8 twin, Gebühr nachbuchen, always books. Versenden mails letters and books
 * nothing, so it is secondary.
 *
 ...
 * from the run's frozen rows; nothing is computed here. Status is the shared `Status` word (K-22) and
 * the level a numbered chip, never colour alone. A24's courtesy gates pre-disable what the role cannot do
...
export function Dunning() {
  ...
  // The history columns for the shared DataTable. Every row OPENS its run into the review panel (K-21:
  // the row is the control, named "Mahnlauf vom … öffnen"). Without that the history was a list of
  // dates that did nothing, so an earlier Mahnlauf could not be reviewed and its letters could not be
  // reprinted: a reminder is the evidence of what was demanded. The open run wears the selection pill
  // and reports `aria-current` (`isRowCurrent`), so which run is open is never the tint alone. Debtor
  // and item counts are numeric, right-aligned tabular figures.
  const historyColumns: DataTableColumn<DunningRunSummary>[] = [
    {
      key: 'runDate',
      header: t('dunning.column.runDate'),
      render: (run) => formatDate(run.runDate),
    },
    {
      key: 'status',
  ...
  return (
    <section className="dunning" aria-labelledby="dunning-title">
      ...
      <p className="dunning-dim">
        {t('dunning.subtitle')} <Link className="link-inline" to="/open-items">{t('dunning.toOpenItems')}</Link>
      </p>
      ...
      {note !== null && (
        <div className="dunning-note" role="status">
          ...
          {note === t('dunning.needsCreditorAddress') && (
            <Link className="link-inline" to="/setup">{t('dunning.toSetup')}</Link>
          )}
        </div>
      )}
        ...
        <>
          ...
          <div className="dunning-history">
              ...
              {savedViews.length > 0 && (
                <div className="dunning-filter">
                  ...
                  <Select
                    ...
                    options={[
                      { value: '', label: t('dunning.allRuns') },
                      ...savedViews.map((view) => ({ value: view.id, label: view.name })),
                    ]}
                    ariaLabel={t('dunning.savedView')}
                  />
                </div>
              )}
            </div>
            <DataTable
              ...
              rowLabel={(run) => t('dunning.rowOpen', { date: formatDate(run.runDate) })}
              isRowCurrent={(run) => run.runId === current?.runId}
            />
          </div>
        </>
      )}
...
/** The Status kind per run status (K-22): proposed is a draft, issued under way, sent done. */
...
/** The run status as the shared Status word: a glyph from the set plus the word, never colour alone. */
function StatusChip({ status }: { status: string }) {
  const t = useT();
  return <Status kind={runStatusKind(status)} label={t(`dunning.status.${status}`)} />;
}
...
function ChangeBadge({ reason }: { reason: DunningItemChange }) {
  ...
  return (
    <span className="dunning-changed" role="note">
      <Status kind="warn" label={t('dunning.changedBadge', { reason: t(`dunning.changed.${changeKey(reason)}`) })} />
    </span>
  );
}
...
function CurrentRun({
  ...
  const reviewColumns: DataTableColumn<DunningItemView>[] = [
    // A debtor or an invoice without a name or number reads as such, never as its raw id (K-38).
    { key: 'debtor', header: t('dunning.column.debtor'), render: (item) => item.debtorName ?? t('dunning.unknownDebtor') },
    {
      key: 'invoice',
      ...
      render: (item) => (
        <>
          {item.number ?? t('dunning.unnumbered')}
    ...
    <div className="dunning-current panel" data-testid="dunning-current">
      <div className="dunning-current-head">
        ...
        <div className="dunning-current-actions">
          {/* The money tint only when this issue BOOKS (a fee not yet on the ledger, `willBook`, the
              same fact the engine's `post` assert keys on). A run with no fee to book issues letters
              and posts nothing, so it is secondary: the header's "Mahnlauf vorschlagen" is the one
              primary (K-08, the accent guard's per-button contract). */}
          {run.status === 'proposed' && (
            <button
              type="button"
              className={willBook ? 'btn btn--accent' : 'btn btn--secondary'}
              data-money-commit={willBook ? 'issue_dunning_run' : undefined}
          ...
          {issued && run.feeSkippedReason === 'period_locked' && (
            <button
              ...
              data-money-commit="issue_dunning_run"
      ...
      {issued && (
        <ul className="dunning-letters">
          {run.debtors.map((group) => (
            <li key={group.debtorId}>
              <button
                type="button"
                className="btn btn--ghost btn--sm"
                onClick={() => onDownload(group.debtorId, group.debtorName)}
              >
                {t('dunning.downloadLetter', { debtor: group.debtorName ?? t('dunning.unknownDebtor') })}
              </button>
              ...
              {group.changedSinceIssue && group.changeReason !== null && (
                <span className="dunning-changed" role="note">
                  <Status
                    kind="warn"
                    label={t('dunning.changedLetterHeld', { reason: t(`dunning.changed.${changeKey(group.changeReason)}`) })}
                  />
                </span>
              )}
            </li>
