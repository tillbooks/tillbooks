...
import { type EnvStatusOk, type EnvironmentRow, isProtected, isReadOnlyFace, formatSize } from './model';
  ...
  return (
    ...
    >
      ...
      {state.kind === 'ok' && env !== null && (
        <>
          {/* D135: only a served (hosted / remote) face is genuinely read-only; a local protected
              main is the real books and stays writable, so it shows no read-only note. */}
          {isReadOnlyFace(env) && <p className="env-readonly-note">{t('env.detail.readOnlyNote')}</p>}
