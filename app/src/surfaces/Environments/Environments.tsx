    ...
    () => [
      {
        ...
        render: (env) => (
          <span className="env-name">
            ...
            {env.name === activeName && <span className="env-chip env-chip--active">{t('env.badge.current')}</span>}
            {env.readOnly && env.name !== activeName && (
              <span className="env-chip env-chip--muted">{t('env.badge.readOnly')}</span>
            )}
            {!env.exists && <span className="env-chip env-chip--warn">{t('env.badge.missing')}</span>}
          </span>
        ),
      },
        ...
        <>
          <section className="env-section" aria-labelledby="env-tiers-title">
            ...
            <DataTable<EnvironmentRow>
              ...
              skeletonRows={3}
              rowClassName={(env) => (env.name === activeName ? 'env-row--active' : '')}
            />
          </section>
            ...
            <section className="env-section" aria-labelledby="env-named-title">
              ...
              {named.length > 0 ? (
                <DataTable<EnvironmentRow>
                  ...
                  rowClassName={(env) => (env.name === activeName ? 'env-row--active' : '')}
                  emptyState={<EmptyState title={t('env.filter.none')} hint="" />}
                />
              ) : (
