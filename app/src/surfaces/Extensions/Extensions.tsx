...
import { statusGlyph, type PluginDto, type PreviewDto, type RegistryEntryDto } from './model';
...
export function Extensions() {
  ...
  const installedPanel: ReactNode =
    installed.kind === 'loading' ? (
      ...
      <ErrorBanner error={{ ok: false, error: installed.code }} message={t(`plugin.error.${installed.code}`)} onRetry={() => void load()} />
          ...
          <li key={plugin.id} className="plugin-card">
            <div className="plugin-card__head">
              ...
              <span className={`plugin-card__status plugin-card__status--${plugin.status}`}>
                <span aria-hidden="true">{statusGlyph(plugin.status)}</span>{' '}
                <span className="plugin-card__status-text">{t(`plugin.status.${plugin.status}`)}</span>
              </span>
            </div>
            ...
            <div className="plugin-card__actions">
              ...
              <button type="button" className="btn btn--danger" disabled={!canManage || busy} onClick={() => onUninstall(plugin)}>
  ...
  const browsePanel: ReactNode = (
    <div className="extensions__browse">
      <div className="extensions__search">
        ...
        <input id={searchId} type="search" value={query} placeholder={t('plugin.registry.searchPlaceholder')} onChange={(e) => setQuery(e.target.value)} />
      ...
      {browse.kind === 'loading' ? (
        ...
        <EmptyState title={t('plugin.error.registry_unreachable')} />
      ) : (browse.kind === 'ok' && browse.entries.length === 0) ? (
        <EmptyState title={t('plugin.registry.no_results', { query: browse.query })} />
      ) : (
        <>
