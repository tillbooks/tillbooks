...
describe('Dateien, the five states', () => {
  it('renders the list with its folder tree, the size and the version', async () => {
    ...
    expect(await screen.findByRole('row', { name: 'Mietvertrag' })).toBeInTheDocument();
  ...
  it('distinguishes "no files" from "no matches" so a filter is never a mystery', async () => {
    ...
    // K-16: the search filters as you type (250ms after the last key), with no Suchen button.
...
describe('retention, as the engine judges it', () => {
  it('shows the lock and the statutory provenance, never colour alone', async () => {
    ...
    await user.click(await screen.findByRole('row', { name: 'Mietvertrag' }));
...
describe('the version history', () => {
  it('nests the chain under its head and never lists a superseded copy as a peer', async () => {
    ...
    expect(await screen.findAllByRole('row', { name: 'Mietvertrag' })).toHaveLength(1);

    await user.click(screen.getByRole('row', { name: 'Mietvertrag' }));
    const drawer = screen.getByRole('dialog');
...
describe('D34: the ceiling', () => {
  ...
  it('says nothing at all when the list is complete', async () => {
    renderFiles({ files_search: LIST, folders_list: TREE });
    await screen.findByRole('row', { name: 'Mietvertrag' });
...
describe('the folder tree', () => {
  it('disables delete with the reason, rather than letting the click be rejected', async () => {
    ...
    // K-21: the folder verbs sit in one overflow; a delete the engine would refuse stays there,
    // disabled, and its label carries the engine's own reason (never a hover-only title).
    const user = userEvent.setup();
    await user.click(await screen.findByRole('button', { name: 'Aktionen für Ordner Verträge' }));
    const item = screen.getByRole('menuitem', { name: 'Löschen geht erst, wenn der Ordner leer ist' });
  ...
  it('names the subfolder reason when that is what blocks the delete', async () => {
    ...
    const user = userEvent.setup();
    await user.click(await screen.findByRole('button', { name: 'Aktionen für Ordner Verträge' }));
    expect(screen.getByRole('menuitem', { name: 'Löschen geht erst ohne Unterordner' })).toHaveAttribute('aria-disabled', 'true');
  });
  ...
  it('renders the folder refusal where it was attempted', async () => {
    ...
    await user.click(await screen.findByRole('button', { name: 'Aktionen für Ordner Verträge' }));
    await user.click(screen.getByRole('menuitem', { name: 'Ordner löschen' }));
...
describe('the shared linked-files panel', () => {
  ...
  it('downloads a linked filing by asking the engine for its verified bytes', async () => {
    const user = userEvent.setup();
    const getContent = vi.fn<CannedHandler>(() =>
      ok({ fileId: 'file_1', filename: 'mietvertrag.pdf', mime: 'application/pdf', contentBase64: btoa('hello') }),
    );
    // jsdom has no real download, so the anchor click is what is observable.
    const clicks: string[] = [];
    const realClick = HTMLAnchorElement.prototype.click;
    ...
    try {
      renderLinked(
        { files_list_linked: ok({ files: [file()] }), files_get_content: getContent },
        'document',
        'doc_1',
      );
      await user.click(await screen.findByRole('button', { name: /Herunterladen: Mietvertrag/ }));
      await waitFor(() => expect(getContent).toHaveBeenCalledTimes(1));
      expect(getContent.mock.calls[0]?.[0]).toMatchObject({ workspaceId: 'ws_test', fileId: 'file_1' });
      // The ORIGINAL filename, so what lands on disk is what was filed.
      await waitFor(() => expect(clicks).toEqual(['mietvertrag.pdf']));
  ...
  // F7, one row over: a role holding `read_master_data` and NOT `read_file_content` may see the filing
  // listed and must not be offered its bytes. The padlock REPLACES the button; nothing is asked of the
  // engine, so there is no refusal to render.
  it('padlocks the per-row download for a role that may see the filing but not open it', async () => {
    renderLinked(
      { files_list_linked: ok({ files: [file()] }) },
      'document',
      'doc_1',
      holding([CAP.readMasterData, CAP.issue, CAP.manageFiles]),
    );
    expect(await screen.findByText('Mietvertrag')).toBeInTheDocument();
    expect(screen.queryByRole('button', { name: /Herunterladen: Mietvertrag/ })).toBeNull();
    // Glyph AND text (screen-reader label), never colour alone, and the reason is on the element.
    expect(screen.getByTitle(/nicht herunterladen/)).toBeInTheDocument();
    expect(screen.getByText('Download gesperrt')).toBeInTheDocument();
  });

  it('shows the integrity warning and saves NOTHING when a linked filing fails its checksum', async () => {
    const user = userEvent.setup();
    const clicks: string[] = [];
    const realClick = HTMLAnchorElement.prototype.click;
    HTMLAnchorElement.prototype.click = function patched(this: HTMLAnchorElement) {
    ...
    try {
      ...
      await user.click(await screen.findByRole('button', { name: /Herunterladen: Mietvertrag/ }));
      expect(await screen.findByText(/stimmt nicht mehr mit ihrer Prüfsumme überein/)).toBeInTheDocument();
      // Handing over bytes the checksum disowns is worse than refusing: nothing was written.
      expect(clicks).toEqual([]);
    } finally {
      HTMLAnchorElement.prototype.click = realClick;
