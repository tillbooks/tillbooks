      ...
      <ul className="files-tree-list">
        ...
        {folders.map((folder) =>
          editing === folder.id ? (
            ...
            <li key={folder.id} className="files-tree-row" style={{ paddingInlineStart: `${folder.depth * 14}px` }}>
              <button
                type="button"
              ...
              {canWrite && (
                <span className="files-tree-actions">
                  <button
                    type="button"
                    className="files-icon-btn"
                    aria-label={`${t('files.folder.action.rename')}: ${folder.name}`}
                    onClick={() => {
                      setDraft(folder.name);
                      setEditing(folder.id);
                    }}
                  >
                    {t('files.folder.action.renameShort')}
                  </button>
                  <button
                    type="button"
                    className="files-icon-btn files-icon-btn--danger"
                    aria-label={`${t('files.folder.action.delete')}: ${folder.name}`}
                    disabled={!folder.deletable}
                    // The engine's own two reasons, told apart, because "empty it first" is useless
                    // advice to somebody looking at a folder whose only content is a subfolder.
                    title={
                      folder.deletable
                        ? undefined
                        : folder.childCount > 0
                          ? t('files.folder.blocked.children')
                          : t('files.folder.blocked.files')
                    }
                    onClick={() => onDelete(folder.id)}
                  >
                    {t('files.folder.action.deleteShort')}
                  </button>
                </span>
              )}
            </li>
