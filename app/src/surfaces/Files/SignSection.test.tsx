...
describe('the Signatur section states', () => {
  ...
  it('renders every status as glyph plus text, never colour alone', () => {
    ...
    for (const label of ['Entwurf', 'Gesendet', 'Angesehen', 'Signiert', 'Abgelehnt', 'Abgelaufen']) {
      ...
      // The shared Status word (K-22): an icon-set glyph beside the label, so the TEXT carries the meaning.
      expect(chip.closest('.status-word')?.querySelector('svg')).not.toBeNull();
    }
...
describe('the request form and the statutory level hints (spec §3)', () => {
  it('shows the SES caveat by default and the OR Art. 14 note when QES is selected', async () => {
    ...
    await user.click(screen.getByLabelText('Signaturniveau'));
    await user.click(screen.getAllByRole('option').find((o) => o.getAttribute('data-value') === 'qes')!);
    expect(
  ...
  it('offers a contact without an email DISABLED with the reason, and submits the chosen signer', async () => {
    ...
    // The Select marks a non-selectable option with aria-disabled (the OS `disabled` attribute has
    // no equivalent on the portaled listbox rows).
    expect(screen.getByRole('option', { name: 'Ohne Mail AG (keine E-Mail)' })).toHaveAttribute('aria-disabled', 'true');
    await user.click(screen.getAllByRole('option').find((o) => o.getAttribute('data-value') === 'contact_1')!);
