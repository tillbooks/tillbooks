...
describe('AssetCategories, create', () => {
  it('opens the drawer, submits a create, and re-reads on success', async () => {
    ...
    await pickOption(user, de.assets.categories.field.assetAccount, ASSET_ACCOUNT_ID);
    await pickOption(user, de.assets.categories.field.accumAccount, ACCUM_ACCOUNT_ID);
    await pickOption(user, de.assets.categories.field.expenseAccount, EXPENSE_ACCOUNT_ID);
...
describe('AssetCategories, archive', () => {
  it('confirms then calls asset_category_archive', async () => {
    ...
    await user.click(screen.getByRole('button', { name: /^Aktionen für Kategorie/ })); // K-21: the row overflow
    await user.click(screen.getByRole('menuitem', { name: de.assets.categories.archive }));
