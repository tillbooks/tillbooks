...
export function AssetDepreciation() {
  ...
  const scheduleColumns: DataTableColumn<ScheduleLine>[] = [
    { key: 'period', header: t('assets.depreciation.schedule.period'), render: (l) => <span className="fa-code">{l.period}</span> },
        ...
        <>
          <div className="fa-filters">
            <label className="fa-field-inline">
              ...
              <select aria-label={t('assets.depreciation.assetLabel')} value={assetId} onChange={(e) => setAssetId(e.target.value)}>
                <option value="">{t('assets.depreciation.chooseAsset')}</option>
                {assets.map((a) => (
            ...
            {showUnits && (
              <label className="fa-field-inline">
                ...
                <input type="number" min={0} value={unitsThisPeriod} onChange={(e) => setUnitsThisPeriod(e.target.value)} />
              </label>
            )}
            {showUnits && (
              <label className="fa-field-inline">
            ...
            {showRate && (
              <label className="fa-field-inline">
                ...
                <input type="number" min={0} value={rateBp} onChange={(e) => setRateBp(e.target.value)} />
              </label>
            )}
            ...
            <section className="fa-summary" aria-label={t('assets.depreciation.nextAmount')}>
              <dl className="fa-summary-grid">
                ...
                <div>
                  ...
                  <dd className="fa-num">{money(selected.acquisitionCostRappen)}</dd>
                </div>
                <div>
                  <dt>{t('assets.depreciation.residual')}</dt>
                  <dd className="fa-num">{money(selected.residualValueRappen)}</dd>
                </div>
                <div>
                  <dt>{t('assets.depreciation.nbv')}</dt>
                  <dd className="fa-num">{money(selected.netBookValueRappen)}</dd>
                </div>
                <div>
                  <dt>{t('assets.depreciation.nextAmount')}</dt>
                  <dd className="fa-num fa-amount">{money(preview.amountRappen)}</dd>
                </div>
                <div>
