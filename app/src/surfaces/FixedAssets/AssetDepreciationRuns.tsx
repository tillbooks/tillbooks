...
export function AssetDepreciationRuns() {
  ...
  // K-22: a run's state is the shared Status word (glyph plus word), never a bespoke pill.
  const badge = (status: string) => (
    <Status
      kind={status === 'posted' ? 'success' : status === 'reversed' ? 'inactive' : 'neutral'}
      label={t(`assets.depreciationRun.status.${status}`)}
    />
  );
  ...
  const historyColumns: DataTableColumn<RunSummary>[] = [
    { key: 'period', header: t('assets.depreciationRun.col.period'), rowHeader: true, render: (r) => cal.month(r.period) },
  ...
  // K-21: the row opens the run's detail; posting and reversing sit behind the one quiet overflow,
  // the reversal last in the danger variant. Both still ask first (the confirm and the reason prompt).
  const runActions = (r: RunSummary): OverflowMenuItem[] => {
    const items: OverflowMenuItem[] = [];
    if (r.status === 'draft') {
      items.push({ key: 'post', label: t('assets.depreciationRun.post'), onSelect: () => setPosting(r), disabled: !canPost || busy });
    }
    if (r.status === 'posted') {
      items.push({
        key: 'reverse',
        label: t('assets.depreciationRun.reverse'),
        onSelect: () => {
          setActionError(null);
          setReason('');
          setReversing(r);
  ...
  return (
    <div className="fa">
      ...
      {draft !== null && (
        <section className="fa-summary" aria-label={t('assets.depreciationRun.review')}>
          <h2 className="fa-drawer-subtitle">
            {t('assets.depreciationRun.review')} {cal.month(draft.run?.period ?? period)} {draft.run !== null && badge(draft.run.status)}
          </h2>
      ...
      <DataTable
        ...
        rowLabel={(r) => t('assets.depreciationRun.rowOpen', { period: cal.month(r.period) })}
        rowActions={runActions}
        rowActionsLabel={(r) => t('assets.depreciationRun.rowActionsFor', { period: cal.month(r.period) })}
      ...
      {selected !== null && (
        <section className="fa-summary" aria-label={t('assets.depreciationRun.detail')}>
          <h2 className="fa-drawer-subtitle">
            {t('assets.depreciationRun.detail')} {cal.month(selected.run.period)} {badge(selected.run.status)}
          </h2>
      ...
      <Modal
        ...
        title={reversing === null ? '' : `${t('assets.depreciationRun.reverseTitle')} ${cal.month(reversing.period)}`}
        closeLabel={t('assets.common.close')}
        footer={
          <>
            <button
