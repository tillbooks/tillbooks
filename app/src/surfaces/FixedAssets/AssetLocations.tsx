...
export function AssetLocations() {
  ...
  const columns: DataTableColumn<Location>[] = [
    ...
    },
    {
      ...
      render: (l) => (
        <span className={`fa-badge ${l.active ? 'fa-badge-active' : 'fa-badge-archived'}`}>
          {l.active ? t('assets.locations.active') : t('assets.locations.archived')}
        </span>
      ),
    },
    {
      key: 'actions',
      header: t('assets.locations.col.actions'),
      headerHidden: true,
      align: 'end',
      render: (l) => (
        <div className="fa-row-actions">
          <button type="button" className="btn btn--ghost btn--sm" onClick={() => openEdit(l)} disabled={!canWrite}>
            {t('assets.locations.edit')}
          </button>
          {l.active && (
            <button
              type="button"
    ...
    <div className="fa">
      ...
      >
        ...
        <div className="fa-field">
          ...
          <input id="fa-loc-name" value={draft.name} onChange={(e) => setDraft({ ...draft, name: e.target.value })} />
        </div>
        <div className="fa-field">
          ...
          >
            <option value="">{t('assets.locations.field.noParent')}</option>
            {parentOptions.map((l) => (
