...
export function AssetMaintenance() {
  ...
  const logColumns: DataTableColumn<MaintenanceLog>[] = [
    { key: 'date', header: t('assets.maintenance.col.date'), render: (l) => cal.date(l.logDate) },
    // The maintenance TYPE is a word, not a state: plain text, no pill (K-22 keeps Status for states).
    { key: 'type', header: t('assets.maintenance.col.type'), render: (l) => t(`assets.maintenance.type.${l.maintenanceType}`) },
    ...
    { key: 'cost', header: t('assets.maintenance.col.cost'), numeric: true, render: (l) => rappenToChf(l.costRappen) },
    {
      ...
      render: (l) => (
        <Status
          kind={l.status === 'completed' ? 'success' : 'inactive'}
          label={
            l.status === 'completed'
  ...
  // K-21: a completed entry opens its edit drawer from the row; cancelling it sits behind the one
  // quiet overflow (danger, and it still asks for a reason). A cancelled entry is read-only.
  const logActions = (l: MaintenanceLog): OverflowMenuItem[] =>
    l.status === 'completed' && canWrite
      ? [
          {
            key: 'cancel',
            label: t('assets.maintenance.cancel'),
            onSelect: () => {
              setWriteError(null);
              setCancelReason('');
              setCancelling(l);
    ...
    <div className="fa">
      ...
      {assetsLoading ? (
        ...
        <>
          <div className="fa-filters">
            <div className="fa-field-inline">
              ...
              <Select
                ...
                options={assets.map((a) => ({ value: a.id, label: `${a.number} ${a.name}` }))}
                ariaLabel={t('assets.maintenance.asset')}
              />
            </div>
            <div className="fa-field-inline">
              ...
              <Select
                ...
                options={[
                  { value: '', label: t('assets.maintenance.filter.allTypes') },
                  ...MAINTENANCE_TYPES.map((ty) => ({ value: ty, label: t(`assets.maintenance.type.${ty}`) })),
                ]}
                ariaLabel={t('assets.maintenance.filter.type')}
              />
            </div>
          ...
          <DataTable
            ...
            {...(canWrite
              ? {
                  onRowClick: (l: MaintenanceLog) => {
                    if (l.status === 'completed') openEdit(l);
                  },
                  rowLabel: (l: MaintenanceLog) => t('assets.maintenance.rowOpen', { title: l.title, date: cal.date(l.logDate) }),
                }
              : {})}
            rowActions={logActions}
            rowActionsLabel={(l) => t('assets.maintenance.rowActionsFor', { title: l.title })}
            footer={
              logs.length > 0
                ? [
      ...
      >
        ...
        <div className="fa-field">
          ...
          <input className="field" id="fa-mnt-cancel-reason" value={cancelReason} onChange={(e) => setCancelReason(e.target.value)} />
        </div>
