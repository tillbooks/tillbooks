...
export function AssetReconciliation() {
  ...
  // asset_reconciliation_report. Drift is flagged by the shared Status word (glyph plus word, K-22) and
  // the delta figure in danger ink, never by a filled red row (DESIGN.md: only the figure is tinted).
  // Opening a row drills into its contributing assets in the shared DetailDrawer.
  const columns: DataTableColumn<ReconAccount>[] = [
    ...
    },
    {
      ...
      render: (a) => (
        <Status kind={a.status === 'drift' ? 'danger' : 'success'} label={t(`assets.reconciliation.status.${a.status}`)} />
      ),
    },
  ];
    ...
    <div className="fa">
      ...
      {check.kind === 'balanced' && (
        <div className="fa-recon-check fa-recon-ok" role="status">
          {t('assets.reconciliation.checkBalanced', { period: cal.month(check.period) })}
        </div>
      )}
      {check.kind === 'drift' && (
        <div className="fa-recon-check fa-recon-drift" role="alert">
          {t('assets.reconciliation.checkDrift', { period: cal.month(check.period), count: String(check.accounts) })}
        </div>
      )}
      ...
      {report !== null && (
        <div className="fa-recon-cards">
          ...
          <div className="fa-recon-card">
            <span className="fa-recon-card-cutoff">{cal.date(report.cutOff)}</span>
        ...
        {openAccount !== null && openAccount.assets.length === 0 ? (
          ...
          openAccount.assets.map((as) => (
            <div className="fa-recon-drill-row" key={as.assetId}>
              ...
              <span className="fa-num t-money">{money(as.amountRappen)}</span>
            </div>
          ))
        )}
      </DetailDrawer>
