...
describe('AssetRegister, the list', () => {
  it('renders an asset row with its number, name and status once loaded', async () => {
    ...
    // the status label appears both as the row badge and as a filter option; assert the badge one
    expect(document.querySelector('.fa-badge-draft')?.textContent).toBe(de.assets.status.draft);
...
describe('AssetRegister, create', () => {
  it('resolves category defaults on pick, submits a create with cost converted to Rappen', async () => {
    ...
    await user.selectOptions(screen.getByLabelText(de.assets.register.field.category), 'ac_1');
...
describe('AssetRegister, the financial-field lock', () => {
  it('disables the financial inputs and shows the lock note when editing an active asset', async () => {
    ...
    const user = userEvent.setup();
    await user.click(await screen.findByText(de.assets.register.edit));
...
describe('AssetRegister, archive', () => {
  it('confirms then calls asset_archive for a draft asset', async () => {
    ...
    await user.click(screen.getByText(de.assets.register.archive));
...
describe('AssetRegister, H02 acquisition', () => {
  it('posts a primary acquisition: cost converted to Rappen, credit account, idempotencyKey', async () => {
    ...
    renderSurface(canned, asked);
    const user = userEvent.setup();
    await user.click(await screen.findByText(de.assets.acquisition.record));
    ...
    await user.selectOptions(screen.getByLabelText(de.assets.acquisition.creditAccount), BANK_ID);
  ...
  it('an active asset offers Add capitalisation and posts asset_add_capitalisation', async () => {
    ...
    renderSurface(canned, asked);
    const user = userEvent.setup();
    await user.click(await screen.findByText(de.assets.acquisition.addCapitalisation));
    await user.type(screen.getByLabelText(de.assets.acquisition.amount), '1500');
    await user.selectOptions(screen.getByLabelText(de.assets.acquisition.creditAccount), CREDITOR_ID);
    await user.click(screen.getByText(de.assets.acquisition.post));
  ...
  it('the Transactions view renders the running-balance ledger from the recorded payload', async () => {
    ...
    renderSurface(canned);
    const user = userEvent.setup();
    await user.click(await screen.findByText(de.assets.acquisition.transactions));
  ...
  it('the credit-account picker offers only asset/liability/equity accounts, never income', async () => {
    ...
    const user = userEvent.setup();
    await user.click(await screen.findByText(de.assets.acquisition.record));
    const picker = screen.getByLabelText(de.assets.acquisition.creditAccount);
...
describe('AssetRegister, H05 transfer', () => {
  ...
  it('opens the transfer drawer and submits a non-posting transfer with a location and reason', async () => {
    ...
    const user = userEvent.setup();
    await user.click(await screen.findByText(de.assets.transfer.action));
    await user.selectOptions(screen.getByLabelText(de.assets.transfer.toLocation), LOC.id);
...
describe('AssetRegister, H06 disposal', () => {
  ...
  it('an active asset offers Dispose, previews the journal, and posts asset_dispose with proceeds converted', async () => {
    ...
    const user = userEvent.setup();
    await user.click(await screen.findByText(de.assets.disposal.action));
    ...
    await user.selectOptions(screen.getByLabelText(de.assets.disposal.proceedsAccount), BANK_ID);
    await user.selectOptions(screen.getByLabelText(de.assets.disposal.gainLossAccount), GL_INCOME_ID);
  ...
  it('the proceeds picker offers only money-side accounts and the gain/loss picker only income/expense', async () => {
    ...
    const user = userEvent.setup();
    await user.click(await screen.findByText(de.assets.disposal.action));
    // The gain/loss picker: income 3600 present, asset 1000 filtered out.
    const gl = screen.getByLabelText(de.assets.disposal.gainLossAccount);
    ...
    const proceeds = screen.getByLabelText(de.assets.disposal.proceedsAccount);
