...
describe('AssetReports', () => {
  ...
  it('K-16: the register search applies live after the pause, with no Anwenden button', async () => {
    const queries: unknown[] = [];
    renderSurface({
      asset_register_report: (input) => {
        queries.push(input.filter);
    ...
    const user = userEvent.setup();
    await user.type(screen.getByLabelText(de.assets.reports.register.search), 'CNC');
    // One settled read for the typed term, not one per keystroke.
    await waitFor(() => expect(queries.at(-1)).toEqual({ q: 'CNC' }));
    expect(queries.filter((q) => JSON.stringify(q) === JSON.stringify({ q: 'C' }))).toHaveLength(0);
  });
});
