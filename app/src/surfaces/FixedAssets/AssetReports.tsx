...
function RegisterReport() {
  ...
  const columns: DataTableColumn<Row>[] = [
    ...
    { key: 'category', header: t('assets.reports.register.col.category'), render: (r) => str(r.categoryCode) },
    {
      ...
      render: (r) => (
        <span className={`fa-badge fa-badge-${str(r.status) || 'draft'}`}>{t(`assets.reports.status.${str(r.status)}`)}</span>
      ),
    },
    ...
    <ReportBody
      filters={
        <div className="fa-filters">
          ...
          />
          <button type="button" className="btn btn--primary" onClick={() => setApplied(q)}>
            {t('assets.reports.apply')}
          </button>
        </div>
      }
      failed={failed}
...
function ForecastReport() {
  ...
  const [applied, setApplied] = useState({ from, to, groupBy });
  const { data, loading, failed, reload } = useReport(
  ...
  const columns: DataTableColumn<Row>[] = [
    { key: 'period', header: t('assets.reports.forecast.col.period'), render: (r) => <span className="fa-code">{str(r.period)}</span> },
  ...
  return (
    <>
      <div className="fa-filters">
        ...
        <input id="fa-fc-from" type="month" value={from} onChange={(e) => setFrom(e.target.value)} />
        <label htmlFor="fa-fc-to">{t('assets.reports.forecast.to')}</label>
        <input id="fa-fc-to" type="month" value={to} onChange={(e) => setTo(e.target.value)} />
        <label htmlFor="fa-fc-group">{t('assets.reports.forecast.groupBy')}</label>
        <select id="fa-fc-group" value={groupBy} onChange={(e) => setGroupBy(e.target.value)}>
          <option value="none">{t('assets.reports.forecast.group.none')}</option>
          <option value="category">{t('assets.reports.forecast.group.category')}</option>
          <option value="location">{t('assets.reports.forecast.group.location')}</option>
        </select>
        <button type="button" className="btn btn--primary" onClick={() => setApplied({ from, to, groupBy })}>
          {t('assets.reports.apply')}
        </button>
      </div>
...
function DisposalsReport() {
  ...
  const columns: DataTableColumn<Row>[] = [
    ...
    { key: 'date', header: t('assets.reports.disposals.col.date'), render: (r) => str(r.disposalDate) },
    ...
    <ReportBody
      filters={
        <div className="fa-filters">
          ...
          <input id="fa-dp-from" type="date" value={from} onChange={(e) => setFrom(e.target.value)} />
          <label htmlFor="fa-dp-to">{t('assets.reports.range.to')}</label>
          <input id="fa-dp-to" type="date" value={to} onChange={(e) => setTo(e.target.value)} />
          <button type="button" className="btn btn--primary" onClick={() => setApplied({ from, to })}>
            {t('assets.reports.apply')}
          </button>
        </div>
      }
...
function AcquisitionsReport() {
  ...
  const columns: DataTableColumn<Row>[] = [
    ...
    { key: 'date', header: t('assets.reports.acquisitions.col.date'), render: (r) => str(r.acquisitionDate) },
    ...
    <ReportBody
      filters={
        <div className="fa-filters">
          ...
          <input id="fa-ac-from" type="date" value={from} onChange={(e) => setFrom(e.target.value)} />
          <label htmlFor="fa-ac-to">{t('assets.reports.range.to')}</label>
          <input id="fa-ac-to" type="date" value={to} onChange={(e) => setTo(e.target.value)} />
          <button type="button" className="btn btn--primary" onClick={() => setApplied({ from, to })}>
            {t('assets.reports.apply')}
          </button>
        </div>
    ...
    <ReportBody
      filters={
        <div className="fa-filters">
          ...
          <select id="fa-nbv-group" value={groupBy} onChange={(e) => setGroupBy(e.target.value)}>
            <option value="category">{t('assets.reports.nbv.group.category')}</option>
            <option value="location">{t('assets.reports.nbv.group.location')}</option>
            <option value="status">{t('assets.reports.nbv.group.status')}</option>
            <option value="method">{t('assets.reports.nbv.group.method')}</option>
          </select>
          <button type="button" className="btn btn--primary" onClick={() => setApplied(groupBy)}>
            {t('assets.reports.apply')}
          </button>
        </div>
      }
...
function EndOfLifeReport() {
  ...
  const [applied, setApplied] = useState({ months: '12', status: 'approaching' });
  const { data, loading, failed, reload } = useReport(
    ...
    <ReportBody
      filters={
        <div className="fa-filters">
          ...
          <select id="fa-eol-status" value={status} onChange={(e) => setStatus(e.target.value)}>
            <option value="approaching">{t('assets.reports.endOfLife.approaching')}</option>
            <option value="fully_depreciated">{t('assets.reports.endOfLife.fullyDepreciated')}</option>
          </select>
          {status === 'approaching' && (
            <>
              <label htmlFor="fa-eol-months">{t('assets.reports.endOfLife.withinMonths')}</label>
              <input id="fa-eol-months" type="number" min={1} max={60} value={months} onChange={(e) => setMonths(e.target.value)} />
            </>
          )}
          <button type="button" className="btn btn--primary" onClick={() => setApplied({ months, status })}>
            {t('assets.reports.apply')}
          </button>
        </div>
      }
