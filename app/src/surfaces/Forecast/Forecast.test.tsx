...
describe('Forecast (Prognose)', () => {
  ...
  it('SUCCESS: renders the weighted table, the KPI strip, the three components, and the ▼ glyph WITH label', async () => {
    ...
    // The vs-actual delta is the signed figure AND a direction word, never colour alone.
    expect(screen.getByText(de.forecast.vsActual.under)).toBeInTheDocument();
    // The FX-excluded quote renders as a warn Status word, a named exclusion (P9 honesty).
    ...
    expect(screen.getByText(de.forecast.disclaimer)).toBeInTheDocument();
    // The custom select field extends the Gruppierung switch; the text field does not. Options live
    // in the portaled listbox, so open the group-by picker before reading them.
  ...
  it('CONTROLS: group-by, horizon and period use the shared controls (D116 target band)', async () => {
    render(tree(POPULATED()));
    await screen.findByText('Qualifiziert');
    // Gruppierung is the shared <Select> (it grows with the custom fields), whose trigger reads and
    // MEASURES like every other Studio filter control (36px), not the ~21px browser default. The
    // four-step Horizont is the shared Segmented (K-11, round 2).
    const combos = screen.getAllByRole('combobox');
    expect(combos.length).toBeGreaterThanOrEqual(1);
    for (const combo of combos) expect(combo).toHaveClass('select-trigger');
    const horizon = screen.getByRole('radiogroup', { name: de.forecast.horizon.label });
    expect(within(horizon).getByRole('radio', { name: '6 Monate' })).toHaveAttribute('aria-checked', 'true');
    // The period picker is a month input; it shares the shared field control class.
