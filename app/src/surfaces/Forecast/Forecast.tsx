 ...
 * and the vs-actual reconciliation (`forecast_vs_actual`, the signed figure AND a direction word,
 * never colour alone). It calls not one write verb: C03 is a pure read model (P5) and this surface honours it.
 *
 ...
 * FX-excluded quotes render as a warn-Status list, so a degraded total says what it excludes.
 *
 * Round 2 (D137): the four-step horizon is a Segmented control (K-11; Gruppierung stays a Select,
 * it grows with the custom fields); the exclusions are Status words and the delta loses its
 * triangle dingbat, its word carries the direction (K-22); a failed read says so (K-35); months
 * read as words (K-38); every figure outside a DataTable is `.t-money` (tabular, never wrapped).
 *
...
export function Forecast() {
  ...
  const revenueColumns: DataTableColumn<RevenueRow>[] = [
    { key: 'period', header: t('forecast.revenue.col.period'), render: (row) => calendar.month(row.periodKey) },
  ...
  return (
    <section className="forecast" aria-labelledby="forecast-title">
      ...
      <div className="forecast-controls">
        <div className="forecast-field">
          ...
          <Select
            ...
            options={[
              { value: 'stage', label: t('forecast.group.stage') },
              { value: 'month', label: t('forecast.group.month') },
              { value: 'quarter', label: t('forecast.group.quarter') },
            ]}
            groups={
              groupFields.length > 0
                ? [
                    {
                      label: t('forecast.group.custom'),
                      options: groupFields.map((f) => ({ value: f.key, label: f.label })),
                    },
                  ]
                : undefined
            }
            ariaLabel={t('forecast.group.label')}
          />
        </div>
        <div className="forecast-field">
          <span id="forecast-horizon-label">{t('forecast.horizon.label')}</span>
          <Segmented
            value={String(horizon)}
            onChange={(val) => setHorizon(Number.parseInt(val, 10))}
            options={HORIZONS.map((months) => ({
              value: String(months),
              label: t('forecast.horizon.months', { count: months }),
            }))}
            labelledBy="forecast-horizon-label"
          />
        </div>
      ...
      {!loading && failed && <ErrorBanner onRetry={() => void load()} context="read" />}
        ...
        <>
          ...
          <section className="forecast-kpis panel" aria-label={t('forecast.kpi.title')}>
            <div className="forecast-kpi">
              ...
              <span className="forecast-kpi-value t-num">
            ...
            <div className="forecast-kpi">
              ...
              <span className="forecast-kpi-value t-money">
            ...
            <div className="forecast-kpi">
              ...
              <span className="forecast-kpi-value t-num">{kpis?.avgCycleDays == null ? '–' : kpis.avgCycleDays}</span>
                ...
                <ul className="forecast-excluded" aria-label={t('forecast.excluded.fx')}>
                  {revenue.excluded.map((entry) => (
                    <li key={entry.quoteId}>
                      <Status kind="warn" label={t('forecast.excluded.fx')} /> <span>({entry.quoteId})</span>
                    </li>
                  ))}
                </ul>
              )}
          ...
          {/* (4) the vs-actual reconciliation: the signed figure AND its direction word, never colour
              alone (US-C03.4). */}
          {vsActual !== null && (
            <section className="panel forecast-panel" aria-label={t('forecast.vsActual.title')}>
              ...
              <dl className="forecast-vs">
                <div>
                  ...
                  <dd className="t-money">{formatMoney(vsActual.actualRevenueMinor, vsActual.baseCurrency)}</dd>
                </div>
                <div>
                  <dt>{t('forecast.vsActual.pipeline')}</dt>
                  <dd className="t-money">{formatMoney(vsActual.wonInPeriodMinor, vsActual.baseCurrency)}</dd>
                </div>
                <div>
                  <dt>{t('forecast.vsActual.delta')}</dt>
                  <dd>
                    <span className="t-money">{formatMoney(vsActual.deltaMinor, vsActual.baseCurrency)}</span>{' '}
                    <span className="forecast-delta-label">{deltaLabel}</span>
                  </dd>
                </div>
                <div>
                  <dt>{t('forecast.vsActual.wonNotInvoiced')}</dt>
                  <dd className="t-money">{formatMoney(vsActual.wonNotInvoicedMinor, vsActual.baseCurrency)}</dd>
                </div>
                <div>
                  <dt>{t('forecast.vsActual.invoicedWithoutDeal')}</dt>
                  <dd className="t-money">{formatMoney(vsActual.invoicedWithoutDealMinor, vsActual.baseCurrency)}</dd>
                </div>
              </dl>
            </section>
          )}
