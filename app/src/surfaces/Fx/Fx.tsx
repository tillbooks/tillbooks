        ...
        {canPost ? (
          ...
          >
            <div className="fx-addrate-fields">
              ...
              <button type="submit" className="btn btn--secondary btn--sm fx-addrate-submit" disabled={!addReady || addPending}>
                ...
                <div className="fx-reval-actions">
                  {postResult === null && (
                    <button
                      ...
                      data-money-commit="post_fx_revaluation"
                      onClick={() => void post()}
...
function DiffCell({ minor, currency, t }: { minor: number; currency: string; t: (k: string, p?: Record<string, string | number>) => string }) {
  if (minor === 0) {
    return <span className="fx-diff fx-diff--flat t-money">{formatMoney(0, currency)}</span>;
  }
  const loss = minor < 0;
  return (
    <span className={loss ? 'fx-diff fx-diff--loss' : 'fx-diff fx-diff--gain'}>
      ...
      <span className="t-money">{formatMoney(minor, currency)}</span>
    </span>
  );
}
