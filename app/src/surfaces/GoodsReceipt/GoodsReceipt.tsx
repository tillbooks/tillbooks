...
import { useSeatLabel } from '../Time/seatLabels';
import './GoodsReceipt.css';

/**
 * A receipt's and a line's state as the one `Status` word (K-22): an icon-set glyph beside the word,
 * never a text dingbat and never colour alone (design-canon, WCAG 2.2).
 */
...
const INSPECTION_KIND: Record<string, StatusKind> = {
  none: 'neutral',
  pending: 'pending',
  accepted: 'success',
  rejected: 'danger',
...
export function GoodsReceipt() {
  ...
  // K-38: an actor is named, never printed as its raw id.
      ...
      render: (r) => (
        <span className="gr-list-cell">
          ...
          <span className="gr-muted">
            {r.poNumber} · {r.supplierName} · {formatCalendar(r.receivedAt)}
          </span>
        </span>
      ),
    },
    {
      ...
      render: (r) => (
        <span className="gr-list-badges">
          <Status kind={STATUS_KIND[r.status] ?? 'neutral'} label={t(`goodsReceipt.status.${r.status}`)} />
          ...
          {r.overReceiptQty > 0 && (
            <Status kind="warn" label={`${t('goodsReceipt.overReceipt.badge')} +${r.overReceiptQty}`} />
          )}
        </span>
      ),
    },
  ...
  const lineColumns: DataTableColumn<ReceiptLine>[] = [
    ...
    { key: 'unitCost', header: t('goodsReceipt.col.unitCost'), numeric: true, render: (l) => formatMoney(l.unitCostRappen, 'CHF') },
    {
      ...
      render: (l) => (
        <Status
          kind={INSPECTION_KIND[l.inspectionStatus] ?? 'neutral'}
          label={t(`goodsReceipt.inspection.${l.inspectionStatus}`)}
        />
      ),
    },
    {
        ...
        <>
          {l.overReceiptQty > 0 && (
            <Status
              className="gr-issue"
              kind="warn"
              label={t('goodsReceipt.overReceipt.line', { qty: String(l.overReceiptQty) })}
            />
          )}
  ...
  return (
    <div className="gr">
      ...
      {failed && (
        <ErrorBanner context="read" message={t('goodsReceipt.error.transport')} onRetry={() => void loadList()} />
      )}

      {showSkeleton ? (
        <DataTable columns={receiptColumns} rows={[]} rowKey={(r) => r.id} loading skeletonRows={4} />
      ) : receipts.length === 0 ? (
        ...
        <div className="gr-split">
          <section className="gr-pane" aria-label={t('goodsReceipt.listLabel')}>
            <DataTable
              ...
              // K-24: the open receipt wears the selection pill on every cell, no side bar. An
              // over-delivery says so as a Status word in its row, never as an edge.
              isRowCurrent={(r) => r.id === selectedId}
            />
          </section>
              ...
              <>
                <div className="gr-detail-head">
                  <div>
                    ...
                    <p className="gr-muted">
                      {t('goodsReceipt.field.receivedAt')}: <strong>{formatCalendar(detail.receivedAt)}</strong>
                      {' · '}
                      <Status kind={STATUS_KIND[detail.status] ?? 'neutral'} label={t(`goodsReceipt.status.${detail.status}`)} />
                    </p>
                    ...
                    {detail.events.length > 0 && (
                      <Provenance
                        ...
                        actor={detail.events[0].actor === 'studio' ? 'studio' : seatLabel(detail.events[0].actor).label || null}
                ...
                {preview !== null && (
                  <p className="gr-total">
                    {t('goodsReceipt.value')}: <strong className="t-money">{formatMoney(preview.valueRappen, 'CHF')}</strong>
                  </p>
                )}
                ...
                <ul className="gr-trail">
                  {detail.events.map((e) => (
                    <li key={e.id}>
                      <span className="t-num">{formatCalendar(e.createdAt)}</span> {t(`goodsReceipt.event.${e.eventType}`)}
                      {e.actor !== null ? ` · ${seatLabel(e.actor).label}` : ''}
        ...
        >
          ...
          <div className="gr-field">
            ...
            <Select
              ...
              value={newPoId}
              onChange={setNewPoId}
              options={pos.map((p) => ({ value: p.id, label: p.number }))}
              ariaLabel={t('goodsReceipt.form.po')}
            />
          </div>
          <div className="gr-field">
          ...
          <div className="gr-field">
            ...
            <Select
              ...
              options={openLines.map((l) => ({
                value: l.id,
                label: `${l.description ?? l.id} (${t('goodsReceipt.form.open')}: ${l.openQty})`,
              }))}
              ariaLabel={t('goodsReceipt.form.poLine')}
            />
          </div>
          <div className="gr-field">
              ...
              {lots.length > 0 && (
                <div className="gr-field">
                  ...
                  <Select
                    id="gr-lot"
                    value={lineLotId}
                    onChange={setLineLotId}
                    options={[
                      { value: '', label: t('goodsReceipt.form.noneSelected') },
                      ...lots.map((l) => ({ value: l.id, label: l.number })),
                    ]}
                    ariaLabel={t('goodsReceipt.form.lot')}
                  />
                </div>
              )}
              <div className="gr-field">
              ...
              {serials.length > 0 && (
                <div className="gr-field">
                  ...
                  <Select
                    id="gr-serial"
                    value={lineSerialId}
                    onChange={setLineSerialId}
                    options={[
                      { value: '', label: t('goodsReceipt.form.noneSelected') },
                      ...serials.map((o) => ({ value: o.id, label: o.number })),
                    ]}
                    ariaLabel={t('goodsReceipt.form.serial')}
                  />
