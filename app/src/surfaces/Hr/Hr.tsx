...
/** ○ Entwurf · ↑ Eingereicht · ✓ Genehmigt · ✓✓ Erstattet · ✕ Abgelehnt · ⊘ Storniert. */
const CLAIM_GLYPH: Record<string, string> = { draft: '○', submitted: '↑', approved: '✓', reimbursed: '✓✓', rejected: '✕', cancelled: '⊘' };
/** ◐ Ferien · ◑ Krankheit · ◌ Andere. */
const ABSENCE_GLYPH: Record<string, string> = { vacation: '◐', sick: '◑', other: '◌' };
...
export function Hr() {
  ...
  const employeeColumns: DataTableColumn<Employee>[] = [
    ...
    { key: 'pct', header: t('hr.employee.pct'), numeric: true, render: (e) => `${e.employmentPct} %` },
    {
      ...
      render: (e) =>
        e.ahvRestricted
          ? <span className="hr-restricted">⊘ {t('hr.employee.ahv_restricted')}</span>
          : (e.ahvNr ?? ''),
    },
  ...
  const absenceColumns: DataTableColumn<Absence>[] = [
    {
      ...
      render: (a) => (
        <>
          <span className="hr-glyph" aria-hidden="true">{ABSENCE_GLYPH[a.kind] ?? '◌'}</span>{' '}
          {t(`hr.absence.kind.${a.kind}`)}
        </>
      ),
    },
    { key: 'range', header: t('hr.absence.col.period'), render: (a) => `${formatDate(a.fromDate)} – ${formatDate(a.toDate)}` },
    {
      ...
      render: (a) => (a.status === 'cancelled' ? <span className="hr-tag">{t('hr.absence.cancelled')}</span> : ''),
    },
  ];

  const claimColumns: DataTableColumn<Claim>[] = [
    { key: 'title', header: t('hr.claim.title'), render: (c) => c.title },
    {
      key: 'status',
      header: t('hr.claim.status'),
      render: (c) => (
        <>
          <span className="hr-glyph" aria-hidden="true">{CLAIM_GLYPH[c.status] ?? '○'}</span>{' '}
          {t(`spesen.status.${c.status}`)}
        </>
      ),
    },
      ...
      render: (c) => (
        <>
          {canApprove && c.status === 'submitted' && (
            <>
              <button className="hr-cta" onClick={() => void write('expense_claim_approve', { claimId: c.id, confirm: true, idempotencyKey: newKey() })}>{t('spesen.action.approve')}</button>
              <button onClick={() => void write('expense_claim_reject', { claimId: c.id, reason: t('spesen.reject.default'), idempotencyKey: newKey() })}>{t('spesen.action.reject')}</button>
            </>
          )}
          {canApprove && c.status === 'approved' && (
            <button className="hr-cta" onClick={() => void write('expense_claim_reimburse', { claimId: c.id, confirm: true, idempotencyKey: newKey() })}>{t('spesen.action.reimburse')}</button>
          )}
        </>
      ),
    },
  ...
  function renderPanel(id: Tab): ReactNode {
    ...
    if (failed) return <ErrorBanner message={t('hr.error.load')} onRetry={() => void load()} />;
    if (id === 'employees') {
      return (
          ...
          {canManage && (
            creatingEmp ? (
              <form className="hr-form" onSubmit={(e) => { e.preventDefault(); void createEmployee(); }}>
                <input aria-label={t('hr.employee.firstName')} placeholder={t('hr.employee.firstName')} value={addEmp.firstName} onChange={(e) => setAddEmp({ ...addEmp, firstName: e.target.value })} />
                <input aria-label={t('hr.employee.lastName')} placeholder={t('hr.employee.lastName')} value={addEmp.lastName} onChange={(e) => setAddEmp({ ...addEmp, lastName: e.target.value })} />
                <input aria-label={t('hr.employee.pct')} type="number" min={1} max={100} value={addEmp.employmentPct} onChange={(e) => setAddEmp({ ...addEmp, employmentPct: e.target.value })} />
                <input aria-label={t('hr.employee.startsOn')} type="date" value={addEmp.startsOn} onChange={(e) => setAddEmp({ ...addEmp, startsOn: e.target.value })} />
                <button type="submit" className="hr-cta">{t('hr.action.save')}</button>
                <button type="button" onClick={() => setCreatingEmp(false)}>{t('hr.action.cancel')}</button>
              </form>
            ) : (
              <button className="hr-cta" onClick={() => setCreatingEmp(true)}>{t('hr.action.add')}</button>
            )
          )}
          <DataTable
            ...
            emptyState={<EmptyState title={t('hr.empty.employees')} />}
          />
        </div>
      );
    }
  ...
  return (
    <section className="hr" aria-labelledby="hr-title">
      <SurfaceHeader title={t('hr.route.title')} titleId="hr-title" help={<SurfaceHelp surface="Hr" />} />
