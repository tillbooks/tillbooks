...
describe('Inventory (Lager), the five states and the UX-pass remediations', () => {
  ...
  it('READY: the grid renders on-hand and the low-stock badge is a glyph AND a label', async () => {
    ...
    // The shared Status word (K-22): the attention glyph from the icon set AND the "Bestand niedrig"
    // text, never colour alone and never a text dingbat.
    const badge = container.querySelector('.status-word[data-kind="warn"]');
    expect(badge).not.toBeNull();
    expect(badge?.querySelector('svg')).not.toBeNull();
    expect(badge?.textContent).toBe(de.stock.badge.low);
  });
  ...
  it('"Bewertung ausführen" is never the money tint: stock_run_valuation posts no journal entry (K-08, C2 F5)', async () => {
    renderSurface(fakeTransport(baseCanned()));
    await screen.findByText('Schraube M4');
    const run = screen.getByRole('button', { name: de.stock.action.run_valuation });
    // The header's "Bewegung erfassen" is the page's one primary, so the report-only run is secondary.
    expect(run).toHaveClass('btn--secondary');
    expect(run).not.toHaveClass('btn--accent');
    expect(document.querySelectorAll('.btn--accent')).toHaveLength(0);
  });
  ...
  it('the valuation method select is labelled "Methode", not "Grund" (the mislabelled-select fix)', async () => {
    ...
    expect(method).toHaveAttribute('role', 'combobox');
  ...
  it('the movement dialog is a focus-trapping role=dialog with a correctly labelled reason select', async () => {
    ...
    expect(within(dialog).getByLabelText(de.stock.movement.reason_label)).toHaveAttribute('role', 'combobox');
  ...
  it('the Inventur commit waits for a synchronous confirm before stock_stocktake_commit fires (C4)', async () => {
    ...
    // The stocktake commit writes quantities and has no ledger reach, so it is never the money tint
    // (K-08, the C2 money critic, F5): secondary beside the header's primary, primary in its confirm.
    const opener = screen.getByRole('button', { name: de.stock.stocktake.commit });
    expect(opener).toHaveClass('btn--secondary');
    expect(opener).not.toHaveClass('btn--accent');
    ...
    const confirmCommit = within(dialog).getByRole('button', { name: de.stock.stocktake.commit });
    expect(confirmCommit).toHaveClass('btn--primary');
    expect(confirmCommit).not.toHaveClass('btn--accent');
    await userEvent.click(confirmCommit);
