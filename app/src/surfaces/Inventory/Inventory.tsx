...
export function Inventory() {
  ...
  // K-34: the table skeleton, never before 200ms and never for less than 300ms once shown.
  ...
  const gridColumns: DataTableColumn<OnHandRow>[] = [
    ...
    { key: 'qty', header: t('stock.movement.qty'), numeric: true, render: (r) => r.onHand },
    {
      ...
      headerHidden: true,
      render: (r) =>
        // The one Status word (K-22): the attention glyph from the icon set beside the label.
        lowIds.has(r.itemId) ? <Status kind="warn" label={t('stock.badge.low')} /> : null,
    },
  ];
  ...
  // The header stands in every state (K-34): the page keeps its title while the table draws its own
  // skeleton, and a failed read names itself as a read (K-35).
  const header = (
    <SurfaceHeader
      title={t('stock.route.title')}
      help={<SurfaceHelp surface="Inventory" />}
      actions={
        canMove && !showSkeleton && !error ? (
  ...
  if (showSkeleton) {
    return (
      <div className="inventory">
        {header}
        <DataTable columns={gridColumns} rows={[]} rowKey={(r) => `${r.itemId}:${r.locationId}`} loading skeletonRows={6} />
      </div>
    );
  }
  if (error) {
      ...
      <section className="inventory__panel" aria-label={t('stock.action.run_valuation')}>
        ...
        <div className="inventory__row">
          <div className="inventory__field">
            ...
            <Select
              ...
              options={[
                { value: 'fifo', label: t('stock.method.fifo') },
                { value: 'weighted_avg', label: t('stock.method.weighted_avg') },
              ]}
              ariaLabel={t('stock.method.label')}
            />
          </div>
          <label>
          ...
          {canPost && (
            // `stock_run_valuation` is report-only (K68): it posts no journal entry, so it is never the
            // money tint (the C2 money critic, F5), and the page header already holds the one primary.
            <button type="button" className="btn btn--secondary" onClick={() => void runValuation()} disabled={asOf === ''}>
        ...
        {valuation !== null && (
          <div className="inventory__valuation">
            ...
            <p>
              {t('stock.valuation.total')}: <strong className="t-money">{formatMoney(valuation.totalValueMinor, baseCurrency)}</strong>
              {' · '}
              {t('stock.valuation.unposted_delta')}:{' '}
              <span className="t-money">{formatMoney(valuation.unpostedDeltaMinor, baseCurrency)}</span>
            </p>
            <DataTable
      ...
      <div className="inventory__form">
        <div className="inventory__field">
          ...
          <Select
            ...
            ariaLabel={t('stock.movement.item')}
          />
        </div>
        <div className="inventory__field">
          ...
          <Select
            ...
            ariaLabel={t('stock.movement.location')}
          />
        </div>
        <div className="inventory__field">
          ...
          <Select
            ...
            options={REASONS.map((r) => ({ value: r, label: t(`stock.reason.${r}`) }))}
            ariaLabel={t('stock.movement.reason_label')}
          />
        </div>
        <label>
          {t('stock.movement.qty')}
          <input className="field" inputMode="numeric" value={qty} onChange={(e) => setQty(e.target.value)} />
        </label>
        <label>
          {t('stock.movement.unit_cost')}
          <input className="field" inputMode="decimal" value={unitCost} onChange={(e) => setUnitCost(e.target.value)} />
        </label>
      </div>
    </Modal>
  );
  ...
  const stocktakeColumns: DataTableColumn<StocktakeLine>[] = [
    ...
    { key: 'bookQty', header: t('stock.stocktake.book_qty'), numeric: true, render: (l) => l.bookQty },
    {
      ...
      render: (l) => (
        <input
          inputMode="numeric"
          className="field inventory__count"
    ...
    <section className="inventory__panel" aria-label={t('stock.stocktake.title')}>
      ...
      {sessionId === null ? (
        <div className="inventory__row">
          <label>
            {t('stock.valuation.as_of')}
            <input className="field" type="date" value={frozenAt} onChange={(e) => setFrozenAt(e.target.value)} />
          </label>
        ...
        <>
          ...
          {canCommit && lines.length > 0 && (
            // The stocktake commit writes quantity movements and has no ledger reach (the value reaches
            // the books only through the valuation post), so it is not the money tint (C2 F5). The
            // page header holds the one primary, so this is secondary; its confirm is the primary.
            <button type="button" className="btn btn--secondary" onClick={() => setConfirmCommit(true)}>
