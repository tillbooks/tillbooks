...
describe('InventoryMovements, the append-only ledger surface', () => {
  ...
  // R4-D1. The `Von` cell resolves the stored actor id (`studio`) to the member's name through the
  // shared roster read, keeping the raw id as the cell's tooltip so it stays discoverable.
  it('resolves the actor id to the member name, with the raw id kept as a tooltip (R4-D1)', async () => {
    const canned = baseCanned();
    canned.list_members = ok({
      members: [{ memberId: 'm1', userId: 'u1', actorId: 'studio', displayName: 'Anna Muster', email: null }],
    });
    renderSurface(canned);
    await screen.findByRole('button', { name: 'Widget' });
    const name = await screen.findByText('Anna Muster');
    expect(name).toHaveAttribute('title', de.invMovements.actorTooltip.replace('{id}', 'studio'));
    // The raw id is not the visible label any more.
    expect(screen.queryByText('studio')).not.toBeInTheDocument();
  });

  // K-38 (D137) supersedes R4-D1's raw-id fallback: with no roster read available (here the
  // transport refuses it), the cell still never blanks, and it names the seat by the shared rule
  // (the Studio seat is "Studio auf diesem Gerät") instead of printing the raw id.
  it('names the seat, never the raw id, when the roster read is refused (K-38)', async () => {
    const canned = baseCanned();
    canned.list_members = reject('permission_denied');
    renderSurface(canned);
    await screen.findByRole('button', { name: 'Widget' });
    await waitFor(() => expect(screen.getByText('Studio auf diesem Gerät')).toBeInTheDocument());
  ...
  it('surfaces insufficient_stock with the surface-scoped message', async () => {
    ...
    await userEvent.click(screen.getByRole('combobox', { name: de.invMovements.form.type }));
    await userEvent.click(screen.getByRole('option', { name: 'Abgang' }));
