...
export function InventoryMovements() {
  ...
  // K-38: an actor is named through the shared rule, never printed as its raw id.
  ...
  const columns: DataTableColumn<Movement>[] = [
    { key: 'date', header: t('invMovements.col.date'), render: (m) => formatCalendar(m.effectiveDate) },
    // The kind of movement is a category, not a state: the word alone, no bordered badge (K-22).
    { key: 'type', header: t('invMovements.col.type'), render: (m) => t(`invMovements.type.${m.movementType}`) },
    {
      key: 'qty',
    ...
    {
      key: 'actor',
      header: t('invMovements.col.actor'),
      render: (m) => {
        if (m.createdBy === null) return '-';
        const actor = seatLabel(m.createdBy);
        return <ActorLabel actor={actor} tooltip={t('invMovements.actorTooltip', { id: actor.id })} />;
      },
    },
  ];
  ...
  return (
    <div className="im">
      ...
      {failed && (
        <ErrorBanner context="read" message={t('invMovements.error.transport')} onRetry={() => void loadBase()} />
      )}

      {showSkeleton ? (
        <DataTable columns={columns} rows={[]} rowKey={(m) => m.id} loading skeletonRows={4} />
      ) : items.length === 0 ? (
        ...
        <div className="im-split">
          <section className="im-pane" aria-label={t('invMovements.listLabel')}>
            <ul className="im-list">
              {items.map((i) => (
                <li key={i.id}>
                  {/* A list row: the open item is current (K-17, K-22), the pill, announced. */}
                  <button
                    type="button"
      ...
      {moveDrawer && selected !== null && (
        ...
        >
          ...
          <div className="im-field">
            ...
            <Select
              id="im-type"
              value={moveDraft.movementType}
              onChange={(value) => setMoveDraft({ ...moveDraft, movementType: value as RecordType })}
              options={RECORD_TYPES.map((ty) => ({ value: ty, label: t(`invMovements.type.${ty}`) }))}
              ariaLabel={t('invMovements.form.type')}
            />
          </div>
          <div className="im-field">
            ...
            <Select
              id="im-loc"
              value={moveDraft.locationId}
              onChange={(value) => setMoveDraft({ ...moveDraft, locationId: value })}
              options={locations.map((l) => ({ value: l.id, label: l.name }))}
              ariaLabel={t('invMovements.form.location')}
            />
          </div>
          <div className="im-field">
        ...
        >
          ...
          <div className="im-field">
            ...
            <Select
              ...
              onChange={(value) => setTransferDraft({ ...transferDraft, fromLocationId: value })}
              options={locations.map((l) => ({ value: l.id, label: l.name }))}
              ariaLabel={t('invMovements.form.from')}
            />
          </div>
          <div className="im-field">
            ...
            <Select
              ...
              options={locations.map((l) => ({ value: l.id, label: l.name }))}
              ariaLabel={t('invMovements.form.to')}
            />
          </div>
          <div className="im-field">
