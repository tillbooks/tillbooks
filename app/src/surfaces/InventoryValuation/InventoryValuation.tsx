...
export function InventoryValuation() {
  ...
  const methodColumns: DataTableColumn<MethodRow>[] = [
    ...
    },
    {
      ...
      render: (m) => (m.isDefault ? <span className="iv-tag">{t('invValuation.isDefault')}</span> : null),
    },
  ];
  ...
  const historyColumns: DataTableColumn<Assignment>[] = [
    ...
    { key: 'method', header: t('invValuation.col.method'), render: (a) => t(`invValuation.method.${a.method}`) },
    {
      ...
      render: (a) => (
        <>
          {a.reason ?? '-'}
          {a.forceRevaluation && <span className="iv-tag">{t('invValuation.history.forced')}</span>}
        </>
      ),
    },
      ...
      <SurfaceHeader
        ...
        actions={
          <p className="iv-total">
            {t('invValuation.workspaceTotal')}: <strong className="t-money">{formatMoney(totalValueMinor, 'CHF')}</strong>
          </p>
        }
      />

      {failed && <ErrorBanner context="read" message={t('invValuation.error.transport')} onRetry={() => void loadBase()} />}
                  ...
                  <>
                    ...
                    <div className="iv-controls">
                      <div className="form-row">
                        ...
                        <input id="iv-asof" className="field" type="date" value={asOf} onChange={(e) => setAsOf(e.target.value)} />
                      </div>
                      <div className="form-row">
                        <label htmlFor="iv-nrv">{t('invValuation.netRealisableValue')}</label>
                        <input id="iv-nrv" className="field" type="number" min="0" value={nrv} onChange={(e) => setNrv(e.target.value)} />
                        ...
                        <dl className="iv-figures">
                          <div>
                            ...
                            <dd>
                              ...
                              <span className="iv-tag">{t(`invValuation.source.${valuation.methodSource}`)}</span>
                            </dd>
                          </div>
                          <div>
                          ...
                          <div>
                            ...
                            <dd className="t-money">{valuation.unitCostMinor === null ? '-' : formatMoney(valuation.unitCostMinor, 'CHF')}</dd>
                          </div>
                          <div>
                            <dt>{t('invValuation.totalValue')}</dt>
                            <dd>
                              <strong className="t-money">{formatMoney(valuation.totalValueMinor, 'CHF')}</strong>
                              ...
                              {valuation.valuationBasis === 'allocated' && (
                                <>
                                  {' '}
                                  <span className="iv-tag">{t('invValuation.basis.allocated')}</span>
                                </>
                              )}
                            </dd>
                          </div>
                          {valuation.lcmApplied && (
                            <div>
                              ...
                              <dd>
                                <span className="iv-tag">{t('invValuation.lcmApplied')}</span>{' '}
                                <span className="t-money">{formatMoney(valuation.writeDownMinor, 'CHF')}</span>
                              </dd>
                            </div>
                          )}
                          {valuation.varianceMinor !== null && (
                            <div>
                              ...
                              <dd className="t-money">{formatMoney(valuation.varianceMinor, 'CHF')}</dd>
                            </div>
                          )}
                        </dl>
      ...
      >
        ...
        <div className="form-stack">
          <div className="form-row">
            ...
            <Select
              id="iv-method"
              value={defaultDraft.method}
              onChange={(value) => setDefaultDraft({ ...defaultDraft, method: value })}
              options={methods
                .filter((m) => m.enabled)
                .map((m) => ({ value: m.method, label: t(`invValuation.method.${m.method}`) }))}
              ariaLabel={t('invValuation.col.method')}
            />
          </div>
          <div className="form-row">
