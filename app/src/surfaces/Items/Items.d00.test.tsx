...
/** The sidebar category buttons, in DOM order, "Alle Artikel" and the add control included. */
...
describe('D00 categories, the sidebar tree', () => {
  ...
  it('degrades to no tree at all when the category read fails, and still lists the items', async () => {
    ...
    expect(sidebarLabels(container)).toEqual(['Alle Artikel', 'Neue Kategorie']);
    expect(rowNames(container)).toContain('Wasserflasche');
...
describe('D00 variants, indented under their parent', () => {
  ...
  it('opens a variant drawer titled for its parent, seeded with the parent defaults', async () => {
    ...
    expect(within(dialog).getByLabelText('Art')).toHaveValue('service');
    expect(within(dialog).getByLabelText('Einheit')).toHaveValue('hour');
    expect(within(dialog).getByLabelText('Kategorie')).toHaveValue('c_dienst');
...
describe('D00 sku, the article number', () => {
  ...
  it('seeds the editor from the stored article number and clears it to null in the patch', async () => {
    ...
    await userEvent.click(within(itemRow(container, 'Beratung')).getByRole('button', { name: 'Bearbeiten' }));
    const dialog = await screen.findByRole('dialog');
...
describe('D00 kind and unit, mirrored enums and nothing invented', () => {
  it('offers exactly the engine kinds, plus one honest "not stated" option', async () => {
    ...
    const options = within(within(dialog).getByLabelText('Art')).getAllByRole('option');
    expect(options.map((o) => (o as HTMLOptionElement).value)).toEqual(['', ...ITEM_KINDS]);
    expect(options.map((o) => o.textContent)).toEqual(['Nicht angegeben', 'Produkt', 'Dienstleistung']);
  });
  ...
  it('offers exactly the engine units, every one of them localised', async () => {
    ...
    const options = within(within(dialog).getByLabelText('Einheit')).getAllByRole('option');
    expect(options.map((o) => (o as HTMLOptionElement).value)).toEqual(['', ...ITEM_UNITS]);
    // No option fell back to a raw i18n key, which is what an unlocalised new enum member looks like.
    for (const option of options) expect(option.textContent).not.toMatch(/^item\.unit\./);
  });
  ...
  it('sends the enum VALUE the engine admits, never the German label a person read', async () => {
    ...
    await userEvent.selectOptions(within(dialog).getByLabelText('Art'), 'service');
    await userEvent.selectOptions(within(dialog).getByLabelText('Einheit'), 'flat');
    await userEvent.click(within(dialog).getByRole('button', { name: 'Speichern' }));
...
describe('D00 stock dials, and a disabled control that says why', () => {
  ...
  it('reveals the Meldebestand only once stock is tracked, and sends it in thousandths', async () => {
    ...
    await userEvent.selectOptions(within(dialog).getByLabelText('Art'), 'product');
    await userEvent.click(within(dialog).getByLabelText('Lagerführung'));
  ...
  it('seeds the editor from a tracked product, threshold included and trailing zeros trimmed', async () => {
    ...
    await userEvent.click(within(itemRow(container, 'Wasserflasche')).getByRole('button', { name: 'Bearbeiten' }));
    const dialog = await screen.findByRole('dialog');
...
describe('D00 the editor category picker', () => {
  it('offers every loaded category and files the new item under the chosen one', async () => {
    ...
    await userEvent.click(screen.getByRole('button', { name: 'Neuer Artikel' }));
    const dialog = await screen.findByRole('dialog');
    const picker = within(dialog).getByLabelText('Kategorie');
    const values = within(picker).getAllByRole('option').map((o) => (o as HTMLOptionElement).value);
    ...
    await userEvent.selectOptions(picker, 'c_mineral');
    await userEvent.click(within(dialog).getByRole('button', { name: 'Speichern' }));
  ...
  it('clears an item out of its category with an explicit null in the patch', async () => {
    ...
    await userEvent.click(within(itemRow(container, 'Beratung')).getByRole('button', { name: 'Bearbeiten' }));
    const dialog = await screen.findByRole('dialog');
    await userEvent.selectOptions(within(dialog).getByLabelText('Kategorie'), '');
    await userEvent.click(within(dialog).getByRole('button', { name: 'Speichern' }));
