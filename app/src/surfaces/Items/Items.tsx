...
export function Items() {
  ...
  const vatLabel = useMemo(() => {
    const map = new Map<string, string>();
    for (const code of vatCodes) map.set(code.code, code.label);
    return map;
  }, [vatCodes]);
  ...
  const emptyState = !hasItems ? (
    ...
    <EmptyState
      ...
      action={{ label: t('item.clearSearch'), onClick: () => setSearch('') }}
    />
  );
  ...
  const columns: DataTableColumn<Row>[] = [
      ...
      render: ({ item, isVariant }) => {
        ...
        return (
          <span className="item-name-cell">
            ...
            {archived && (
              <span className="item-archived-tag">
                <ArchiveGlyph />
                {t('item.archived')}
              </span>
            )}
    ...
    },
    {
      ...
      render: ({ item }) => {
        ...
        return <span className="item-badge">{vatLabel.get(taxCode) ?? taxCode}</span>;
      },
    },
    {
      key: 'category',
    ...
    {
      key: 'actions',
      header: t('item.col.actions'),
      headerHidden: true,
      align: 'end',
      render: ({ item }) => {
        if (!canManage) return null;
        const archived = item.archived === true;
        // Destructive LAST and marked `danger`, like every other caller in the Studio (D15/C2).
        ...
        return (
          <span className="item-actions">
            ...
            <OverflowMenu label={t('item.rowActions', { name: item.name })} items={actions} />
          </span>
        );
      },
    },
  ...
  const catalogPanel = (
    <div className="items-layout">
      <nav className="items-sidebar panel" aria-label={t('item.categories')}>
        ...
        {canManage && (
          <button type="button" className="items-cat items-cat--add" onClick={() => void createCategory()}>
            {t('item.newCategory')}
          </button>
        )}
      </nav>
    ...
    <div className="items-cat-dialog-overlay" role="presentation" onClick={onCancel}>
      ...
      >
        ...
        >
          ...
          />
          {touched && invalid && (
            <p id={errorId} className="item-field-error" role="alert">
...
function CategoryEntry({ category, isChild, canManage, active, onSelect, onAddChild, onRename, onDelete }: CategoryEntryProps) {
  ...
  return (
    <div className="items-cat-entry">
      ...
      {canManage && (
        <OverflowMenu label={t('item.categoryActions', { name: category.name })} items={actions} />
      )}
    </div>
  );
}
