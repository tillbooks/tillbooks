  ...
  return (
    <div className="pricelists">
        ...
        <div className="item-row2">
          ...
          <div className="item-field-inner">
            ...
            <Select
              value={scope}
              onChange={(value) => {
                setScope(value as 'segment' | 'contact');
              ...
              options={[
                { value: 'segment', label: t('pricelists.scope.segment') },
                { value: 'contact', label: t('pricelists.scope.contact') },
              ]}
              ariaLabel={t('pricelists.scopeLabel')}
            />
          </div>
        </div>
        <div className="item-field-inner">
          ...
          {scope === 'contact' && contactsReadable ? (
            <Select
              ...
              options={[
                { value: '', label: t('pricelists.contactNone') },
                ...contacts.map((c) => {
                  ...
                  return {
                    value: c.id,
                    label: taken ? t('pricelists.contactTaken', { name: c.name }) : c.name,
                    disabled: taken,
                  };
                }),
              ]}
              ariaLabel={t('pricelists.scope.contact')}
            />
          ) : (
            <input
              className="field"
              value={scopeValue}
              aria-label={scope === 'contact' ? t('pricelists.scope.contact') : t('pricelists.scope.segment')}
      ...
      {lists === null ? null : lists.length === 0 ? (
        // No action on purpose: the create form stands directly above, so a second "Preisliste
        // anlegen" would be the same control twice on one screen.
        <EmptyState title={t('pricelists.empty')} hint={t('pricelists.emptyHint')} />
      ) : (
        <ul className="item-list panel">
          {lists.map((list) => (
            <li key={list.id} className="item-row">
              {/* The list whose prices show below is the CURRENT row (K-22): announced, and the pill. */}
              <button
                type="button"
                className="item-row-open"
                aria-current={selected === list.id ? 'true' : undefined}
        ...
        <div className="pricelists-prices panel">
          ...
          <div className="item-row2">
            <div className="item-field-inner">
              ...
              <Select
                value={priceItemId}
                onChange={(value) => setPriceItemId(value)}
                options={[
                  { value: '', label: t('item.itemNone') },
                  ...items.map((i) => ({ value: i.id, label: i.name })),
                ]}
                ariaLabel={t('pricelists.item')}
              />
            </div>
            <label className="item-field-inner">
              ...
              {prices.map((p) => {
                ...
                return (
                  <li key={p.id} className="item-row">
                    ...
                    <span className="item-price item-num t-money">{formatMoney(p.priceMinor, p.currency)}</span>
      ...
      <div className="pricelists-resolve panel">
        ...
        <div className="item-row2">
          <div className="item-field-inner">
            ...
            <Select
              value={resolveItemId}
              onChange={(value) => setResolveItemId(value)}
              options={[
                { value: '', label: t('item.itemNone') },
                ...items.map((i) => ({ value: i.id, label: i.name })),
              ]}
          ...
          <div className="item-field-inner">
            ...
            {contactsReadable ? (
              <Select
                ...
                onChange={(value) => setResolveContactId(value)}
              ...
              <input
                className="field"
                value={resolveContactId}
                aria-label={t('pricelists.scope.contact')}
        ...
        {resolveResult !== null && (
          <p className="pricelists-result">
            <span className="item-price item-num t-money">{formatMoney(resolveResult.priceMinor, resolveResult.currency ?? baseCurrency)}</span>
            {/*
            ...
            */}
            <span className="item-tag">{tStrict(`pricelists.source.${resolveResult.source}`)}</span>
          </p>
        )}
      </div>
