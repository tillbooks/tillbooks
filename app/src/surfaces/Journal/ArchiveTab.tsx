    ...
    <div className="journal-archive" data-testid="archive-tab">
      ...
      >
        <label className="journal-field">
          ...
          <input className="field" type="date" value={filters.from} onChange={(e) => setFilters((f) => ({ ...f, from: e.target.value }))} />
        </label>
        <label className="journal-field">
          {t('journal.filters.to')}
          <input className="field" type="date" value={filters.to} onChange={(e) => setFilters((f) => ({ ...f, to: e.target.value }))} />
        </label>
        <label className="journal-field">
        ...
        <label className="journal-field">
          ...
          <input className="field" type="text" value={filters.text} onChange={(e) => setFilters((f) => ({ ...f, text: e.target.value }))} />
        </label>
        {/* The archive query is a paged read over an imported archive, the one place an explicit
            apply is kept (K-16): ONE secondary, the reset and the periods toggle quiet beside it. */}
                ...
                <tbody>
                  {periods.map((p) => (
                    <tr key={p.period} className="journal-archive-row">
                      <td>{formatCalendar(p.period, locale)}</td>
            ...
            {state.entries.map((entry) => {
              ...
              return [
                <tr key={entry.entryId} className="journal-archive-row">
                  ...
                  <td className="journal-num t-money">{formatMoney(totalMinor, entry.lines[0]?.currency ?? 'CHF')}</td>
                  <td>
                    {/* K-22: each flag is the shared Status word, glyph plus word, never a chip. */}
                    <span className="journal-archive-flags">
                      {!entry.balanced && <Status kind="warn" label={t('archive.row.unbalanced')} />}
                      {entry.lines.some((l) => l.targetNumber === null) && (
                        <Status kind="warn" label={t('archive.row.unmapped')} />
                      )}
                    </span>
                  </td>
                </tr>,
                          ...
                          <tbody>
                            {entry.lines.map((line, i) => (
                              <tr key={i} className="journal-archive-row">
                                ...
                                <td className="journal-num t-money">
