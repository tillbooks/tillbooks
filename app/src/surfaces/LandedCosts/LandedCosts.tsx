...
/** A voucher's state as the one `Status` word (K-22), never a text glyph. */
const STATUS_KIND: Record<string, StatusKind> = { draft: 'neutral', allocated: 'success', reversed: 'inactive' };
...
export function LandedCosts(): React.ReactElement {
  ...
  const columns: DataTableColumn<VoucherRow>[] = [
    { key: 'number', header: t('landedCosts.col.number'), render: (r) => r.number },
    {
      ...
      render: (r) => <Status kind={STATUS_KIND[r.status] ?? 'neutral'} label={t(`landedCosts.status.${r.status}`)} />,
    },
    ...
    { key: 'date', header: t('landedCosts.col.date'), render: (r) => formatCalendar(r.effectiveDate) },
  ];
        ...
        <section className="lc-form" aria-label={t('landedCosts.create')}>
          <div className="lc-field">
            ...
            <Select
              ...
              options={[
                { value: '', label: '·' },
                ...grs.map((g) => ({ value: g.id, label: g.number })),
              ]}
              ariaLabel={t('landedCosts.form.receipt')}
            />
          </div>
          <div className="lc-field">
            ...
            <Select
              ...
              options={COMPONENT_TYPES.map((c) => ({ value: c, label: t(`landedCosts.component.${c}`) }))}
              ariaLabel={t('landedCosts.form.component')}
            />
          </div>
          <label>
          ...
          <div className="lc-field">
            ...
            <Select
              ...
              options={METHODS.map((m) => ({ value: m, label: t(`landedCosts.method.${m}`) }))}
              ariaLabel={t('landedCosts.form.method')}
            />
          </div>
          <div className="lc-field">
            ...
            <Select
              value={formInvAcc}
              onChange={setFormInvAcc}
              options={[
                { value: '', label: '·' },
                ...accounts.map((a) => ({ value: a.id, label: `${a.number} ${a.name}` })),
              ]}
              ariaLabel={t('landedCosts.form.inventoryAccount')}
            />
          </div>
          <div className="lc-field">
            ...
            <Select
              value={formClrAcc}
              onChange={setFormClrAcc}
              options={[
                { value: '', label: '·' },
                ...accounts.map((a) => ({ value: a.id, label: `${a.number} ${a.name}` })),
              ]}
              ariaLabel={t('landedCosts.form.clearingAccount')}
            />
          </div>
        ...
        footer={
          detail !== null ? (
            <>
              ...
              {/* K-08: Stornieren is destructive, so it lives in the foot's overflow (it still asks
                  first); capitalising is the commit of a money write behind its confirm, the one role
                  of the tint. */}
              {detail.status === 'allocated' && mayWrite ? (
                <OverflowMenu
                  label={t('landedCosts.moreActions', { number: detail.number })}
                  items={[{ key: 'reverse', label: t('landedCosts.reverse'), danger: true, onSelect: () => setPendingWrite('reverse') }]}
                />
              ) : null}
              {detail.status === 'draft' && mayWrite ? (
                <button type="button" className="btn btn--accent" data-money-commit="landed_cost_allocate_confirm" onClick={() => setPendingWrite('confirm')}>
      ...
      <Modal
        ...
        footer={
          <>
            ...
            <button
              type="button"
              className={pendingWrite === 'reverse' ? 'btn btn--danger' : 'btn btn--accent'}
              data-money-commit={pendingWrite === 'reverse' ? undefined : 'landed_cost_allocate_confirm'}
