...
export function Members() {
  ...
  // K-08: "Rolle definieren" is the header's one primary while the Rollen tab is open; the editor
  // it opens carries its own save, so the header action steps aside while a draft is open.
  const defineRole =
    tab === 'roles' && canManage && state.kind === 'ok' && draftRole === null ? (
      <button
        type="button"
        className="btn btn--primary"
        disabled={busy}
        onClick={() => setDraftRole({ id: null, name: '', capabilities: [] })}
      >
  ...
  return (
    <section className="members" aria-labelledby="members-title">
      <SurfaceHeader
        ...
        help={<SurfaceHelp surface="Members" />}
        actions={defineRole}
      />
...
function MembersPanel({
  ...
  const columns: DataTableColumn<MemberDto>[] = [
      ...
      render: (member) => (
        <>
          <span className="members-person">
            ...
            {member.kind === 'agent' && <span className="members-kind-word">{t('members.rowKind.agent')}</span>}
          </span>
          {member.actorId !== null && (
      ...
      render: (member) =>
        canManage ? (
          <Select
            ...
            onChange={(val) => onSetRole(member.memberId, val)}
            options={assignable.map((role) => ({ value: role.id, label: roleLabel(role, tStrict) }))}
          />
        ) : (
    ...
    },
    {
      ...
      // K-22: the shared Status, a drawn glyph plus the word, never colour alone.
      render: (member) => (
        <Status kind={member.status === 'pending' ? 'pending' : 'success'} label={t(`members.status.${member.status}`)} />
      ),
    },
    {
      key: 'since',
  ...
  return (
    <>
      {/*
        ...
        redeemable by nobody) and links the Hosting journey. In local mode the invite fieldset below
        is DISABLED with this notice as its stated reason (see the fieldset comment); the Rollen tab
        stays open for preparing rolesets. Absent in served mode.
      */}
      {canManage && localMode && (
        <div id={noticeId}>
          <ActionFeedback
            ...
            message={
              <>
                {t('members.goOnline.notice')} <Link className="link-inline" to="/operations">{t('members.goOnline.link')}</Link>
              </>
            }
          />
        </div>
        ...
        >
          ...
          */}
          ...
          >
            ...
            {/* K-08: inviting is not a money write, so it is the form's primary, never the accent. */}
            <button type="submit" className="btn btn--primary" disabled={busy || inviteEmail === ''}>
              {t('members.invite')}
            </button>
          </fieldset>
            ...
            <li key={role.id} className="members-role panel">
              <div className="members-role-head">
                ...
                {role.isFixed ? (
                  ...
                  <span className="members-pill--fixed" tabIndex={0} aria-disabled="true" title={role.id}>
                    <LockGlyph />
      ...
      {draft !== null && (
        ...
        >
          ...
          <div className="members-role-editor-actions">
            <button type="submit" className="btn btn--primary" disabled={busy || draft.name === ''}>
