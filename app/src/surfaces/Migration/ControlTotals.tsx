  ...
  return (
    <section className="control-totals" aria-label={t('migration.controlTotals.title')}>
      ...
      >
        ...
        <select id="ct-kind" value={kind} onChange={(e) => pickKind(e.target.value)}>
          {DECLARABLE.map((d) => (
            <option key={d.kind} value={d.kind}>{t(`check.kind.${KIND_KEY[d.kind]}`)}</option>
          ))}
        </select>
        ...
        <button type="submit" className="migration-primary" disabled={busy || amount.trim() === ''}>
      ...
      {confirmRedeclare !== null && (
        <div className="control-totals-redeclare" role="group" aria-label={t('migration.controlTotals.redeclare.title')}>
          ...
          <button type="button" onClick={() => setConfirmRedeclare(null)}>{t('migration.intake.back')}</button>
        </div>
      )}
