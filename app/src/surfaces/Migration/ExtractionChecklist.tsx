    ...
    return (
      <section className="extraction" aria-label={t('migration.extraction.title')}>
        ...
        <div className="extraction-empty">
          ...
          {canManageImport && (
            <button type="button" className="btn btn--secondary" onClick={() => void createManifest()}>
  ...
  return (
    <section className="extraction" aria-label={t('migration.extraction.title')}>
            ...
            <li key={gi.id} className="extraction-item" data-attention={status === 'blocked' ? 'true' : undefined}>
              ...
              {canManageImport && (
                <Select
                  ...
                  ariaLabel={gi.what}
                  value={status}
                  onChange={(val) => void setItemStatus(gi.id, val as Status)}
                  options={STATUSES.map((s) => ({
                    value: s,
                    label: t(`migration.extraction.item.${statusKey(s)}`),
                  }))}
                />
              )}
            </li>
      ...
      <div className="extraction-letter">
        <button className="btn btn--secondary" type="button" onClick={() => setLetterOpen((v) => !v)} aria-expanded={letterOpen}>
