...
export function ImplementationProject({
  ...
  if (state.status === 'loading') {
    return (
      <section className="impl-project" aria-busy="true">
        <div className="impl-skeleton" aria-hidden="true" />
      </section>
    );
  }
  ...
  if (state.status === 'error') {
    return (
      <section className="impl-project" role="alert">
        <p>{t('implProject.error')}</p>
      </section>
    );
  }
                    ...
                    <RunbookItemRow
                      ...
                      actions={
                        task.status === 'open' && canManage ? (
                          <button type="button" onClick={() => void completeTask(task)}>{t('implProject.task.done')}</button>
                        ) : undefined
                      }
          ...
          {SIGNOFF_KINDS.map((camel) => {
            ...
            return (
              <li key={camel} className="impl-signoff-card">
                ...
                {canSign ? (
                  <button type="button" onClick={() => void recordSignoff(wire)}>{t('implProject.signoff.record')}</button>
                ) : (
        ...
        {view.parallelRun.periods.length === 0 ? (
          ...
          view.parallelRun.periods.map((p) => (
            <div key={p.period} className="impl-parallel-period" data-status={p.status}>
              ...
              {canManage && <button type="button" onClick={() => void runCheck(p.period)}>{t('implProject.parallel.recheck')}</button>}
            </div>
          ))
        )}
      </div>
...
function CreateProjectForm({ workspaceId, disabled, onCreated }: { workspaceId: string; disabled: boolean; onCreated: () => void }): React.ReactElement {
  ...
  return (
    <div className="impl-create">
      <label>{t('implProject.createField.sourceSystem')}<input value={sourceSystem} onChange={(e) => setSourceSystem(e.target.value)} /></label>
      <label>{t('implProject.createField.cutoverDate')}<input type="date" value={cutoverDate} onChange={(e) => setCutoverDate(e.target.value)} /></label>
      <label>{t('implProject.createField.mwstMethod')}
        <select value={mwstMethod} onChange={(e) => setMwstMethod(e.target.value)}>
          <option value="effektiv">effektiv</option>
          <option value="saldo">saldo</option>
        </select>
      </label>
      <button type="button" disabled={disabled} onClick={() => void create()}>{t('implProject.create')}</button>
      ...
      <ul>
        {templates.map((tpl) => (
          <li key={tpl.templateId}>
            ...
            <p>{tpl.description}</p>
            <button type="button" disabled={disabled} onClick={() => void instantiate(tpl.templateId)}>{t('implProject.runbook.instantiate')}</button>
          </li>
        ))}
      </ul>
    </div>
