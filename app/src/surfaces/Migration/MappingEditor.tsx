...
export function MappingEditor(props: { workspaceId: string; planId: string; currency: string }): React.ReactElement {
  ...
  async function suggest(): Promise<void> {
    ...
    // when a proposal has nothing to say about that row.
  ...
  const columns: DataTableColumn<MapEntry>[] = [
    ...
    },
    {
      ...
      render: (e) => (
        <input
          type="text"
          className="mapping-target-input"
          value={e.target ?? ''}
  ...
  return (
    <section className="mapping" aria-label={t('migration.mapping.title')}>
      ...
      <div className="mapping-tabs" role="tablist" aria-label={t('migration.mapping.title')}>
        {KINDS.map((k) => (
          <button
            key={k}
            type="button"
            ...
            onClick={() => setKind(k)}
          >
            {t(`migration.mapping.kind.${k}`)}
          </button>
        ))}
      </div>

      {state.status === 'error' ? (
        <p className="mapping-error" role="alert">{t('migration.mapping.error')}</p>
      ) : (
        <DataTable
          columns={columns}
      ...
      {state.status === 'loaded' && (
        <>
          ...
          <div className="mapping-actions">
            <button type="button" className="mapping-suggest" onClick={() => void suggest()}>{t('migration.mapping.suggest')}</button>
            <button type="button" className="migration-primary" disabled={saving || draft.length === 0} onClick={() => void save()}>
