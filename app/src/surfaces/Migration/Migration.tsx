  ...
  if (workspaceId === null) {
    return (
      <section className="migration" aria-labelledby="migration-title">
        <SurfaceHeader title={t('migration.title')} titleId="migration-title" />
    ...
    return (
      <section className="migration" aria-labelledby="migration-title" aria-busy="true">
        <h1 id="migration-title">
          {t('migration.title')}
          <SurfaceHelp surface="Migration" />
        </h1>
        <p className="migration-loading">{t('migration.loading')}</p>
        <div className="migration-skeleton" aria-hidden="true">
          <div className="migration-skeleton-row" />
          <div className="migration-skeleton-row" />
          <div className="migration-skeleton-row" />
        </div>
      </section>
  ...
  if (view.status === 'error') {
    return (
      <section className="migration" aria-labelledby="migration-title">
        ...
        <p className="migration-error" role="alert">{t('migration.error')}</p>
      </section>
    );
  }
  ...
  return (
    <section className="migration" aria-labelledby="migration-title">
      ...
      <div className="migration-plan-identity">
        ...
        {plans.length > 1 && (
          <label className="migration-plan-select">
            {t('migration.plan.selectorLabel')}
            <select value={plan.planId} onChange={(e) => { setAbandonConfirm(false); setSelectedPlanId(e.target.value); }}>
              {plans.map((p) => (
                <option key={p.planId} value={p.planId}>{planIdentity(t, p)}</option>
              ))}
            </select>
          </label>
        )}
        ...
        {canManageImport && plan.state !== 'closed' && plan.state !== 'abandoned' && (
          !abandonConfirm ? (
            <button type="button" className="migration-abandon-open" onClick={() => setAbandonConfirm(true)}>
            ...
            <div className="migration-abandon-confirm" role="group" aria-label={t('migration.plan.abandon')}>
              ...
              <button type="button" className="migration-danger" onClick={() => void abandon()}>
                {t('migration.plan.abandon')}
              </button>
              <button type="button" onClick={() => { setAbandonConfirm(false); setAbandonError(null); }}>{t('migration.intake.back')}</button>
              {abandonError !== null && (
            ...
            <li key={step.stepId} className="migration-step" data-attention={attention ? 'true' : undefined}>
              ...
              {step.state === 'checked' && (
                <div className="migration-approval" role="group" aria-label={t('migration.approval.pending')}>
                  ...
                  <button type="button" className="migration-primary" onClick={() => void approve(step)}>
              ...
              {step.state === 'diverged' && (
                <div className="migration-diverged" role="group" aria-label={t('migration.diverged.title')}>
                  ...
                  <button type="button" onClick={() => void rollback(step)}>{t('migration.diverged.reverse')}</button>
                  ...
                  <p className="migration-diverged-restore">
                    {t('migration.diverged.restoreHint')} <Link to="/setup">{t('migration.diverged.restoreLink')}</Link>.
                  </p>
      ...
      {canManageImport && (plan.state === 'live' || plan.state === 'closed') && (
        <div className="migration-close">
          {plan.state === 'closed' ? (
            ...
            <button type="button" className="migration-close-open" onClick={() => setCloseConfirm(true)}>
            ...
            <div className="migration-close-confirm" role="group" aria-label={t('migration.plan.close')}>
              ...
              <button type="button" className="migration-primary" onClick={() => void closePlan()}>
                {t('migration.plan.close')}
              </button>
              <button type="button" onClick={() => setCloseConfirm(false)}>{t('migration.intake.back')}</button>
            </div>
          )}
          {closeError !== null && (
