    ...
    return (
      <section className="check" aria-busy="true" aria-label={t('check.title')}>
        ...
        <div className="check-skeleton" aria-hidden="true">
          <div className="check-skeleton-block" />
          <div className="check-skeleton-block" />
        </div>
      </section>
    );
  ...
  if (state.status === 'error') {
    return (
      <section className="check" aria-label={t('check.title')}>
        <h2>{t('check.title')}</h2>
        <p className="check-error" role="alert">{t('check.error')}</p>
      </section>
    );
  }
  ...
  if (state.status === 'empty') {
    return (
      <section className="check" aria-label={t('check.title')}>
        ...
        <p>{t('check.empty')}</p>
        {stepId !== null && (
          <button type="button" className="migration-primary" onClick={() => void run()}>
            {t('check.run')}
          </button>
        )}
  ...
  return (
    <section className="check" aria-label={t('check.title')}>
            ...
            <li key={control.controlId} className="check-control" data-status={control.status}>
              ...
              {control.status === 'failed' && control.kind !== 'source_as_at' && (
                <div className="check-recover" role="group" aria-label={t('check.recover.fixOpening')}>
                  ...
                  {stepId !== null && (
                    <button type="button" className="check-recover-action" onClick={() => void run()}>
                ...
                <div className="check-waive">
                  {waivingId === control.controlId ? (
                    <>
                      ...
                      <button type="button" onClick={() => void waive(control.controlId)}>{t('check.waive.action')}</button>
                    </>
                  ) : (
                    <button
                      type="button"
      ...
      <div className="check-actions">
        ...
        <button type="button" onClick={() => void exportBericht(check.checkId)}>{t('check.export')}</button>
