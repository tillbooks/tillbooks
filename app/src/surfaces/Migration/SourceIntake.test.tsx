...
describe('SourceIntake', () => {
  ...
  it('offers a per-file worksheet choice when a source reports worksheets (G18 US-G18.2)', async () => {
    ...
    fireEvent.change(input, { target: { files: [file] } });
    const picker = await screen.findByLabelText(de.migration.source.worksheet);
    expect(picker).toBeTruthy();
    fireEvent.click(picker);
    expect(screen.getAllByRole('option').map((o) => o.getAttribute('data-value'))).toEqual(['Kontakte', 'Notizen']);
    fireEvent.click(screen.getByRole('option', { name: 'Notizen' }));
    expect(picker).toHaveTextContent('Notizen');
  });
  ...
  it('forces a chosen format on retry and sends it as the discover override (K-15)', async () => {
    ...
    fireEvent.click(screen.getByLabelText(de.migration.intake.failure.forceEncoding));
    fireEvent.click(screen.getByRole('option', { name: 'latin1' }));
    fireEvent.click(screen.getByLabelText(de.migration.intake.failure.forceAdapter));
    fireEvent.click(screen.getAllByRole('option').find((o) => o.getAttribute('data-value') === 'bexio_csv')!);
