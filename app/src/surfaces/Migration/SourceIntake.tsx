    ...
    <section className="intake" aria-busy={busy ? 'true' : undefined}>
      ...
      {phase === 'files' && (
        <div className="intake-files">
          ...
          >
            <p>{t('migration.intake.drop.hint')}</p>
            <button type="button" className="intake-choose" disabled={busy} onClick={() => fileInput.current?.click()}>
              {t('migration.intake.drop.choose')}
            </button>
            <input
              ref={fileInput}
              type="file"
              ...
              aria-label={t('migration.intake.drop.choose')}
              onChange={(e) => {
                if (e.target.files !== null && e.target.files.length > 0) void ingest(e.target.files);
                e.target.value = '';
              }}
            />
          </div>
              ...
              {discovered.map((f) => (
                <li key={f.fileId} className="intake-file">
                  ...
                  {f.dataClasses.length > 0 && (
                    <p className="intake-file-classes">
                      {f.dataClasses.map((c) => (
                        <span key={c} className="intake-class-chip">{classLabel(t, c)}</span>
                      ))}
                    </p>
                  )}
                  {f.warnings.length > 0 && (
              ...
              {failures.map((f) => (
                <li key={f.fileId} className="intake-failure" role="note">
                    ...
                    <div className="intake-failure-force-fields">
                      <label>
                        ...
                        <select
                          ...
                          onChange={(e) => setOverrideField(f.fileId, 'adapter', e.target.value)}
                        >
                          <option value="">{t('migration.intake.failure.forceAuto')}</option>
                          {adapters.map((a) => (
                      ...
                      <label>
                        ...
                        <select
                          ...
                          onChange={(e) => setOverrideField(f.fileId, 'encoding', e.target.value)}
                        >
                          <option value="">{t('migration.intake.failure.forceAuto')}</option>
                          {FORCE_ENCODINGS.map((enc) => (
                      ...
                      <label>
                        ...
                        <select
                          ...
                          onChange={(e) => setOverrideField(f.fileId, 'delimiter', e.target.value)}
                        >
                          <option value="">{t('migration.intake.failure.forceAuto')}</option>
                          {FORCE_DELIMITERS.map((d) => (
                            <option key={d} value={d}>{t(`migration.intake.failure.delimiterName.${d}`)}</option>
                          ))}
                        </select>
                      </label>
                    </div>
                    <button type="button" className="intake-failure-force-retry" disabled={busy} onClick={() => void forceRetry(f)}>
                  ...
                  <div className="intake-failure-actions">
                    <button type="button" className="intake-failure-retry" disabled={busy} onClick={() => void retryDiscover(f)}>
                      {t('migration.intake.failure.retry')}
                    </button>
                    <button type="button" className="intake-failure-remove" onClick={() => removeFailure(f.fileId)}>
          ...
          {discovered.length > 0 && (
            <button type="button" className="migration-primary" disabled={busy} onClick={() => setPhase('plan')}>
      ...
      {phase === 'plan' && (
        ...
        >
          ...
          <select id="intake-source" value={sourceSystem} onChange={(e) => setSourceSystem(e.target.value)}>
            {adapters.map((a) => (
              <option key={a.id} value={a.id}>{a.label}</option>
            ))}
          </select>
          ...
          <div className="intake-actions">
            <button type="button" className="intake-back" onClick={() => setPhase('files')}>{t('migration.intake.back')}</button>
            <button type="submit" className="migration-primary" disabled={busy || cutoverDate === ''}>
      ...
      {phase === 'scope' && (
        <div className="intake-scope">
          ...
          <button type="button" className="migration-primary" disabled={busy || selected.size === 0} onClick={() => void saveScope()}>
