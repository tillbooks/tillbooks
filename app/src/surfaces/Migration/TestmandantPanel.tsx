  ...
  if (phase.status === 'loading') {
    return (
      <section className="testmandant" aria-busy="true">
        <div className="testmandant-skeleton" aria-hidden="true" />
      </section>
    );
  }
  ...
  if (phase.status === 'none') {
    ...
    return (
      <section className="testmandant testmandant-empty">
        ...
        {canCreate && (
          <button type="button" className="migration-primary" disabled={busy || !caps.can(CAP.manageImport)} onClick={() => void createTestmandant()}>
  ...
  return (
    <section className="testmandant">
      ...
      <div className="workspace-mode-banner" role="status">
        <span className="workspace-mode-glyph" aria-hidden="true">⚑</span>
        ...
        {checkClean ? (
          missingRight !== null ? (
            <button type="button" className="migration-primary" disabled aria-disabled="true" title={t(`capability.${missingRight}`)}>
              {primaryLabel}
            </button>
          ) : cutoverPending ? (
            ...
            <button type="button" className="migration-primary" disabled aria-disabled="true" aria-describedby="testmandant-stichtag-note">
              {primaryLabel}
            </button>
          ) : null
        ) : (
          <button type="button" className="migration-primary" onClick={onChanged}>{primaryLabel}</button>
        )}
      </div>
      ...
      {checkClean && missingRight === null && !cutoverPending && (
        ...
        >
          ...
          <button type="submit" className="migration-primary" disabled={busy || confirmName !== phase.legalName}>
      ...
      <details className="testmandant-overflow">
        ...
        <button type="button" className="testmandant-diff" disabled={busy || diff.status === 'loading'} onClick={() => void runDiff()}>
        ...
        {!discardConfirm ? (
          ...
          <div className="testmandant-discard-confirm" role="group" aria-label={t('migration.testmandant.discard')}>
            ...
            <button type="button" className="migration-danger" disabled={busy} onClick={() => void discard()}>
              {t('migration.testmandant.discard')}
            </button>
            <button type="button" onClick={() => setDiscardConfirm(false)}>{t('migration.intake.back')}</button>
          </div>
        )}
      </details>
    </section>
