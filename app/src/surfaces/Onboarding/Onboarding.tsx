...
export function Onboarding(): React.ReactElement {
  ...
  if (phase.status === 'loading') {
    // A skeleton that traces the real path-choice layout (title + three cards), never one block.
    return (
      <section className="onboarding" role="status" aria-busy="true" aria-live="polite">
        <span className="visually-hidden">{t('states.loading.label')}</span>
        <span className="skeleton onboarding-skeleton-title" aria-hidden="true" />
        <div className="onboarding-skeleton" aria-hidden="true">
          <span className="skeleton onboarding-skeleton-card" />
          <span className="skeleton onboarding-skeleton-card" />
          <span className="skeleton onboarding-skeleton-card" />
        </div>
      </section>
  ...
  return (
    <section className="onboarding">
      <h1>
        {t('onboarding.title')}
        <SurfaceHelp surface="Onboarding" />
      </h1>
      ...
      {progress !== null && progress.completedAt === null && (
        <div className="onboarding-resume" role="status">
          {/* K-24: the path name lives in the supporting line; the button names the saved STEP, so
              its accessible name can never collide with the path card's button below. */}
          <span className="onboarding-resume-label">
