  ...
  return (
    <>
      <tr>
        ...
        <td className="oi-num oi-money">{formatMoney(row.baseTotalOpenMinor, baseCurrency)}</td>
      </tr>
  ...
  const itemColumns: DataTableColumn<OpenItem>[] = [
    {
      ...
      render: (item) =>
        item.kind === 'document' && item.documentId !== null ? (
          <Link to={`/documents/${item.documentId}`}>{item.number ?? ''}</Link>
        ) : (
    ...
    <div className="oi-expansion">
      <dl className="oi-figures">
        <div className="oi-figure">
          ...
          <dd className="oi-money">{formatMoney(view.baseTotalOpenMinor, view.baseCurrency)}</dd>
        </div>
        {view.onAccountMinor !== 0 && (
          <div className="oi-figure">
            ...
            <dd className="oi-money">
              {formatMoney(Math.abs(view.onAccountMinor), view.baseCurrency)}{' '}
