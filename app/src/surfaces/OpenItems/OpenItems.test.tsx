...
describe('OpenItems, five states', () => {
  ...
  it('EMPTY (filtered): renders no zero tiles above the panel (gate F8.4)', async () => {
    ...
    // The header total is its own money span now (t-money), so a settled "CHF 0.00 offen" is the one
    // zero figure allowed on screen; no tile or panel may print another.
    expect(screen.queryAllByText('CHF 0.00').filter((el) => el.closest('.oi-total') === null)).toHaveLength(0);
  });
  ...
  it('POPULATED: renders the recorded rows with recorded money and recorded dates', async () => {
    ...
    expect(document.querySelector('.oi-total')?.textContent).toBe("CHF 4'075.00 offen");
    expect(document.querySelector('.oi-total .t-money')).toHaveTextContent("CHF 4'075.00");
...
describe('the aging tiles', () => {
  ...
  it('K-27: the tiles carry their own label with the help trigger right beside it', async () => {
    renderOpenItems();
    await screen.findByText('R-2026-0006');
    const heading = screen.getByText('Nach Fälligkeit');
    const head = heading.closest('.oi-tile-head') as HTMLElement;
    expect(head).not.toBeNull();
    expect(within(head).getByRole('button', { name: 'Hilfe zur Aufteilung nach Fälligkeit' })).toBeInTheDocument();
  });

  it('K-11: the two views are the shared Tabs', async () => {
    renderOpenItems();
    await screen.findByText('R-2026-0006');
    const list = screen.getByRole('tablist', { name: 'Offene Posten' });
    expect(within(list).getByRole('tab', { name: 'Nach Beleg' })).toHaveAttribute('aria-selected', 'true');
    expect(within(list).getByRole('tab', { name: 'Nach Kunde' })).toHaveClass('tabs-tab');
  });
});
...
describe('the reconciliation statement', () => {
  ...
  it('replaces it with the mismatch band, quoting both recorded figures', async () => {
    ...
    // Each figure is its own money span inside its sentence (C2 F4), so the sentence is read off the
    // paragraph and the figures off their spans.
    await waitFor(() =>
      expect(document.querySelector('.oi-mismatch-title')).toHaveTextContent(
        'Die offenen Posten und Konto 1100 unterscheiden sich um CHF -12.50.',
      ),
    );
    expect(document.querySelector('.oi-mismatch-detail')).toHaveTextContent("Die Liste zeigt CHF 4'075.00, das Konto CHF 4'087.50.");
    expect([...document.querySelectorAll('.oi-mismatch .t-money')].map((el) => el.textContent)).toEqual([
      'CHF -12.50',
      "CHF 4'075.00",
      "CHF 4'087.50",
    ]);
...
describe('a mixed-currency workspace', () => {
  it('shows every row in its own currency and never converts one in the browser', async () => {
    ...
    expect(document.querySelector('.oi-total')?.textContent).toBe("CHF 4'972.44 offen");
  });
...
describe('the Kunde filter', () => {
  ...
  it('offers exactly the customers the engine returned, plus an all-customers option', async () => {
    ...
    expect(picker).toHaveTextContent('Alle Kunden');
    await userEvent.click(picker);
    // The recording holds two customers across seven rows: each is offered once.
    expect(
      screen.getAllByRole('option').map((option) => option.textContent?.replace('✓', '').trim()),
    ).toEqual(['Alle Kunden', 'Beispiel GmbH', 'Muster AG']);
  });
  ...
  it('sends the chosen customer to the engine and puts it in the URL', async () => {
    ...
    await userEvent.click(screen.getByRole('option', { name: 'Beispiel GmbH' }));
  ...
  it('NAMES the customer a URL-borne filter is narrowed to, and clears it back to all', async () => {
    ...
    const picker = screen.getByLabelText('Kunde');
    expect(picker).toHaveTextContent('Muster AG');
    await userEvent.click(picker);
    expect(screen.getByRole('option', { selected: true })).toHaveTextContent('Muster AG');

    await userEvent.click(screen.getByRole('option', { name: 'Alle Kunden' }));
    await waitFor(() => expect(listSpy).toHaveBeenCalledTimes(2));
  ...
  it('still names the filtered customer when the filter matched nothing at all', async () => {
    ...
    const picker = screen.getByLabelText('Kunde');
    expect(picker).toHaveTextContent('Kunde contact_9');
    await userEvent.click(picker);
    expect(screen.getByRole('option', { selected: true })).toHaveTextContent('Kunde contact_9');
  });
});
...
describe('the Fälligkeits-Aufteilung popover', () => {
  ...
  it('hangs off the element that holds its trigger, and the stylesheet lifts it out of flow', async () => {
    ...
    // K-28: the stacking order is a token on the one overlay tier, never a literal.
    expect(popover).toMatch(/z-index:\s*var\(--t-z-popover\)/);
  });
