  ...
  const itemColumns: DataTableColumn<OpenItem>[] = [
    {
      ...
      render: (item) => {
        ...
        // A parked row belongs to no document, so it carries a word-only chip and no link.
        const primary =
          item.kind === 'document' && item.documentId !== null ? (
            <Link to={`/documents/${item.documentId}`}>{label}</Link>
          ) : (
            <span className="oi-chip">{label}</span>
          );
        // The Mahnstufe rides here as a word-only chip, and ONLY when the document has actually been
        // dunned (level > 0). A "Mahnstufe 0" chip would imply a dunning decision nothing has made.
    ...
    },
    {
      ...
      render: (item) => {
        const label = item.number ?? t(parkedKindKey(item));
        // This surface owns NO allocation control (A14 owns the allocator): the row action is an
        // entry point that navigates to where the write actually happens. No posting here.
        const actions =
        ...
        return actions.length > 0 ? (
          <OverflowMenu label={t('openItems.rowActionsFor', { row: label })} items={actions} />
        ) : null;
      },
    },
  ];
    ...
    <section className="oi" aria-labelledby="oi-title">
      ...
      */}
      <div className="oi-summary">
        {loading ? (
          ...
          <p className="oi-total oi-money">
            {t('openItems.headerTotal', { total: formatMoney(total, baseCurrency) })}
          </p>
        ) : null}
        {settled && reconciled && (
          <ReconciledLine
      ...
      <div className="oi-toolbar">
        <div className="oi-tabs" role="tablist" aria-label={t('openItems.title')}>
          {(['document', 'customer'] as const).map((value) => (
            <button
              key={value}
              type="button"
              role="tab"
              aria-selected={tab === value}
              className={`oi-tab${tab === value ? ' oi-tab--on' : ''}`}
              onClick={() => setParam('by', value === 'document' ? null : value)}
            >
              {t(value === 'document' ? 'openItems.tab.byDocument' : 'openItems.tab.byCustomer')}
            </button>
          ))}
        </div>
        ...
        <div className="oi-filters">
          ...
          */}
          {tab === 'document' && (
            <label className="oi-filter" htmlFor="oi-customer">
              <span>{t('openItems.column.customer')}</span>
              <select
                id="oi-customer"
                value={customerParam ?? ''}
                onChange={(event) => setParam('customer', event.target.value === '' ? null : event.target.value)}
              >
                <option value="">{t('openItems.filter.allCustomers')}</option>
          ...
          {tab === 'document' && (
            <label className="oi-filter" htmlFor="oi-currency">
              <span>{t('openItems.column.currency')}</span>
              <select
                id="oi-currency"
                value={currencyParam ?? ''}
                onChange={(event) => setParam('currency', event.target.value === '' ? null : event.target.value)}
              >
                <option value="">{t('openItems.filter.allCurrencies')}</option>
          ...
          {visibleItems.length === 0 ? (
            filterActive ? (
              <EmptyState
                ...
                action={{ label: t('openItems.empty.filtered.action'), onClick: resetFilters }}
              />
            ) : (
              <EmptyState
                title={t('openItems.empty.nothingOpen.title')}
