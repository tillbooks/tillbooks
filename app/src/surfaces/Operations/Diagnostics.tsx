...
/** The journal's loading state: the table's own shape, never a spinner (DESIGN.md). */
function JournalSkeleton() {
  const t = useT();
  return (
    <div className="diag-table-wrap" role="status" aria-busy="true" aria-live="polite">
    ...
    <Panel title={t('feedback.log.title')}>
      ...
      {state.kind === 'loading' && (
        <div role="status" aria-busy="true" aria-live="polite">
          <span className="visually-hidden">{t('states.loading.label')}</span>
          <span className="skeleton diag-skeleton-row" />
          <span className="skeleton diag-skeleton-row" />
        </div>
      )}
