        ...
        <ul className="pay-detail-list">
          {allocations.map((allocation) => (
            <li key={allocation.id}>
              ...
              <span className="pay-num t-money">{formatMoney(allocation.amountMinor, payment.currency)}</span>
              {allocation.skontoMinor > 0 && (
                <span className="pay-dim">
                  {t('payment.skonto')}{' '}
                  <span className="t-money">{formatMoney(allocation.skontoMinor, payment.currency)}</span>
                </span>
              )}
              {allocation.writeoffMinor > 0 && (
                <span className="pay-dim">
                  {t('payment.writeOff.noun')}{' '}
                  <span className="t-money">{formatMoney(allocation.writeoffMinor, payment.currency)}</span>
                </span>
              )}
