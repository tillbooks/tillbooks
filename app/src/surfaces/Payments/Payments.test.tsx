...
/**
 * The payment-row expand toggles, EXCLUDING the G17 surface-help glyph: the glyph is also a
 * collapsed disclosure (`aria-expanded=false`) and sits first in the header, so an unscoped
 * "first collapsed button" click would open the help panel instead of a row.
 */
function rowToggles(buttons: HTMLElement[]): HTMLElement[] {
  return buttons.filter((b) => !b.classList.contains('help-trigger'));
}
...
describe('S1: the Zahlungen list, in all five states', () => {
  ...
  it('EMPTY: invites the first payment, and does not blame a filter that is not set', async () => {
    ...
    expect(await screen.findByText(/Noch keine Zahlungen erfasst/)).toBeInTheDocument();
    expect(screen.queryByText(/Keine Zahlungen in diesem Zeitraum/)).not.toBeInTheDocument();
  });
  ...
  it('EMPTY (filtered): says the FILTER is empty and offers to reset it', async () => {
    ...
    expect(await screen.findByText('Keine Zahlungen in diesem Zeitraum.')).toBeInTheDocument();
  ...
  it('keeps the status word-only, with Guthaben as a CHIP that never replaces it', async () => {
    ...
    expect(screen.getByText(/Guthaben CHF 19.00 für Muster AG/)).toBeInTheDocument();
  });
...
describe('S3: the payment detail', () => {
  it('LOADING: proves get_payment went in flight before claiming a skeleton', async () => {
    ...
    await screen.findByText('Muster AG');
    await userEvent.click(rowToggles(screen.getAllByRole('button', { expanded: false }))[0]!);
...
describe('S2/S4/S5: the allocator, stacked money then candidates then preview', () => {
  ...
  it('"Als Guthaben parken" reveals the Guthaben field and fills it with the remainder', async () => {
    ...
    await within(dialog).findByRole('option', { name: 'Beispiel GmbH' });
    await userEvent.selectOptions(within(dialog).getByLabelText('Gegenpartei'), 'contact_2');
