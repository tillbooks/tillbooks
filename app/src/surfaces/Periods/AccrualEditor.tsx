      ...
      render: (r) =>
        r.status === 'draft' ? (
          <button
            ...
            className="btn btn--secondary btn--sm"
            disabled={!allowed || busy}
            onClick={() => setConfirm({ verb: 'provision_post', id: r.id, label: r.description })}
          >
            {t('accruals.post')}
          </button>
        ) : r.status === 'posted' || r.status === 'released' ? (
          <span className="accrual-row-actions">
            {r.openBalanceMinor > 0 && (
              <button
                type="button"
                className="btn btn--secondary btn--sm"
                disabled={!allowed || busy}
            ...
            {r.status === 'posted' && (
              <button
                ...
                className="btn btn--secondary btn--sm"
                disabled={!allowed || busy}
                onClick={() => setConfirm({ verb: 'provision_reverse', id: r.id, label: r.description })}
              >
                {t('accruals.reverse')}
              </button>
            )}
            {r.openBalanceMinor < r.amountMinor && (
              <button type="button" className="btn btn--secondary btn--sm" disabled={busy} onClick={() => void openReleases(r)}>
                {t('provisions.releases')}
              </button>
            )}
          </span>
        ) : null,
          ...
          >
            {drawer === 'accrual' ? (
              ...
              <label className="period-field">
                ...
                <select className="field" value={reason} onChange={(e) => setReason(e.target.value as ProvisionReason)}>
                  {PROVISION_REASONS.map((r) => (
                    <option key={r} value={r}>
                      {t(`provisions.reason.${r}`)}
                    </option>
                  ))}
                </select>
            ...
            {drawer === 'provision' && (
              <label className="period-field">
                ...
                <select className="field" value={provisionAccountId} onChange={(e) => setProvisionAccountId(e.target.value)}>
                  <option value="">{t('accruals.lines.pick')}</option>
                  {provisionOptions.map((a) => (
            ...
            <select
              ...
              onChange={(e) => (drawer === 'accrual' ? setContraId(e.target.value) : setExpenseAccountId(e.target.value))}
              aria-invalid={fieldError?.field === 'account' ? true : undefined}
              aria-describedby={fieldError?.field === 'account' ? errorId : undefined}
              disabled={accounts === null}
            >
              <option value="">{accountsFailed ? t('accruals.accountsFailed') : accounts === null ? t('accruals.accountsLoading') : t('accruals.lines.pick')}</option>
              {contraOptions.map((a) => (
                <option key={a.id} value={a.id}>
                  {a.number} {a.name}
                </option>
      ...
      {release !== null && (
        ...
        >
          ...
          <label className="period-field">
            ...
            <select className="field" value={releaseTargetId} onChange={(e) => setReleaseTargetId(e.target.value)}>
              <option value="">{t('accruals.lines.pick')}</option>
              {(accounts ?? []).filter((a) => a.type === 'income' || a.type === 'expense').map((a) => (
                <option key={a.id} value={a.id}>
                  {a.number} {a.name}
                </option>
              ))}
