 ...
 * (a filed VAT return or a year-end close) never shows an active reopen/unlock control: it is disabled
 * with a lock glyph and a note naming why, rather than shown then rejected. The year-end close is the
 * most destructive action, so it is confirm-gated. Every write carries an idempotency key
...
export function Periods(props: PeriodsProps = {}) {
  ...
  useEffect(() => {
    ...
    const row = document.querySelector<HTMLElement>('.period-row--focus');
  ...
  const explain = useCallback(
    (err: Err): string => {
      const period = typeof err.period === 'string' ? err.period : '';
      switch (err.error) {
        case 'period_locked':
  ...
  const closeMonth = useCallback(
    (period: string) => {
      ...
      void runWrite('close_month', { period, idempotencyKey: newIdempotencyKey() }, () => ({
        tone: 'success',
        text: t('period.monthClosed', { period }),
      }));
    },
    [runWrite, t],
  );
  ...
  const reopenMonth = useCallback(
    (period: string) => {
      void runWrite('reopen_month', { period, idempotencyKey: newIdempotencyKey() }, () => ({
        tone: 'success',
        text: t('period.monthReopened', { period }),
      }));
    },
    [runWrite, t],
  );
  ...
  const unlockPeriod = useCallback(
    (period: string) => {
      void runWrite('unlock_period', { period, idempotencyKey: newIdempotencyKey() }, () => ({
        tone: 'success',
        text: t('period.periodUnlocked', { period }),
      }));
    },
    [runWrite, t],
  );
  ...
  function renderLockAction(lock: PeriodLock) {
    const sealed = lock.kind === 'hard' && lock.reason !== null && SEALED_REASONS.has(lock.reason);
    // A sealed hard lock never offers an active control: disabled with a lock glyph and a reason note.
    if (sealed) {
      return (
        <div className="period-action">
          <button type="button" className="btn btn--secondary btn--sm" disabled aria-describedby={noteId}>
            <LockGlyph size={14} />
            {t('period.unlock')}
          </button>
          <span id={noteId} className="lock-note">
            {t('period.sealedNote')}
          </span>
        </div>
      );
    ...
    const label = isSoft ? t('period.reopenMonth') : t('period.unlock');
    const allowed = isSoft ? canManage : canUnlock;
    const onClick = isSoft ? () => reopenMonth(lock.period) : () => unlockPeriod(lock.period);
    return (
      <div className="period-action">
        <button
          type="button"
  ...
  // so there is no `footer` total to render. The actions column keeps its inline reopen/unlock
  // control per row: it stays where the state it acts on is read.
  const lockColumns: DataTableColumn<PeriodLock>[] = [
    {
      key: 'period',
      header: t('period.columns.period'),
      render: (lock) => <span className="period-num">{lock.period}</span>,
    },
    {
      key: 'state',
    ...
    },
    {
      ...
      render: (lock) =>
        lock.reason !== null && SEALED_REASONS.has(lock.reason)
          ? t(`period.reason.${lock.reason}`)
          : '',
    },
    {
      key: 'actions',
      header: t('period.columns.actions'),
      render: (lock) => renderLockAction(lock),
    },
  ];
  ...
  return (
    <section className="periods" aria-labelledby="periods-title">
        ...
        >
          {feedback.result !== undefined && (
            <p className="action-feedback__detail">
              {t('period.yearResult')}: {formatMoney(feedback.result.minor, feedback.result.currency)}
            </p>
          )}
      ...
      <div className="periods-actions panel">
        ...
        >
          <label className="period-field">
            ...
            >
              <option value="">{t('period.yearPlaceholder')}</option>
              {closableYears.map((year) => (
          ...
          <DataTable<PeriodLock>
            ...
            rowClassName={(lock) =>
              [
                ...
                lock.period === focusMonth ? 'period-row--focus' : '',
              ]
                .filter(Boolean)
                .join(' ')
            }
      ...
      {confirmMonth !== null && (
        <ConfirmPeriodDialog
          title={t('period.confirmMonthTitle', { period: confirmMonth })}
