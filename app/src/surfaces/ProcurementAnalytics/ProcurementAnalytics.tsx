...
export function ProcurementAnalytics() {
  ...
  return (
    <div className="procanalytics">
      ...
      <div className="procanalytics__body">
        <nav className="procanalytics__reports" aria-label={t('procurement.analytics.reportsLabel')}>
          {REPORTS.map((r) => (
            <button
              key={r.id}
              type="button"
              className={r.id === reportId ? 'procanalytics__report is-active' : 'procanalytics__report'}
              aria-current={r.id === reportId}
              onClick={() => setReportId(r.id)}
            >
              {t(`procurement.analytics.report.${r.id}`)}
            </button>
          ))}
        </nav>
        ...
        <section className="procanalytics__main">
          {report.needsPeriod && (
            <div className="procanalytics__filters">
              ...
              <button type="button" className="procanalytics__run" onClick={() => void load()}>
                {t('procurement.analytics.run')}
              </button>
            </div>
          )}
            ...
            <div className="procanalytics__totals" role="status">
              {totals.map((tt) => (
                <div key={tt.labelKey} className="procanalytics__total">
                  ...
                  <span className="procanalytics__total-value">{tt.value}</span>
                </div>
              ))}
            </div>
          )}
