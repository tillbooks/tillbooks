...
describe('Projects: the list keeps the width, the project opens in a drawer (K-19, D137)', () => {
  it('shows no second column on first paint, and opens the picked project in the wide drawer', async () => {
    const user = userEvent.setup();
    renderProjects(happyCanned());
    const list = await screen.findByRole('table', { name: 'Projektliste' });
    // Nothing is selected, and nothing stands beside the list waiting to be filled.
    expect(screen.queryByRole('dialog')).not.toBeInTheDocument();

    const row = within(list).getByRole('row', { name: /^P-0002/ });
    await user.click(row);
    const drawer = await screen.findByRole('dialog', { name: 'Umbau Büro' });
    expect(drawer).toHaveAttribute('data-width', 'wide');
    // The open project is the current row, the selection pill rather than a surface class.
...
describe('Projects: the editor', () => {
  it('creates a project with integer Rappen from the decimal input', async () => {
    ...
    await user.click(within(dialog).getByLabelText('Kunde'));
    await user.click(screen.getAllByRole('option').find((o) => o.getAttribute('data-value') === 'c2')!);
