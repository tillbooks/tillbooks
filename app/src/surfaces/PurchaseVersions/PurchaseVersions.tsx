 ...
 *
 * Round 2 (D137): order, version and violation states are Status words, the bordered and accent
 * badges are gone (K-22); the open order is the current-row pill (K-24); the amend inputs are the
 * shared `.field` (K-15); the impact box is the elevated step without a hairline (K-09); the empty
 * list offers the way to Einkauf (K-33); a failed read says so (K-35); every figure outside the
 * table is `.t-money` (tabular, never wrapped); sizes are tokens (K-37).
 */
...
/** An order's state as the one `Status` word (K-22), the same kinds Einkauf uses. */
const PO_STATUS_KIND: Record<string, StatusKind> = {
  draft: 'neutral',
  sent: 'pending',
  received: 'success',
  closed: 'inactive',
...
/** A version is either the one in force or one a later version replaced. */
const VERSION_STATUS_KIND: Record<string, StatusKind> = { active: 'success', superseded: 'inactive' };
...
export function PurchaseVersions() {
  ...
  const columns: DataTableColumn<PoSummary>[] = [
    { key: 'number', header: t('poVersions.col.number'), render: (r) => <span className="pov-code">{r.number}</span> },
    {
      ...
      render: (r) => <Status kind={PO_STATUS_KIND[r.status] ?? 'neutral'} label={t(`poVersions.status.${r.status}`)} />,
    },
  ...
  return (
    <div className="pov">
      ...
      {failed && <ErrorBanner message={t('poVersions.error.transport')} onRetry={() => void load()} context="read" />}
      ...
      <DataTable
        ...
        emptyState={
          <EmptyState
            title={t('poVersions.empty.title')}
            hint={t('poVersions.empty.hint')}
            action={{ label: t('poVersions.empty.action'), to: '/purchasing' }}
          />
        }
      />
      ...
      {detail !== null && (
        <DetailDrawer
          ...
          headerExtra={
            <Status kind={PO_STATUS_KIND[detail.status] ?? 'neutral'} label={t(`poVersions.status.${detail.status}`)} />
          }
            ...
            <ul className="pov-timeline">
              {versions.map((v) => (
                <li key={v.id}>
                  ...
                  <Status kind={VERSION_STATUS_KIND[v.status] ?? 'neutral'} label={t(`poVersions.vstatus.${v.status}`)} />{' '}
                  <span className="t-money">{formatMoney(v.totalRappen, v.currency)}</span>
          ...
          {amendmentId !== null && (
            <div className="pov-amend" role="group" aria-label={t('poVersions.amend.title')}>
              ...
              <div className="pov-field">
                ...
                <input id="pov-reason" className="field" value={reason} onChange={(e) => setReason(e.target.value)} />
              </div>
              {edits.map((e, i) => (
              ...
              {impact !== null && (
                <div className="pov-impact" role="group" aria-label={t('poVersions.amend.impactTitle')}>
                  <p className="pov-impact-delta">
                    {t('poVersions.amend.delta')}:{' '}
                    <strong className="t-money">{signedMoney(impact.committedValueDeltaRappen, detail.currency)}</strong>
                  </p>
                  {impact.violations.length > 0 && (
                    <ul className="pov-violations">
                      {impact.violations.map((v, k) => (
                        <li key={k}>
                          <Status kind="warn" label={t(`poVersions.violation.${v.code}`)} />
                        </li>
                      ))}
                    </ul>
                  )}
