...
/**
 * An order's state as the one `Status` word (K-22): an icon-set glyph beside the word, never a text
 * glyph and never colour alone (spec §6, WCAG 2.2). A draft has nothing to act on yet, a sent order
 * waits for its goods, a received one is done, a closed or cancelled one is out of play.
 */
...
export function Purchasing() {
  ...
  /** Status as the one Status word, assembled once for the list and the drawer. */
  const statusTag = (s: string): ReactNode => <Status kind={STATUS_KIND[s] ?? 'neutral'} label={statusLabel(s)} />;
  ...
  return (
    <section className="po" aria-labelledby="po-title">
      ...
      {failed && <ErrorBanner context="read" message={t('po.error.transport')} onRetry={() => void load()} />}

      {/* The shared Tabs (K-11) in its shared-panel model: only the active view renders, so the
          Scorecard, which reads on mount, still mounts only when its tab is chosen. */}
      <Tabs
        label={t('po.title')}
        tabs={(['orders', 'open', 'prices', 'scorecard'] as Tab[]).map((tabId) => ({ id: tabId, label: t(`po.tab.${tabId}`) }))}
        activeId={tab}
            ...
            <fieldset className="po-lines">
              ...
              {lines.map((line, index) => (
                <div key={index} className="po-line-row">
                  ...
                  />
                  {lines.length > 1 && (
                    <button type="button" className="btn btn--ghost btn--sm btn--icon" aria-label={t('po.action.removeLine')} onClick={() => setLines(lines.filter((_, i) => i !== index))}>
                      <CloseGlyph size={16} aria-hidden="true" />
                    </button>
                  )}
                </div>
              ))}
...
function PoDetailDrawer(props: DetailProps) {
  ...
  // K-08/K-32: the drawer foot holds the one primary (Senden, on a draft) at the bottom right, and
  // every rarer verb in one overflow beside it, the destructive Stornieren last and never a red
  // button in the open.
  const more = [
    ...(canWrite && status === 'sent'
      ? [
          { key: 'revise', label: t('po.action.revise'), onSelect: props.onRevise },
          { key: 'closeShort', label: t('po.action.closeShort'), onSelect: props.onCloseShort },
        ]
      : []),
    ...(canWrite && (status === 'draft' || status === 'sent')
      ? [{ key: 'cancel', label: t('po.action.cancel'), danger: true, onSelect: props.onCancel }]
      : []),
  ];
  const actions: ReactNode[] = [];
  if (more.length > 0) {
    actions.push(<OverflowMenu key="more" label={t('po.action.more', { number: detail.po.number })} items={more} />);
  }
  if (canWrite && status === 'draft') {
    actions.push(
      <button key="send" type="button" className="btn btn--primary" onClick={props.onSend}>
        {t('po.action.send')}
      </button>,
    );
  }
    ...
    <DetailDrawer
      ...
      headerExtra={
        <span className="po-detail-status">
          {statusTag(status)}
          {detail.po.revision > 1 && <span className="po-tag">{t('po.field.revision', { n: detail.po.revision })}</span>}
      ...
      {status === 'sent' && canWrite && (
        ...
        >
          <div className="po-field">
            ...
            <Select
              value={props.receiptLocation}
              onChange={props.setReceiptLocation}
              options={[
                { value: '', label: t('po.field.locationPick') },
                ...props.activeLocations.map((l) => ({ value: l.id, label: l.name })),
              ]}
              ariaLabel={t('po.field.location')}
            />
          </div>
          {detail.lines
        ...
        <div className="po-matches">
          ...
          <ul>
            {detail.matches.map((m) => (
              <li key={m.id}>
                <Status kind={m.status === 'variance' ? 'warn' : 'success'} label={t(`po.status.${status}`)} />{' '}
                <span className="t-money">{formatMoney(m.priceVarianceRappen, detail.po.currency)}</span>
              </li>
            ))}
          </ul>
        </div>
    ...
    <>
      {canWrite && (
        ...
        >
          ...
          />
          <input className="field" type="text" inputMode="decimal" aria-label={t('po.field.price')} placeholder="90.00" value={price} onChange={(e) => setPrice(e.target.value)} />
          <input className="field" type="date" aria-label={t('po.field.validFrom')} value={validFrom} onChange={(e) => setValidFrom(e.target.value)} />
          <input className="field" type="number" min="0" aria-label={t('po.field.leadTime')} value={leadTime} onChange={(e) => setLeadTime(e.target.value)} />
          <button type="submit" className="btn btn--primary btn--sm">
