 ...
 * right-aligned `.t-num` cell. The page header, the saved-view picker, the show-superseded toggle
 ...
 * the header and the drawer chrome is gone; what remains is genuinely Quotes-specific: the status
 * chip, the form fields and line grid, the drawer fact grid and the honest cloud-tier note.
 *
...
export function Quotes() {
  ...
  const statusGlyph = (q: QuoteView) => (q.expired ? STATUS_GLYPH.expired : STATUS_GLYPH[q.status] ?? '○');
  const statusChip = (q: QuoteView) => (
    <span className="quotes-status">
      <span aria-hidden="true">{statusGlyph(q)}</span> {statusLabel(q)}
    </span>
  );
          ...
          >
            <label className="quotes-field">
              ...
              <select value={draft.contactId} onChange={(e) => setDraft({ ...draft, contactId: e.target.value })} required>
                <option value="">{t('quotes.field.contactPick')}</option>
      ...
      {selected !== undefined && (
        ...
        >
          ...
          <dl className="quotes-drawer-facts">
            ...
            <dd className="quotes-money">{formatMoney(selected.totalMinor, selected.currency)}</dd>
            <dt>{t('quotes.field.status')}</dt>
            <dd>
              <span aria-hidden="true">{statusGlyph(selected)}</span> {statusLabel(selected)}
            </dd>
          ...
          {acceptLink !== null && acceptLink.quoteId === selected.id && (
            <div className="quotes-cloud-note" role="note">
              ...
              <input type="text" readOnly value={acceptLink.url} aria-label={t('quotes.send.copyLink')} onFocus={(e) => e.target.select()} />
            </div>
          )}
          ...
          {decliningId === selected.id && (
            ...
            >
              ...
              <div className="quotes-form-actions">
                ...
                <button type="button" className="btn btn--secondary btn--sm" onClick={() => setDecliningId(null)}>
                  {t('quotes.editor.discard')}
                </button>
              </div>
            </form>
