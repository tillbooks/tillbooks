...
describe('Reconciliation: the five states', () => {
  ...
  it('happy: a high row shows glyph plus text and the apply control', async () => {
    ...
    // K-26: "Korrigieren" rides the row's one overflow.
    await userEvent.click(within(row).getByRole('button', { name: /^Aktionen für die Gutschrift/ }));
    expect(await screen.findByRole('menuitem', { name: 'Korrigieren' })).not.toHaveAttribute('aria-disabled', 'true');
  });
});
...
describe('Reconciliation: the decisions', () => {
  ...
  it('override is reachable on an APPLIED row and shows the audit stamp', async () => {
    ...
    const user = userEvent.setup();
    const row = (await screen.findByText('R-2026-0001')).closest('tr') as HTMLElement;
    await user.click(within(row).getByRole('button', { name: /^Aktionen für die Gutschrift/ }));
    await user.click(await screen.findByRole('menuitem', { name: 'Korrigieren' }));
  ...
  it('f14: an open, unmatched row shows exactly ONE override control (Manuell zuordnen), not a duplicate Korrigieren', async () => {
    ...
    expect(within(row).queryByRole('button', { name: /^Aktionen für die Gutschrift/ })).toBeNull();
  });
...
describe('the camt board', () => {
  it('a statement in the URL loads the board and renders its unmatched row with Confirm and Book', async () => {
    ...
    // K-21: "Buchen" and "Aufteilen" ride the row's one overflow.
    await userEvent.click(within(row).getByRole('button', { name: /^Aktionen für die Bewegung/ }));
    expect(await screen.findByRole('menuitem', { name: 'Buchen' })).not.toHaveAttribute('aria-disabled', 'true');
    expect(screen.getByRole('menuitem', { name: 'Aufteilen' })).toBeInTheDocument();
  });
  ...
  it('A36-U1: the confirm dialog populates its picker from an engine-shaped list_vendor_bills row', async () => {
    ...
    expect(screen.getByRole('option', { name: /LG-1/ })).toHaveAttribute('data-value', 'vb_1');
  });
  ...
  it('confirming a debit against a vendor bill sends confirm_match with vendorBillId', async () => {
    ...
    const select = within(dialog).getByLabelText('Kreditorenrechnung');
    await user.click(select);
    await user.click(await screen.findByRole('option', { name: /LG-1/ }));
  ...
  it('A36-U3: the Werkbank books a debit through confirm_match with allocations, preserving the statement-line link', async () => {
    ...
    renderSurface(undefined, { transport, route: '/reconciliation?statement=stmt_1' });
    const user = userEvent.setup();
    await user.click(await screen.findByRole('button', { name: /^Aktionen für die Bewegung/ }));
    await user.click(await screen.findByRole('menuitem', { name: 'Aufteilen' }));
  ...
  it('booking an unmatched txn sends create_entry_for_txn with the chosen contra account', async () => {
    ...
    const user = userEvent.setup();
    await user.click(await screen.findByRole('button', { name: /^Aktionen für die Bewegung/ }));
    await user.click(await screen.findByRole('menuitem', { name: 'Buchen' }));
    const dialog = await screen.findByRole('dialog');
  ...
  it('permission denied pre-disables Confirm and Book, never shown-then-rejected', async () => {
    ...
    expect(note).not.toBeNull();
    expect(note).toHaveTextContent('Erfordert die Berechtigung zum Buchen von Zahlungen');
    // "Buchen" is in the row's overflow, offered disabled (never shown live and then refused).
    await userEvent.click(within(row).getByRole('button', { name: /^Aktionen für die Bewegung/ }));
    expect(await screen.findByRole('menuitem', { name: 'Buchen' })).toHaveAttribute('aria-disabled', 'true');
    await userEvent.keyboard('{Escape}');
    expect(screen.getByRole('button', { name: 'Kontoauszug importieren' })).toBeDisabled();
  });

  it('K-13: the statement import takes its file through the shared FileDrop, never a bare browser input', async () => {
    renderSurface(boardOf([camtRow('bt_fd')]), { route: '/reconciliation?statement=stmt_1' });
    const user = userEvent.setup();
    await user.click((await screen.findAllByRole('button', { name: 'Kontoauszug importieren' }))[0]!);
    const form = await screen.findByRole('form', { name: 'Kontoauszug importieren' });
    const input = form.querySelector('input[type="file"]');
    expect(input).not.toBeNull();
    // The real input is the FileDrop's visually hidden proxy, named by the Studio's own button text.
    expect(input).toHaveClass('file-drop-input');
    expect(within(form).getByLabelText('camt.053/054-Datei')).toBe(input);
...
describe('the camt board: A36 ranked suggestions', () => {
  ...
  it('a blocked batch-total mismatch shows both figures side by side and disables the one-click confirm', async () => {
    ...
    renderSurface(canned, { route: '/reconciliation?statement=stmt_1' });
    const row = (await screen.findByText('NTRY-1')).closest('tr') as HTMLElement;
    // The two figures are separate money spans (t-money), so the pair is read off the batch line's
    // whole text rather than one text node.
    await waitFor(() =>
      expect(row.querySelector('.camt-suggestion-batch')?.textContent).toMatch(/CHF 50\.00 \/ CHF 60\.00/),
    );
    const confirmBatch = within(row).getByRole('button', { name: 'Sammelzahlung bestätigen' });
