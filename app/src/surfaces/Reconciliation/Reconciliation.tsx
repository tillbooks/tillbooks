...
export function Reconciliation() {
  ...
  /** The row whose Review lane is expanded, or whose override dialog is open. */
    ...
    <section className="qr" aria-labelledby="qr-title">
      <SurfaceHeader
        ...
        actions={
          <>
            {savedViews.length > 0 && (
              <label className="qr__view">
                {t('qrmatch.savedView')}
                <select
                  value={viewParam ?? ''}
                  onChange={(e) => {
                    const next = new URLSearchParams(params);
                    if (e.target.value === '') next.delete('view');
                    else next.set('view', e.target.value);
                    setParams(next);
                  }}
                >
                  <option value="">{t('qrmatch.savedViewAll')}</option>
                  {savedViews.map((v) => (
      ...
      {note !== null && (
        <p className="qr__note" role="status">
          {note}
          {note === t('qrmatch.periodLocked') && <Link to="/periods">{t('qrmatch.toPeriods')}</Link>}
          {note === t('qrmatch.needsBankAccount') && <Link to="/bank-accounts">{t('qrmatch.toBankAccounts')}</Link>}
        </p>
      )}
      ...
      {loading ? (
        ...
        <>
          ...
          <table className="qr__table">
            <thead>
              <tr>
                <th scope="col">{t('qrmatch.col.date')}</th>
                <th scope="col" className="qr-num">
                  {t('qrmatch.col.amount')}
                </th>
                <th scope="col">{t('qrmatch.col.reference')}</th>
                <th scope="col">{t('qrmatch.col.match')}</th>
                <th scope="col">{t('qrmatch.col.state')}</th>
                <th scope="col">
                  <span className="visually-hidden">{t('qrmatch.col.actions')}</span>
                </th>
              </tr>
            </thead>
            <tbody>
              {[...openRows, ...decidedRows].map((row) => (
                <QueueRow
                  key={row.creditId}
                  row={row}
                  commitTarget={row.creditId === appliedId}
                  busy={busy === row.creditId}
                  canDecide={canDecide}
                  writeOffThresholdMinor={queue.writeOffThresholdMinor}
                  reviewOpen={reviewId === row.creditId}
                  onReviewToggle={() => setReviewId(reviewId === row.creditId ? null : row.creditId)}
                  onOverride={() => setOverrideId(row.creditId)}
                  onApply={apply}
                />
              ))}
            </tbody>
...
function QueueRow({
  ...
  row: QrCreditRowView;
  /** The row the Commit moment lands (D122 D-I): the credit just applied. */
  ...
  const lossMinor =
    score.invoiceOpenMinor !== null ? Math.max(0, score.invoiceOpenMinor - row.amountMinor) : 0;
  ...
  // f14: an open, unmatched row already offers "Manuell zuordnen", which opens the override dialog.
  // The generic "Korrigieren" button opens the SAME dialog with the SAME action, so on such a row the
  // two controls are indistinguishable duplicates: suppress "Korrigieren" when "Manuell zuordnen"
  // shows, leaving exactly one override control for the open-unmatched case.
  const showManualMatch = row.status === 'open' && (score.confidence === 'none' || !matched);

  return (
    <>
      <tr className={[row.status !== 'open' ? 'qr-row--decided' : '', commitTarget ? COMMIT_TARGET_CLASS : ''].filter(Boolean).join(' ') || undefined}>
        <td>{formatDate(row.valueDate)}</td>
        <td className="qr-num">{formatMoney(row.amountMinor, row.currency)}</td>
        <td className="qr-ref">
          {score.referenceDisplay ?? t('qrmatch.noReference')}
        ...
        <td>
          {matched ? (
            ...
            <span className="qr-muted">{t('qrmatch.noMatch')}</span>
          )}
        </td>
        <td>
        ...
        <td className="qr-actions">
          {row.status === 'open' && score.confidence === 'high' && matched && (
            <button
              type="button"
              className="btn btn--secondary"
              disabled={!canDecide || busy}
              aria-describedby={canDecide ? undefined : 'qr-decide-denied'}
              onClick={() => void onApply(row, score.invoiceId as string, 'full', applyKey)}
            >
              {t('qrmatch.applyFull')}
            </button>
          )}
          {row.status === 'open' && score.confidence === 'medium' && matched && !currencyDiffers && (
            <button
              type="button"
              className="btn btn--secondary"
              disabled={!canDecide || busy}
          ...
          {row.status === 'open' && currencyDiffers && (
            // The F6 lane: no button on this row can book a cross-currency credit (the engine
            // refuses every one), so the row's action is the surface that can.
      ...
      {reviewOpen && row.status === 'open' && score.confidence === 'medium' && matched && !currencyDiffers && (
        <tr className="qr-review">
          <td colSpan={6}>
            <div className="qr-review__panel">
              <p>
                ...
                {(score.creditedOpenMinor ?? 0) > 0 && (
                  <span className="qr-payer">
                    {t('qrmatch.creditedShare', {
                      credited: formatMoney(score.creditedOpenMinor ?? 0, row.currency),
                    })}
                  </span>
                )}
              ...
              <div className="qr-review__actions">
                <button
                  type="button"
                  className="btn btn--secondary"
                  disabled={!canDecide || busy}
                  onClick={() => void onApply(row, score.invoiceId as string, 'partial', applyKey)}
                >
                  {t('qrmatch.applyPartial')}
                </button>
                {/*
                  The F3 copy rule: accepting a shortfall as full payment BOOKS A LOSS, so the
                  button names the exact amount it forgives, and it is offered only inside A14's
                  one-click threshold: above it the engine refuses, so the surface points at the
                  deliberate route instead of rendering a control that cannot succeed.
                */}
                ...
                {lossMinor > writeOffThresholdMinor && (
                  <span className="qr-payer">
                    ...
                    <Link to="/payments">{t('qrmatch.toPayments')}</Link>
                  </span>
                )}
              </div>
            </div>
    ...
    >
      <label>
        ...
        <select value={bankAccountId} onChange={(e) => setBankAccountId(e.target.value)} required>
          <option value="">{t('qrmatch.form.accountPick')}</option>
          {bankAccounts.map((a) => (
  ...
  return (
    ...
    >
      <p className="qr-dialog__credit">
        {formatDate(row.valueDate)} · {formatMoney(row.amountMinor, row.currency)}
      ...
      {invoices === null ? (
        ...
        <label className="qr-dialog__pick">
          ...
          <select value={picked} onChange={(e) => setPicked(e.target.value)}>
            <option value="">{t('qrmatch.overridePickNone')}</option>
            {invoices
              .filter((i) => i.currency === row.currency && i.documentId !== row.invoiceId)
              .map((i) => (
                <option key={i.documentId} value={i.documentId}>
                  {`${i.number ?? i.documentId} · ${i.customerName ?? ''} · ${formatMoney(i.openMinor, i.currency)}`}
                </option>
              ))}
          </select>
        </label>
...
function ImportCamtForm({
  ...
  return (
    ...
    >
      ...
      {fileName.length > 0 && <span className="qr-payer">{fileName}</span>}
      <label className="qr__dial">
        <input
          type="checkbox"
        ...
        <span className="qr-payer camt-suggestion-batch">
          ...
          {proposal?.blocked === true && (
            <>
              ...
              {' / '}
              {typeof batchTotal === 'number' ? formatMoney(batchTotal, row.currency) : '…'}
              {')'}
            </>
          )}
        </span>
      ...
      {row.classification !== 'incoming_credit' && row.status === 'unmatched' && (
        <>
          ...
          <button
            ...
            disabled={!canDecide}
            aria-describedby={canDecide ? undefined : 'camt-decide-denied'}
            onClick={() => onWerkbank(row.bankTxnId)}
          >
            {t('camt.dialog.split')}
          </button>
          <button
            ...
            disabled={!canDecide}
            aria-describedby={canDecide ? undefined : 'camt-decide-denied'}
            onClick={() => onCreateEntry(row.bankTxnId)}
          >
            {t('camt.createEntry')}
          </button>
        </>
        ...
        <tbody>
          {preview.legs.map((leg: PreviewLeg, index: number) => (
            <tr key={`${leg.accountId}-${index}`}>
              ...
              <td className="qr-num">{leg.debitMinor > 0 ? `${t('camt.preview.soll')} ${formatMoney(leg.debitMinor, currency)}` : ''}</td>
              <td className="qr-num">{leg.creditMinor > 0 ? `${t('camt.preview.haben')} ${formatMoney(leg.creditMinor, currency)}` : ''}</td>
            </tr>
          ))}
          <tr className="camt-preview-total">
            <td>{t('camt.preview.total')}</td>
            <td className="qr-num">{formatMoney(total, currency)}</td>
            <td />
          </tr>
        </tbody>
      </table>
    ...
    >
      ...
      <label className="qr-dialog__pick">
        ...
        <select value={pickedId} onChange={(e) => setPickedId(e.target.value)}>
          <option value="">{t('qrmatch.overridePickNone')}</option>
          {bills.map((b) => (
            <option key={b.vendorBillId} value={b.vendorBillId}>
              {vendorBillLabel(b)}
            </option>
          ))}
        </select>
      ...
      <label className="qr-dialog__pick">
        ...
        <input value={entryId} onChange={(e) => setEntryId(e.target.value)} placeholder={t('camt.dialog.entryIdPlaceholder')} />
      </label>
    </Modal>
  );
}
                ...
                <tbody>
                  {bills.map((b) => (
                    <tr key={b.vendorBillId} className="pay-cand-row">
                      ...
                      <td className="qr-num">{formatMoney(b.openMinor, b.currency)}</td>
    ...
    <div className="camt-preview" aria-label={t('camt.preview.title')}>
      ...
      <table className="camt-preview-legs">
        <tbody>
          <tr>
            <td>{debitLeg}</td>
            <td className="qr-num">{`${t('camt.preview.soll')} ${amount}`}</td>
            <td className="qr-num" />
          </tr>
          <tr>
            <td>{creditLeg}</td>
            <td className="qr-num" />
            <td className="qr-num">{`${t('camt.preview.haben')} ${amount}`}</td>
          </tr>
        </tbody>
      </table>
    </div>
    ...
    <section className="qr-board panel" aria-labelledby="camt-statements-title">
      ...
      {statements === null ? (
        ...
        <DataTable<BankStatementSummaryView>
          ...
          rowClassName={(s) => (s.statementId === openId ? 'camt-statement--open' : undefined)}
        />
      )}
    </section>
  );
