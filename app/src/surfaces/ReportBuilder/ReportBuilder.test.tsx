...
/** Open a saved report's row overflow (K-21), where run, schedule, edit, duplicate and delete sit. */
async function openRowMenu(name: string): Promise<void> {
  await userEvent.click(screen.getByRole('button', { name: de.reportBuilder.rowActions.replace('{name}', name) }));
}
...
describe('ReportBuilder (Berichte)', () => {
  ...
  it('POPULATED: renders the saved report and its Ausführen action', async () => {
    ...
    // K-21: Ausführen sits in the row's one overflow.
    await openRowMenu('Kontaktliste');
    expect(screen.getByRole('menuitem', { name: de.reportBuilder.action.run })).toBeInTheDocument();
  });
  ...
  it('SCHEDULE: a weekly report opens the editor on weekly and saving untouched preserves it', async () => {
    ...
    await screen.findByText('Kontaktliste');
    // The list reads the stored cadence in words, never the engine's canonical string.
    expect(screen.getByText('Wöchentlich, Mittwoch, 08:00')).toBeInTheDocument();
    await openRowMenu('Kontaktliste');
    await userEvent.click(screen.getByRole('menuitem', { name: de.reportBuilder.action.schedule }));
    const freqSelect = screen.getByLabelText(de.reportBuilder.field.frequency);
    expect(freqSelect).toHaveTextContent(de.reportBuilder.freq.weekly);
    // The weekday the stored string carried (3, Wednesday) is prefilled by name, not reset.
    expect(screen.getByLabelText(de.reportBuilder.field.weekday)).toHaveTextContent(de.reportBuilder.weekday['3']);
  ...
  it('DUPLICATE: a rejected reports_duplicate shows the error banner', async () => {
    ...
    await openRowMenu('Kontaktliste');
    await userEvent.click(screen.getByRole('menuitem', { name: de.reportBuilder.action.duplicate }));
  ...
  it('BUILDER TITLE: create shows the new-report title, edit shows the edit title', async () => {
    ...
    await openRowMenu('Kontaktliste');
    await userEvent.click(screen.getByRole('menuitem', { name: de.reportBuilder.action.edit }));
