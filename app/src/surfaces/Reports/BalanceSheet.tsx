  ...
  const renderSection = (section: BalanceSheetSection) => (
    <Fragment key={section.key}>
      ...
      {section.lines.map((line) => {
        ...
        return (
          <tr key={`${section.key}-${line.key}`} className={computed ? 'rp-line rp-line--computed' : 'rp-line'}>
            ...
            <td className="rp-num rp-money">{formatMoney(line.balanceMinor, currency)}</td>
            {comparing && (
              <>
                <td className="rp-num rp-money">{compareCell(line.compareBalanceMinor)}</td>
      ...
      <tr className="rp-subtotal">
        ...
        <td className="rp-num rp-money">{formatMoney(section.subtotalMinor, currency)}</td>
        {comparing && (
          <>
            <td className="rp-num rp-money">{compareCell(section.compareSubtotalMinor)}</td>
        ...
        <table className="rp-table rp-table--statement">
          ...
          <tfoot>
            <tr>
              ...
              <td className="rp-num rp-money">{formatMoney(view.aktivenMinor, currency)}</td>
              {comparing && (
                <>
                  <td className="rp-num rp-money">{compareCell(view.compareAktivenMinor)}</td>
                  <td className="rp-num" />
                </>
              )}
            </tr>
            <tr>
              ...
              <td className="rp-num rp-money">{formatMoney(view.passivenMinor, currency)}</td>
              {comparing && (
                <>
                  <td className="rp-num rp-money">{compareCell(view.comparePassivenMinor)}</td>
                  <td className="rp-num" />
                </>
              )}
            </tr>
