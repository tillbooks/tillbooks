  ...
  return (
    <section className="rp" aria-labelledby="rp-title">
      ...
      */}
      ...
      <div className="rp-tabs" role="tablist" aria-label={t('reports.tablist')}>
        {REPORT_TABS.map((value) => (
          <button
            ...
            className={`rp-tab${tab === value ? ' rp-tab--on' : ''}`}
            onClick={() => setParam('report', value === 'trial' ? null : value)}
          >
            {t(TAB_LABEL[value])}
          </button>
        ))}
        ...
        {tab !== 'ledger' && (
          <label className="rp-field" htmlFor="rp-compare">
            ...
            <select
              id="rp-compare"
              value={preset}
              onChange={(event) => setParam('compare', event.target.value === 'none' ? null : event.target.value)}
            >
              {COMPARE_PRESETS.map((value) => (
                <option key={value} value={value}>
                  {t(COMPARE_LABEL[value])}
                </option>
              ))}
            </select>
          </label>
      ...
      {failed && <ErrorBanner message={t('reports.error.transport')} onRetry={() => void load()} />}
