 ...
 *
 * Round 2 (D137): the states are Status words, the bordered and accent badges are gone (K-22); the
 * open requisition is the current-row pill (K-24); Ablehnen and Stornieren leave the drawer foot for
 * its overflow (K-08); the inputs are the shared `.field` and the per-surface field rules are gone
 * (K-15); the convert panel is the elevated step without a hairline (K-09); secondary text is dim,
 * not faint (K-36); dates read TT.MM.JJJJ (K-38); a failed read says so (K-35); sizes are tokens.
 */
...
import { useMemberNames, ActorLabel } from '../../lib/useMemberNames';
...
/** A requisition's state as the one `Status` word (K-22), by what it asks of the reader. */
const STATUS_KIND: Record<string, StatusKind> = {
  draft: 'neutral',
  pending_approval: 'warn',
  approved: 'success',
  rejected: 'danger',
  partially_converted: 'pending',
  converted: 'success',
  cancelled: 'inactive',
  closed: 'inactive',
};
...
export function Requisitions() {
  ...
  const columns: DataTableColumn<Requisition>[] = [
    { key: 'number', header: t('requisitions.col.number'), render: (r) => <span className="req-code">{r.number}</span> },
    {
      ...
      render: (r) => <Status kind={STATUS_KIND[r.status] ?? 'neutral'} label={t(`requisitions.status.${r.status}`)} />,
    },
    {
      key: 'requester',
      header: t('requisitions.col.requester'),
      render: (r) => {
        const actor = memberNames.resolve(r.requesterId);
        return <ActorLabel actor={actor} tooltip={t('requisitions.requesterTooltip', { id: actor.id })} />;
      },
    },
    { key: 'neededBy', header: t('requisitions.col.neededBy'), render: (r) => (r.neededBy ? formatCalendar(r.neededBy) : '') },
  ...
  // foot off entirely; the header close control still closes the drawer. The two refusals (Ablehnen,
  // Stornieren) are destructive, so they ride the foot's overflow, never a red button beside the
  // primary (K-08).
  const detailActions: ReactNode[] = [];
  const destructive: OverflowMenuItem[] = [];
  if (s === 'draft') {
    detailActions.push(
  ...
  if (s === 'pending_approval') {
    ...
    destructive.push({
      key: 'reject',
      label: t('requisitions.action.reject'),
      danger: true,
      disabled: !canWrite,
      onSelect: () => void act('requisition_reject', { reason: decisionText }),
    });
  }
  ...
  if ((s === 'draft' || s === 'pending_approval' || s === 'approved') && detail !== null && detail.conversions.length === 0) {
    destructive.push({
      key: 'cancel',
      label: t('requisitions.action.cancel'),
      danger: true,
      disabled: !canWrite,
      onSelect: () => void act('requisition_cancel'),
    });
  }
  if (destructive.length > 0 && detail !== null) {
    detailActions.unshift(
      <OverflowMenu key="more" label={t('requisitions.moreActions', { number: detail.number })} items={destructive} />,
    );
  }
  ...
  return (
    <div className="req">
      ...
      {failed && <ErrorBanner message={t('requisitions.error.transport')} onRetry={() => void load()} context="read" />}
        ...
        >
          ...
          <div className="req-field">
            ...
            <Select
              id="req-urgency"
              value={draft.urgency}
              onChange={(value) => setDraft({ ...draft, urgency: value as Urgency })}
              options={URGENCIES.map((u) => ({ value: u, label: t(`requisitions.urgency.${u}`) }))}
              ariaLabel={t('requisitions.field.urgency')}
            />
          </div>
          <div className="req-field">
            <label htmlFor="req-desc">{t('requisitions.field.description')}</label>
            <input id="req-desc" className="field" value={draft.description} onChange={(e) => setDraft({ ...draft, description: e.target.value })} />
          </div>
          <div className="req-field">
            ...
            <Select
              ...
              onChange={(value) => setDraft({ ...draft, costCenterId: value })}
              options={[
                { value: '', label: t('requisitions.field.none') },
                ...costCenters.map((c) => ({ value: c.id, label: c.label })),
              ]}
              ariaLabel={t('requisitions.field.costCenter')}
            />
          </div>
          ...
          {draft.lines.map((l, i) => (
            <div className="req-line-draft" key={i}>
              <Select
                ariaLabel={`${t('requisitions.field.item')} ${i + 1}`}
                value={l.itemId}
                onChange={(value) => patchLine(i, { itemId: value })}
                options={[
                  { value: '', label: t('requisitions.field.freeText') },
              ...
              />
              <Select
                ariaLabel={`${t('requisitions.field.supplier')} ${i + 1}`}
                value={l.supplierId}
                onChange={(value) => patchLine(i, { supplierId: value })}
                options={[
                  { value: '', label: t('requisitions.field.none') },
      ...
      {detail !== null && (
        <DetailDrawer
          ...
          closeLabel={t('requisitions.action.back')}
          headerExtra={
            <Status kind={STATUS_KIND[detail.status] ?? 'neutral'} label={t(`requisitions.status.${detail.status}`)} />
          }
          provenance={
        ...
        >
          ...
          <table className="req-detail-table">
            <tbody>
              {detail.lines.map((l) => (
                <tr key={l.id}>
                  <td>{l.description}</td>
                  <td className="req-num t-num">{l.qtyMilli / MILLI}</td>
                  <td className="req-num t-num">
                    {t('requisitions.detail.open')} {l.openQtyMilli / MILLI}
                  </td>
                  <td className="req-num t-money">{money(l.estimatedTotalRappen)}</td>
                </tr>
              ))}
            </tbody>
          </table>
            ...
            <ul className="req-timeline">
              {detail.approvalEvents.map((e) => (
                <li key={e.id}>
                  ...
                  {e.comment ? ` - ${e.comment}` : ''} <span className="req-muted">{formatCalendar(e.createdAt)}</span>
                </li>
              ))}
            </ul>
          )}
          ...
          {(s === 'pending_approval' || s === 'rejected') && (
            <div className="req-field">
              ...
              <input id="req-decision" className="field" value={decisionText} onChange={(e) => setDecisionText(e.target.value)} />
            </div>
          )}
            ...
            <div className="req-convert" role="group" aria-label={t('requisitions.convert.title')}>
              ...
              <div className="req-field">
                ...
                <Select
                  ...
                  options={[
                    { value: '', label: t('requisitions.field.choose') },
                    ...vendors.map((v) => ({ value: v.id, label: v.label })),
                  ]}
                  ariaLabel={t('requisitions.field.supplier')}
                />
              </div>
              <div className="req-field">
                ...
                <Select
                  id="req-cv-as"
                  value={convertAs}
                  onChange={(value) => setConvertAs(value as 'draft' | 'sent')}
                  options={[
                    { value: 'draft', label: t('requisitions.field.createDraft') },
                    { value: 'sent', label: t('requisitions.field.createSent') },
                  ]}
                  ariaLabel={t('requisitions.field.createAs')}
                />
              </div>
