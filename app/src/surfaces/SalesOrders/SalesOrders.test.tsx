...
describe('SalesOrders, the list and detail', () => {
  ...
  it('opens the detail and offers Confirm on a draft order', async () => {
    ...
    await user.click(await screen.findByRole('row', { name: 'AU-2026-0001' }));
  ...
  it('Escape closes the drawer and returns focus to the row that opened it', async () => {
    const user = userEvent.setup();
    withCapabilities(baseCanned());
    const trigger = await screen.findByRole('row', { name: 'AU-2026-0001' });
...
describe('SalesOrders, the drawer keeps one solid primary', () => {
  ...
  // so both actions appear in the drawer at once. DESIGN.md allows at most one solid primary per
  // surface: invoicing keeps the solid `.btn--primary`, and the deliver go action inside its
  // disclosed sub-form drops to the accent-tint variant.
  ...
  it('renders exactly one solid primary across the whole drawer, with deliver on the accent variant', async () => {
    const user = userEvent.setup();
    ...
    // The one solid primary is invoicing; the accent-tint go action is the deliver button.
    expect(screen.getByRole('button', { name: de.so.action.invoice })).toHaveClass('btn--primary');
    expect(screen.getByRole('button', { name: de.so.action.deliver })).toHaveClass('btn--accent');
  });
  ...
  it('shows the backorder badge as glyph plus a labelled state on a backordered line', async () => {
    ...
    await user.click(await screen.findByRole('row', { name: 'AU-2026-0001' }));
    await screen.findByRole('button', { name: de.so.action.invoice });
    // The line badge carries the state in words for assistive tech (colour is never the sole carrier).
    const badges = await screen.findAllByLabelText(de.so.badge.backorder);
