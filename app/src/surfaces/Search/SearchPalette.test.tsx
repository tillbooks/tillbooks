...
describe('F-13 J2.6: the palette lands on the open amount', () => {
  ...
  it('keeps only the record row for a contact with nothing open, and when the balance read is refused', async () => {
    // "No open-amount row" only means something once the balance read has ANSWERED. A fixed 50 ms
    // sleep did not guarantee that: under a loaded box the read settled after the assertion (so the
    // assertion checked nothing) and after the test (an act warning that failed the push gate,
    // 23.09.2026). Wait for the read itself, flush its state update inside act, then assert.
    let balanceReads = 0;
    const settled = async () => {
      await waitFor(() => expect(balanceReads).toBeGreaterThan(0));
      await act(async () => {});
    };
    ...
    const empty: Transport = async (action) => {
      ...
      if (action === 'customer_balance') {
        balanceReads += 1;
        return ok({ customerId: 'contact_1', items: [], baseTotalOpenMinor: 0, baseCurrency: 'CHF' });
      }
      return { status: 404, body: { ok: false, error: 'unknown_action' } };
    };
    ...
    const refused: Transport = async (action) => {
      ...
      if (action === 'customer_balance') {
        balanceReads += 1;
        return { status: 403, body: { ok: false, error: 'permission_denied', capability: 'read_sales' } };
      }
      return { status: 404, body: { ok: false, error: 'unknown_action' } };
    };
    render(tree(refused));
...
describe('round 2: the palette is the one omnibox (K-04, K-28, K-42, K-43)', () => {
  it('K-42: the words on the Journal header button find the verb, and it is the default row', async () => {
    render(tree(transportOf({})));
    await openPalette();
    await userEvent.type(screen.getByRole('combobox'), 'Neue Buchung');
    const row = await screen.findByRole('option', { name: /Buchung erfassen/ });
    expect(row).toHaveAttribute('aria-selected', 'true');
  });

  it('K-42: the Übersicht door word "Buchen" finds the same verb', async () => {
    render(tree(transportOf({})));
    await openPalette();
    await userEvent.type(screen.getByRole('combobox'), 'Buchen');
    expect(await screen.findByRole('option', { name: /Buchung erfassen/ })).toBeInTheDocument();
  });

  it('K-42: a typed question preselects the ask row, never a scattered-letter guess', async () => {
    render(tree(transportOf({})));
    await openPalette();
    await userEvent.type(screen.getByRole('combobox'), 'wie viele offene rechnungen habe ich');
    const ask = await screen.findByRole('option', { name: /Frag den Agenten/ });
    await waitFor(() => expect(ask).toHaveAttribute('aria-selected', 'true'));
  });

  it('K-28: the overlay is portaled to the body, so no drawer or dialog can stack over it', async () => {
    render(tree(transportOf({})));
    await openPalette();
    const dialog = await screen.findByRole('dialog');
    expect(dialog.closest('.palette-overlay')?.parentElement).toBe(document.body);
  });

  it('K-43: the input is a plain text combobox (no browser cancel cross) and each row carries its glyph', async () => {
    render(tree(transportOf({})));
    await openPalette();
    expect(screen.getByRole('combobox')).toHaveAttribute('type', 'text');
    const first = (await screen.findAllByRole('option'))[0] as HTMLElement;
    expect(first.querySelector('.palette-row-glyph svg')).not.toBeNull();
  });

  it('K-43: an empty query offers the daily queues', async () => {
    render(tree(transportOf({})));
    await openPalette();
    const nav = await screen.findByRole('group', { name: 'Navigation' });
    const names = [...nav.querySelectorAll('[role="option"]')].map((o) => o.textContent);
    expect(names.some((n) => n?.includes('Aufgaben'))).toBe(true);
    expect(names.some((n) => n?.includes('Suche'))).toBe(false);
  });

  it('K-04: with a query the footer leads to the results page', async () => {
    render(tree(transportOf({ search_global: RESULTS })));
    await openPalette();
    await userEvent.type(screen.getByRole('combobox'), 'Muster');
    await userEvent.click(await screen.findByRole('button', { name: 'Alle Treffer' }));
    expect(screen.getByTestId('location')).toHaveTextContent('/search?q=Muster');
    expect(screen.queryByRole('dialog')).toBeNull();
  });
});
