...
import { ChevronDownGlyph, ChevronRightGlyph } from '../../components/icons';
...
function CreateWorkspaceForm({
   ...
   * face passes `secondary`: its solid primary is already spent on the company save, and the tinted
   * `.btn--accent` is only for the commit of a money write (K-08, D137), which creating a workspace
   * is not (the C2 money critic, F2).
   */
    ...
    <form className={framed ? 'panel setup-panel form-stack' : 'form-stack'} onSubmit={submit} noValidate>
      ...
      <div className="form-row">
        ...
        <Select
          id={legalId}
          value={legalForm}
          onChange={(val) => setLegalFormPick(val)}
          options={LEGAL_FORMS.map((form) => ({ value: form, label: t(`company.legalForm.${form}`) }))}
          ariaLabel={t('company.legalForm.label')}
        />
      ...
      <div className="form-actions">
        ...
        <button type="submit" className={submitVariant === 'primary' ? 'btn btn--primary' : 'btn btn--secondary'} disabled={busy}>
      ...
      <form className="form-stack" onSubmit={onSubmit} noValidate>
        ...
        {offerVat ? (
          <div className="form-row">
            ...
            <Select
              id={ids.vat}
              value={vatChoice}
              onChange={(val) => setVatChoice(val as (typeof VAT_CHOICES)[number])}
              options={VAT_CHOICES.map((choice) => ({ value: choice, label: t(`company.vatChoice.${choice}`) }))}
              ariaLabel={t('company.vatMethod.label')}
            />
            {vatChoice !== 'none' && (
              <div className="setup-vat-timing">
                ...
                <Select
                  id={ids.vatTiming}
                  value={vatTiming}
                  onChange={(val) => setVatTiming(val as 'soll' | 'ist')}
                  options={[
                    { value: 'soll', label: t('company.vatTiming.soll') },
                    { value: 'ist', label: t('company.vatTiming.ist') },
                  ]}
                  ariaLabel={t('company.vatTiming.label')}
                />
              </div>
            )}
          ...
          <div className="form-row">
            <span className="field-label-row">
              ...
              <Link to="/vat" className="setup-inline-link link-inline">
        ...
        <section className="setup-disclosure" aria-labelledby={ids.addressRegion}>
          <h3 className="setup-disclosure-title">
            ...
            >
              <DisclosureChevron open={addressOpen} />
            ...
            <div className="form-stack">
              ...
              <fieldset className="setup-address-group" aria-invalid={errorFor('address') !== undefined || undefined}>
                ...
                <div className="form-row">
                  ...
                  <input className="field" id={ids.street} value={street} onChange={(e) => setStreet(e.target.value)} />
                </div>
                <div className="form-row">
                  <label htmlFor={ids.buildingNo}>{t('company.address.buildingNo')}</label>
                  <input className="field" id={ids.buildingNo} value={buildingNo} onChange={(e) => setBuildingNo(e.target.value)} />
                </div>
                <div className="form-row">
                  <label htmlFor={ids.zip}>{t('company.address.zip')}</label>
                  <input className="field" id={ids.zip} value={zip} onChange={(e) => setZip(e.target.value)} />
                </div>
                <div className="form-row">
                  <label htmlFor={ids.town}>{t('company.address.town')}</label>
                  <input className="field" id={ids.town} value={town} onChange={(e) => setTown(e.target.value)} />
                </div>
                <div className="form-row">
                  <label htmlFor={ids.country}>{t('company.address.country')}</label>
                  <input className="field" id={ids.country} value={country} onChange={(e) => setCountry(e.target.value)} />
                </div>
        ...
        <section className="setup-disclosure" aria-labelledby={ids.moreRegion}>
          <h3 className="setup-disclosure-title">
            ...
            >
              <DisclosureChevron open={moreOpen} />
              ...
              <div className="form-row">
                ...
                <div className="setup-fy-start">
                  <Select
                    id={ids.fyStart}
                    value={pad2(fyMonth)}
                    onChange={(val) => setMonthDay(Number(val), fyDay)}
                    options={monthNames(locale).map((label, i) => ({ value: pad2(i + 1), label }))}
                    disabled={locked}
                    describedBy={lockNoteId}
                    ariaLabel={t('company.fiscalYearStartMonth')}
                  />
                  <Select
                    value={pad2(fyDay)}
                    onChange={(val) => setMonthDay(fyMonth, Number(val))}
                    options={Array.from({ length: DAYS_IN_MONTH[fyMonth - 1] }, (_, i) => ({
                      value: pad2(i + 1),
                      label: String(i + 1),
                    }))}
                    disabled={locked}
                    describedBy={lockNoteId}
                    ariaLabel={t('company.fiscalYearStartDay')}
                  />
                </div>
              </div>
              {locked && (
  ...
  return (
    <Panel title={t('ebillConfig.title')}>
      ...
      <p className="setup-explainer">
        <a className="link-inline" href="https://www.ebill.ch/en/home/business/network-partners-at-a-glance.html" target="_blank" rel="noopener noreferrer">
...
/**
 * The disclosure's drawn chevron (K-22: no text triangles), pointing right while closed and down
 * while open. Decorative: the toggle's aria-expanded carries the state.
 */
function DisclosureChevron({ open }: { open: boolean }) {
  return open ? (
    <ChevronDownGlyph className="setup-disclosure-chevron" size={16} aria-hidden="true" />
  ) : (
    <ChevronRightGlyph className="setup-disclosure-chevron" size={16} aria-hidden="true" />
  );
}

/**
 * Read-only MWST method + timing, reflecting stored values (editing lives in the VAT surface). Plain
 * dim facts, never pills (K-22); `.setup-badge` stays only as the hook the VAT settings flow reads.
 */
      ...
      {createOpen && (
        <div ref={formHostRef}>
          <section className="panel setup-panel" aria-labelledby={headingId}>
            ...
            <CreateWorkspaceForm framed={false} submitVariant="secondary" seat="onboard_client" onCancel={() => setCreateOpen(false)} />
          </section>
        </div>
      )}
    </>
