...
describe('Tasks, the gates (real CapabilitiesProvider, real whoami)', () => {
  it('hides create/edit/snooze/cancel without tasks.write, and disables the check for a non-assignee', async () => {
    ...
    // Without tasks.write the title is plain text and there is no row overflow (snooze, cancel).
    expect(screen.queryByRole('button', { name: `${de.tasks.action.edit}: Offerte nachfassen` })).not.toBeInTheDocument();
    expect(
      screen.queryByRole('button', { name: de.tasks.rowActions.replace('{title}', 'Offerte nachfassen') }),
...
describe('Tasks, the writes', () => {
  ...
  it('opens the edit drawer seeded from the row, with no recurrence field', async () => {
    ...
    await screen.findByText('Offerte nachfassen');
    // K-12: the title is the row's opener.
    await userEvent.click(screen.getByRole('button', { name: `${de.tasks.action.edit}: Offerte nachfassen` }));
  ...
  it('renders the engine refusal for a bad snooze where it was attempted', async () => {
    ...
    renderTasks(canned);
    await screen.findByText('Offerte nachfassen');
    // K-21: Zurückstellen sits in the row's one overflow.
    await userEvent.click(screen.getByRole('button', { name: de.tasks.rowActions.replace('{title}', 'Offerte nachfassen') }));
    await userEvent.click(await screen.findByRole('menuitem', { name: de.tasks.action.snooze }));
