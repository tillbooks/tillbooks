 ...
 * at query time, never cached), and the surface only groups. The title opens the task, Erledigen is the one visible row verb, and
 * Zurückstellen and Abbrechen sit behind the row overflow (K-21, D137); every one is a real button in
 * a sensible tab order, and status is the shared Status, glyph AND word (WCAG 2.2 AA).
 *
 * THE PERMISSION GATES HERE ARE A CONVENIENCE AND NOT THE ENFORCEMENT (the standing Studio rule):
 * `whoami` is the one source, it fails open, and the engine is the real gate. Erledigen stays enabled
 ...
 * bucket sections keep their own `<h2>`-headed `<ul>` of elevated row panels. Erledigen and the one-field
 * snooze form stay inline as queue quick-actions, where the work finishes. No `Provenance`/`ConsequenceLine`: a task has no engine consequence sentence and no
...
/** K-22: the task state as the shared Status, a drawn glyph plus the word, never a dingbat. */
const STATUS_KIND: Record<string, StatusKind> = { open: 'neutral', doing: 'pending', done: 'success', cancelled: 'inactive' };
...
export function Tasks() {
  ...
  const editorFields = (value: Draft, onChange: (next: Draft) => void, idPrefix: string) => (
    <div className="tasks-editor-fields">
      ...
      <label className="tasks-field">
        ...
        <input className="field" id={`${idPrefix}-due`} type="date" value={value.dueAt} onChange={(e) => onChange({ ...value, dueAt: e.target.value })} />
      </label>
      <label className="tasks-field">
  ...
  const section = (bucket: (typeof BUCKETS)[number]) => {
    ...
    return (
      <section key={bucket} className={`tasks-section tasks-section--${bucket}`} aria-labelledby={`tasks-h-${bucket}`}>
        ...
        {rows.length === 0 ? (
          // K-33: each bucket says what is not there, never a bare "Nichts hier."
          <p className="tasks-section-empty">{t(`tasks.section.empty.${bucket}`)}</p>
        ) : (
              ...
              return (
                <li key={task.id} className="tasks-row">
                  <div className="tasks-row-main">
                    <Status
                      kind={STATUS_KIND[task.status] ?? 'neutral'}
                      label={t(`tasks.status.${task.status}`)}
                      className="tasks-status"
                    />
                    {/* K-12: the title opens the task (ink 500); with no write right it is plain text. */}
                    {canWrite && live ? (
                      <button
                        type="button"
                        className="tasks-row-title tasks-row-open"
                        aria-label={`${t('tasks.action.edit')}: ${task.title}`}
                    ...
                    {/* K-22: a quiet word, never a pill with a cycle dingbat. */}
                    {task.recurrenceRule !== null && <span className="tasks-note">{t('tasks.recurring.badge')}</span>}
                    <span className="tasks-meta">
                      ...
                      {/* K-38: the seat by name, never the raw user id. */}
                      <span>{t('tasks.field.assignee')}: {seatName(task.assigneeUserId).label}</span>
                    </span>
                  </div>
                  {live && (
                    <div className="tasks-row-actions">
                      {/* K-21: completing is the row's one visible verb; the rest sit behind ONE
                          overflow, Abbrechen last as danger. */}
                      <button
                        type="button"
                      ...
                      {canWrite && (
                        <OverflowMenu
                          quiet
                          label={t('tasks.rowActions', { title: task.title })}
                          items={[
                            ...(task.reminderAt !== null
                              ? [
                                  {
                                    key: 'snooze',
                                    label: t('tasks.action.snooze'),
                                    onSelect: () => {
                              ...
                              : []),
                            {
                              ...
                              danger: true,
                              onSelect: () => void write('tasks_cancel', { taskId: task.id, idempotencyKey: newKey() }),
                            },
                          ]}
                        />
                      )}
                  ...
                  {snoozingId === task.id && (
                    ...
                    >
                      <label className="tasks-field">
                        ...
                        <input className="field" type="datetime-local" value={snoozeUntil} onChange={(e) => setSnoozeUntil(e.target.value)} required />
                      </label>
  ...
  const headerActions = (
    <>
      {views.length > 0 && (
        <div className="tasks-view-picker">
          <span>{t('tasks.view.label')}</span>
          <Select
            value={viewId}
            onChange={(val) => setViewId(val)}
            options={[
              { value: '', label: t('tasks.view.all') },
  ...
  return (
    <section className="tasks" aria-labelledby="tasks-title">
      ...
      {failed && <ErrorBanner context="read" message={t('tasks.error.transport')} onRetry={() => void load()} />}

      {loading ? (
        <Skeleton rows={4} height={56} labelKey="tasks.loading" />
