...
export function ThreeWayMatch() {
  ...
  const columns: DataTableColumn<Exception>[] = [
    { key: 'supplier', header: t('procurement.match.col.supplier'), render: supplierName },
    {
      ...
      render: (r) => <span className={`twm-badge twm-badge--${r.status}`}>{t(`procurement.match.status.${r.status}`)}</span>,
    },
  ...
  return (
    <div className="twm-surface">
      ...
      <DataTable
        ...
        rowClassName={(r) => (r.billId === selected ? 'twm-row--selected' : undefined)}
      ...
      {selectedRow !== null && (
        <DetailDrawer
          ...
          headerExtra={
            <span className={`twm-badge twm-badge--${selectedRow.status}`}>{t(`procurement.match.status.${selectedRow.status}`)}</span>
          }
