 ...
 *
 * Round 2 (D137): every state (the evaluation, each line, the outcome, a refusal) is a Status word
 * instead of a dingbat badge or a coloured sentence (K-22); the evaluation loads behind the shared
 * skeleton (K-34); the override reason is the shared `.field` (K-15); the three totals are
 * `.t-money` (tabular, never wrapped). Every figure and its `chf()` formatting are untouched.
 */
...
/**
 * A match state as the one `Status` word (K-22): glyph AND label, never colour alone. Shared by the
 * exception list, the drawer header, the evaluation and each comparison line.
 */
export const MATCH_STATUS_KIND: Record<string, StatusKind> = {
  matched: 'success',
  partial: 'warn',
  overridden: 'warn',
  variance: 'danger',
  nothing_received: 'pending',
  no_candidate_po: 'neutral',
};
...
export function ThreeWayMatchPanel({ billId, onMatched, onDialogOpenChange }: ThreeWayMatchPanelProps) {
  ...
  // Text left, quantities right (numeric, `.t-num`); status the Status word. Nothing is
  ...
  const lineColumns: DataTableColumn<EvalLine>[] = [
    ...
    { key: 'qtyVar', header: t('procurement.match.col.qtyVar'), numeric: true, render: (l) => l.qtyVariance },
    {
      ...
      render: (l) => (
        <Status kind={MATCH_STATUS_KIND[l.lineStatus] ?? 'neutral'} label={t(`procurement.match.status.${l.lineStatus}`)} />
      ),
    },
  ];
  ...
  return (
    <section className="twm-panel" aria-label={t('procurement.match.title')}>
      ...
      <p className="twm-state" role="status">
        <Status kind={MATCH_STATUS_KIND[status] ?? 'neutral'} label={t(`procurement.match.status.${status}`)} />
      </p>

      {done !== null ? (
        <p className="twm-state" role="status">
          <Status kind="success" label={t(`procurement.match.done.${done}`)} />
        </p>
      ) : (
        <>
          <DataTable
          ...
          <dl className="twm-totals">
            <div>
              ...
              <dd className="t-money">{chf(evaluation.totalExpectedRappen)}</dd>
            </div>
            <div>
              <dt>{t('procurement.match.billed')}</dt>
              <dd className="t-money">{chf(evaluation.totalBilledRappen)}</dd>
            </div>
            <div>
              <dt>{t('procurement.match.valueVar')}</dt>
              <dd className="t-money">{chf(evaluation.valueVarianceRappen)}</dd>
            </div>
          </dl>
      ...
      >
        ...
        {intent === 'override' && (
          <label className="twm-field">
            ...
            <input type="text" className="field" value={reason} onChange={(e) => setReason(e.target.value)} minLength={5} />
          </label>
        )}
        {message !== '' && (
          <p className="twm-state" role="alert">
            <Status kind="danger" label={message} />
          </p>
        )}
      </Modal>
    </section>
