...
describe('B04 Mandate tab', () => {
  ...
  it('generates an invoice for the chosen period', async () => {
    ...
    await user.click(await screen.findByRole('button', { name: de.retainer.action.generate }));
    const month = screen.getByLabelText(de.retainer.field.genPeriod);
    await user.type(month, '2026-06');
    // The submit button inside the generate form (there are two matching the label; the form's submit).
    const buttons = screen.getAllByRole('button', { name: de.retainer.action.generate });
