...
export function Unbilled() {
  ...
  return (
    <section className="time-billing" aria-labelledby="billing-title">
      ...
      {failed && <ErrorBanner message={t('billing.error.transport')} onRetry={() => void load()} />}

      {loading ? (
        <div role="status" aria-busy="true" aria-live="polite">
          <span className="time-sr">{t('billing.loading')}</span>
          <Skeleton rows={3} />
        </div>
      ) : groups.length === 0 ? (
        ...
        <>
          <table className="time-billing-tree">
            <thead>
              <tr>
                <th scope="col">
                  <span className="time-sr">{t('billing.col.select')}</span>
                </th>
          ...
          <div className="time-billing-footer" role="status">
            <span>
              {t('billing.total')}: <b>{formatMoney(totalRappen, groups[0]?.currency ?? 'CHF')}</b>
            </span>
            {selected.size > 0 && (
              <span>
                {t('billing.selected')}: <b>{formatMoney(selectedTotal, groups.find((g) => g.contactId === activeContact)?.currency ?? 'CHF')}</b>
              </span>
            )}
            {canGenerate && (
              <button
          ...
          {mixedSelection && (
            <p className="time-hint" role="note">
              <span aria-hidden="true">⚠</span> {t('billing.error.mixed_contacts')}
            </p>
          )}
        </>
      )}
      ...
      <div className="time-billing-wip" aria-labelledby="billing-wip-title">
        ...
        {loading ? (
          ...
          <div className="time-billing-wip-card" role="group" aria-label={t('billing.wip.title')}>
            <div className="time-billing-wip-value">{formatMoney(wipTotal, wipCurrency)}</div>
        ...
        p.phases.map((ph) =>
          ph.entries.map((e) => (
            <tr key={e.id}>
              ...
              <td className="time-num">{formatMoney(e.valueRappen, contact.currency ?? 'CHF')}</td>
            </tr>
          )),
        ),
      )}
