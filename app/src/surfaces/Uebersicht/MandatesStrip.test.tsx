 ...
 * fewer than three workspaces render no strip at all (K-03: two mandates read their counts in the rail
 * switcher's menu); the counts are the engine's totals, never a fake zero; and beyond the eager cap
 * the tail reads only on demand.
 */
...
import { EAGER_CAP, MIN_MANDATES, MandatesStrip } from './MandatesStrip';
...
describe('F-13 J2.5: the mandates strip', () => {
  ...
  it('renders NOTHING for two workspaces: the rail switcher carries their counts (K-03)', async () => {
    expect(MIN_MANDATES).toBe(3);
    const { transport, read } = transportFor([WS(1), WS(2)], { ws_1: 0, ws_2: 5 });
    const { container } = renderStrip(transport);
    await waitFor(() => expect(container.querySelector('output')).toBeInTheDocument());
    await new Promise((r) => setTimeout(r, 20));
    expect(container.querySelector('.mandates')).toBeNull();
    // No strip, so no count reads either.
    expect(read).toEqual([]);
  });
  ...
  it('one click switches the workspace to the mandate that needs work', async () => {
    const { transport } = transportFor([WS(1), WS(2), WS(3)], { ws_1: 0, ws_2: 5, ws_3: 0 });
    renderStrip(transport);
    const nav = await screen.findByRole('navigation', { name: 'Mandate' });
  ...
  it('a failed count is a muted retry on that row, never a zero', async () => {
    const totals: Record<string, number | null | 'fail'> = { ws_1: 1, ws_2: 'fail', ws_3: 0 };
    const { transport } = transportFor([WS(1), WS(2), WS(3)], totals);
    renderStrip(transport);
    const nav = await screen.findByRole('navigation', { name: 'Mandate' });
  ...
  it('has no axe violations', async () => {
    const { transport } = transportFor([WS(1), WS(2), WS(3)], { ws_1: 2, ws_2: 0, ws_3: 1 });
    const { container } = renderStrip(transport);
