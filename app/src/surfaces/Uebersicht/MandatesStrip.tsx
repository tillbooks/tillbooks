 ...
 * only from three mandates (K-03, D137): a solo owner never sees a one-row list of themselves, and two
 * mandates read their counts in the rail switcher's menu instead of a second switcher here.
...
/**
 * The strip appears from THREE mandates (K-03, D137). With two, it repeated the rail switcher 90px
 * below it (the same two names, only with counts); the counts now ride the switcher's menu rows, and
 * the strip is kept for the Treuhänder whose list is longer than a menu comfortably shows.
 */
export const MIN_MANDATES = 3;
...
export function MandatesStrip() {
  ...
  useEffect(() => {
    if (mandates === null || mandates.length < MIN_MANDATES) return;
  ...
  if (mandates === null || mandates.length < MIN_MANDATES) return null;
