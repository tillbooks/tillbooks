...
describe('SetupCard: the first-hour setup guide', () => {
  ...
  it('F-09: steps the profile already answers are marked done, in words, and stay links', async () => {
    ...
    // The link renders before the profile read resolves; the done state arrives with that read, so
    // wait for it rather than assert on the first paint (a loaded box made the race visible).
    await waitFor(() => expect(profileStep).toHaveAccessibleDescription('erledigt'));
    ...
    // The card writes its done-state back unconditionally after the profile read; flush that update
    // into act rather than letting it escape past the test (matches the sibling tests above).
    await act(async () => {});
  });
