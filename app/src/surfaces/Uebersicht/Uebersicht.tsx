...
import { EmptyState, ErrorBanner, NoWorkspaceState, Skeleton, useSkeletonHold } from '../../components/states';
...
/**
 * The dashboard-layout saved views `list_saved_views` returned, read defensively into the SHARED
 * picker's DTO (the same `SavedViewDto` Customization and Recurring feed their picker). A view of
 * another layout is not this surface's, and an unreadable row is dropped rather than trusted.
 */
function parseViews(body: unknown): SavedViewDto[] {
  ...
  return views.filter(
    (v): v is SavedViewDto =>
      ...
      (v as { layout?: unknown }).layout === 'dashboard',
  );
}
...
export function Uebersicht() {
  ...
  // null = the built-in default view; a string is a chosen saved view's id, passed to the read.
  ...
  const load = useCallback(async () => {
    ...
    const [wall, savedViews] = await Promise.all([
      client.call('dashboard_overview', {
        ...
        from: range.from,
        to: range.to,
        ...(viewId === null ? {} : { savedViewId: viewId }),
      }),
  ...
  // K-34: nothing before 200ms, and a skeleton that appeared stays at least 300ms.
  ...
  return (
    <section className="uebersicht" aria-labelledby="uebersicht-title">
      ...
      />
      {/* K-13: on a fresh, data-less workspace the setup steps are the point of the page, so they
          lead, directly under the header and ABOVE the range controls (which act on an all-zero wall
          that does not yet matter). The card renders nothing once the books carry real data, so an
          established workspace sees the controls at the top as before. */}
      {!loading && !failed && overview !== null ? <SetupCard tiles={overview.tiles} /> : null}
      <div className="uebersicht-controls">
        {/* K-11/K-12 (D137): the range is the shared Segmented control, four sibling values of one
            view. Its chosen segment is a neutral raise, so "Monat" and the "Buchen" primary are no
            longer two solid brass blocks side by side. */}
        <Segmented
          className="uebersicht-range"
          label={t('dashboard.range.label')}
          options={(['month', 'quarter', 'year', 'custom'] as const).map((p) => ({ value: p, label: t(`dashboard.range.${p}`) }))}
          value={preset}
          onChange={setPreset}
        />
        {preset === 'custom' ? (
        ...
        {/* The SHARED saved-view picker (the same one Customization and Recurring mount), not a
            hand-rolled select. It is shown only once a dashboard view has been saved: with none (the
            common case) a lone "Standardansicht" option is noise, so the control is hidden and the
            wall already shows that default. Its width is capped in CSS so a long name never stretches
            the control row into the range toggle. */}
        {views.length > 0 ? (
          <SavedViewPicker
            views={views}
            selected={viewId}
            label={t('dashboard.view.label')}
            defaultOptionLabel={t('dashboard.view.default')}
            personalGroupLabel={t('dashboard.view.personalGroup')}
            sharedGroupLabel={t('dashboard.view.sharedGroup')}
            onSelect={setViewId}
          />
        ) : null}
        <button type="button" className="uebersicht-save btn btn--secondary" onClick={() => setSaving((s) => !s)}>
      ...
      {saving ? (
        <div className="uebersicht-save-form">
          <label>
            ...
            <input type="text" className="field" value={saveName} onChange={(e) => setSaveName(e.target.value)} maxLength={120} />
          </label>
          ...
          {/* K-08: saving a view is not a money write, so it is a secondary, never the tinted accent. */}
          <button type="button" className="btn btn--secondary" onClick={() => void saveView()} disabled={saveName.trim() === ''}>
      ...
      {showSkeleton ? (
        // K-34: eight tiles in the wall's own 4 x 2 shape, not four full-width bars that grow later.
        <div className="uebersicht-grid-skeleton">
          <Skeleton rows={8} columns={4} height={112} />
        </div>
      ) : failed ? (
        // K-35: a failed READ says so (no "prüfe die Eingaben" on a page load) and can be reported.
        <ErrorBanner context="read" message={t('dashboard.error.read')} onRetry={() => void load()} />
      ) : overview === null ? null : shown.length === 0 ? (
        // K-33: the first booking is what fills the wall, so the empty state offers it.
        <EmptyState
          title={t('dashboard.empty.title')}
          hint={t('dashboard.empty.hint')}
          action={{ label: t('command.post_entry.label'), to: '/journal?new=1' }}
        />
      ) : (
        <ul className="uebersicht-grid">
          {shown.map((tile) => {
            ...
            const isMoney = typeof tile.valueRappen === 'number';
            const value =
            ...
            const body = (
              <>
                ...
                {/* A money figure is a money cell: tabular and never wrapped (K-18, the core's .t-money). */}
                <span className={isMoney ? 'uebersicht-value t-money' : 'uebersicht-value'}>{value}</span>
                {trend !== null ? (
            ...
            return (
              <li key={tile.tile}>
                {/* A tile that drills is a real control: the Wave 1 `.panel--interactive` treatment
                    gives it a pointer, a resting control shadow and a hover-lift, so the owner reads
                    it as clickable before the pointer arrives. A tile with no drill is inert, so it
                    renders as a plain (non-focusable, non-lifting) card and never looks clickable. */}
                {drill !== undefined ? (
                  <button
                    type="button"
                    className="uebersicht-tile panel--interactive"
                    onClick={() => navigate(drill.studioRoute)}
                  >
                    {body}
                  </button>
                ) : (
                  <div className="uebersicht-tile uebersicht-tile--inert">{body}</div>
                )}
              </li>
            );
          })}
