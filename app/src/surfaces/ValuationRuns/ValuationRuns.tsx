...
/** A run's and an account's state as the one `Status` word (K-22), never a bordered badge. */
const RUN_KIND: Record<string, StatusKind> = { draft: 'neutral', posted: 'success', reversed: 'inactive' };
const RECON_KIND: Record<string, StatusKind> = { balanced: 'success', drift: 'danger', unposted: 'neutral' };
...
export function ValuationRuns() {
  ...
  const statusBadge = (status: string) => <Status kind={RUN_KIND[status] ?? 'neutral'} label={t(`invRun.status.${status}`)} />;
  ...
  const runColumns = useMemo<DataTableColumn<RunSummary>[]>(
    () => [
      { key: 'asOf', header: t('invRun.col.asOf'), render: (r) => formatCalendar(r.asOf) },
  ...
  // K-21: Stornieren, a destructive verb, sits behind a posted run's overflow, never a row button.
  const runActions = (r: RunSummary) =>
    r.status === 'posted' && canPost
      ? [{ key: 'reverse', label: t('invRun.reverse'), danger: true, disabled: busy, onSelect: () => void reverseRun(r.id) }]
      : [];
  ...
  const reconColumns = useMemo<DataTableColumn<ReconAccount>[]>(
    () => [
      ...
      { key: 'delta', header: t('invRun.col.delta'), numeric: true, render: (a) => formatMoney(a.deltaRappen, 'CHF') },
      {
        ...
        render: (a) => <Status kind={RECON_KIND[a.status] ?? 'neutral'} label={t(`invRun.reconStatus.${a.status}`)} />,
      },
    ],
    // eslint-disable-next-line react-hooks/exhaustive-deps
    [t],
      ...
      {loading ? (
        ...
        <div data-testid="runs-table">
          <DataTable
            ...
            rowActions={canPost ? runActions : undefined}
            rowActionsLabel={(r) => t('invRun.rowActions', { asOf: formatCalendar(r.asOf) })}
  ...
  const reportPane = (
    <div className="valrun-pane">
      <div className="valrun-toolbar">
        ...
        <button type="button" className="btn btn--secondary" disabled={busy} onClick={() => void runReport()} data-testid="run-report">
          {t('invRun.compute')}
        </button>
      </div>
      {report === null ? (
  ...
  const reconPane = (
    <div className="valrun-pane">
      <div className="valrun-toolbar">
        ...
        <button type="button" className="btn btn--secondary" disabled={busy} onClick={() => void runRecon()} data-testid="run-recon">
          {t('invRun.runCheck')}
        </button>
      </div>
      {recon === null ? (
        ...
        <div data-testid="recon-result">
          <p className="valrun-total">
            <span data-testid="recon-status">
              <Status kind={RECON_KIND[recon.status] ?? 'neutral'} label={t(`invRun.reconStatus.${recon.status}`)} />
            </span>
          </p>
          <DataTable
    ...
    <section className="valrun" aria-labelledby="valrun-title">
      ...
      <DetailDrawer
        ...
        footer={
          <>
            ...
            {draft !== null ? (
              // The commit of a money write, followed by the confirm: the one role of the tint (K-08).
              <button type="button" className="btn btn--accent" data-money-commit="inventory_valuation_post" disabled={busy} onClick={() => setConfirmPost(true)} data-testid="post-run">
                {t('invRun.post')}
              </button>
            ) : null}
          </>
      ...
      >
        <div className="valrun-toolbar">
          ...
          <button type="button" className="btn btn--secondary" disabled={busy} onClick={() => void calculate()} data-testid="calculate">
            {t('invRun.calculate')}
          </button>
        </div>
        {draft !== null ? (
          <div className="valrun-draft" data-testid="draft-review">
            <p className="valrun-total">
              {t('invRun.proposedTotal')} <strong className="t-money">{formatMoney(draft.run.totalValueRappen, 'CHF')}</strong>
            </p>
            <p className="valrun-total">
              {t('invRun.proposedDelta')} <strong className="t-money">{formatMoney(draft.run.deltaRappen, 'CHF')}</strong>
            </p>
      ...
      {draft !== null ? (
        <Modal
          ...
          footer={
            <>
              <button type="button" className="btn btn--secondary" onClick={() => setConfirmPost(false)}>
                {t('invRun.cancel')}
              </button>
              <button type="button" className="btn btn--accent" data-money-commit="inventory_valuation_post" disabled={busy} onClick={() => void postDraft()} data-testid="confirm-post-run">
                {t('invRun.post')}
              </button>
            </>
          }
