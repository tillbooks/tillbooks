...
describe('TaxCodePicker, the five states', () => {
  ...
  it('error: an archived/unknown selected code flags inline via aria-describedby, and stays selectable', () => {
    ...
    // The archived code survives as an option so the draft still shows what it carried.
    expect(screen.getByRole('option', { name: 'OLD77' })).toBeInTheDocument();
  });

  it('success: options group by kind and fire onChange with the chosen code', async () => {
    ...
    expect(screen.getByRole('option', { name: 'Umsatzsteuer 8.1% (Normalsatz) · UST81' })).toBeInTheDocument();
    await userEvent.selectOptions(screen.getByRole('combobox', { name: 'Tax code 1' }), 'UST81');
