 ...
 * it and click the option that carries the code.
 */
...
describe('DocumentEditor hands the VAT controls the draft currency (A11-G2)', () => {
  it('labels the line readout and the summary in the currency the picker chose', async () => {
    ...
    // Kunde, MWST and Währung are all the shared <Select> (a combobox; MWST since K-30, Währung since
    // K-15): open it and click the option. The currency option is picked by its value, as
    // CurrencyPicker.test.tsx does.
    await userEvent.click(await screen.findByRole('combobox', { name: 'Kunde' }));
    await userEvent.click(await screen.findByRole('option', { name: 'Muster AG' }));
    await userEvent.click(screen.getByRole('combobox', { name: 'Währung' }));
    const currency = screen.getAllByRole('option').find((o) => o.getAttribute('data-value') === eur.currency);
    if (currency === undefined) throw new Error(`the currency list offers no ${eur.currency}`);
