    ...
    <>
        ...
        {steps.map((step) => {
          ...
          return (
            <li key={step} className={isDone ? 'vr-step vr-step--done' : 'vr-step'} data-attested={step === '4' && attestedAt !== null ? 'true' : undefined}>
              ...
              {step === '4' ? (
                <a className="vr-step-link link-inline" href={portalUrl} target="_blank" rel="noreferrer noopener">
                  {label}
      ...
      {run !== null && (
        <p className="vr-journey-run">
          <Link className="link-inline" to={`/checklisten?run=${encodeURIComponent(run.runId)}`}>{t('vat.return.journey.openRun')}</Link>
        </p>
      )}
    </>
  );
