      ...
      <section className="vr-settle panel" aria-labelledby={`${bodyId}-title`}>
        ...
        <p className="vr-settle-note" role="note">
          ...
          {state.code === 'missing_account' && (
            <>
              {' '}
              <Link className="link-inline" to="/accounts">{t('vat.settlement.openChart')}</Link>
            </>
          )}
          {state.code === 'saldo_input_vat_booked' && (
            <>
              {' '}
              <Link className="link-inline" to="/journal">{t('vat.settlement.openJournal')}</Link>
            </>
          )}
        </p>
      </section>
          ...
          <tbody>
            {rows.map((r) => (
              <tr key={r.key} data-row={r.key}>
                ...
                <td className="num t-money">{formatMoney(r.booked, currency)}</td>
                <td className="num t-money">{formatMoney(r.declared, currency)}</td>
                <td className="num t-money">{r.diff === 0 ? t('vat.settlement.noDifference') : formatMoney(r.diff, currency)}</td>
              </tr>
            ))}
          </tbody>
        </table>
          ...
          <ul className="vr-settle-lines-list">
            {model.lines.map((l, i) => (
              <li key={`${l.accountNumber}-${i}`}>
                ...
                <span className="num t-money">{formatMoney(l.debitMinor > 0 ? l.debitMinor : l.creditMinor, currency)}</span>
              </li>
            ))}
          </ul>
        </details>
        ...
        <Modal
          ...
          footer={
            <>
              ...
              <button
                type="button"
                className={confirming === 'post' ? 'btn btn--accent' : 'btn btn--danger'}
                data-money-commit={confirming === 'post' ? 'vat_settlement_post' : undefined}
                disabled={pending}
        ...
        >
          ...
          <dl className="vr-dialog-facts">
            ...
            <div>
              ...
              <dd className="t-money">{formatMoney(model.netMinor, currency)}</dd>
            </div>
            {hasDifference && (
              <div>
                <dt>{t('vat.settlement.col.difference')}</dt>
                <dd className="t-money">{formatMoney(model.differences.netMinor, currency)}</dd>
              </div>
            )}
          </dl>
          {writeError !== null && (
            <p className="vr-dialog-error" role="alert">
              ...
              {writeError.code === 'period_locked' && writeError.reason === 'year_close' && (
                <>
                  {' '}
                  <Link className="link-inline" to="/periods">{t('vat.settlement.openPeriods')}</Link>
                </>
              )}
            </p>
          )}
