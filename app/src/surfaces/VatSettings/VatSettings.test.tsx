...
describe('VatSettings, five states', () => {
  ...
  it('has no axe violations on the configured success render', async () => {
    ...
    await screen.findByText("Dein steuerbarer Umsatz 2025: CHF 312'400.00.");
...
describe('VatSettings, method switch effektiv <-> saldo', () => {
  ...
  it('shows every existing saldo row on the SAME ESTV Ziffer, because the form has one', async () => {
    ...
    expect(screen.getByLabelText('Saldosteuersatz 1')).toHaveValue('620');
    expect(screen.getByLabelText('Saldosteuersatz 2')).toHaveValue('370');
  });
});
...
describe('VatSettings, N-rate saldo editing', () => {
  ...
  it('blocks a duplicate rate client-side and disables Save', async () => {
    ...
    await userEvent.selectOptions(screen.getByLabelText('Saldosteuersatz 2'), '620');
  ...
  it("a duplicate rate must not collide the Tätigkeit picker's option keys", async () => {
    ...
    const picker = (row as HTMLElement).querySelector('select');
    expect(picker).not.toBeNull();
    const offered = [...(picker as HTMLSelectElement).options].map((o) => o.value).filter((v) => v !== '');
...
describe('VatSettings, tax-code register', () => {
  it('lists each code with its ESTV Ziffer and status', async () => {
    ...
    expect(within(ust77Row).getByRole('button', { name: 'Code UST77 reaktivieren' })).toBeInTheDocument();
    expect(within(ust77Row).queryByRole('button', { name: 'Code UST77 archivieren' })).not.toBeInTheDocument();
  });
  ...
  it('archives an active code via vat_code_deactivate after a confirm (never deletes)', async () => {
    ...
    await userEvent.click(screen.getByRole('button', { name: 'Code UST81 archivieren' }));
  ...
  it('reactivates an archived code via vat_code_reactivate with no confirm (the restorative mirror)', async () => {
    ...
    await userEvent.click(screen.getByRole('button', { name: 'Code UST77 reaktivieren' }));
