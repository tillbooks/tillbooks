  ...
  return (
    <span className="vat-eligibility">
      <span className="vat-eligibility-limits">
        {t('vat.eligibility.limits', {
          turnoverLimit: formatMoney(limits.turnoverLimitMinor, 'CHF'),
          taxLimit: formatMoney(limits.taxDueLimitMinor, 'CHF'),
        })}
      </span>
      ...
      {state.kind === 'measured' && (
        <span className="vat-eligibility-figure">
          {t('vat.eligibility.measured', { year: state.year, amount: formatMoney(state.turnoverMinor, 'CHF') })}
        </span>
      )}
          ...
          {rates.map((rateBp, index) => {
            ...
            return (
              <li key={index} className="vat-saldo-row">
                <select
                  className="vat-input"
                  aria-label={t('vat.saldo.position', { n: position })}
                  value={rateBp === 0 ? '' : String(rateBp)}
                  onChange={(e) => updateRate(index, e.target.value === '' ? 0 : Number(e.target.value))}
                >
                  <option value="">{t('vat.saldo.choose')}</option>
                  {ESTV_SALDO_LADDER.map((bp) => (
...
function ActivityEditor({
  ...
  return (
    <section className="vat-activities" aria-labelledby="vat-activities-title">
              ...
              <li key={a.activityId} className="vat-activity-row">
                <div className="vat-activity-head">
                  ...
                  <select
                    ...
                    onChange={(e) => patch(index, { rateBp: Number(e.target.value) })}
                    aria-label={t('vat.activity.rateLabel', { name: a.name || String(index + 1) })}
                    aria-invalid={orphan || undefined}
                  >
                    <option value="">{t('vat.saldo.choose')}</option>
                    {approvedRates.map((bp) => (
      ...
      <button type="button" className="btn btn--quiet" onClick={add} disabled={approvedRates.length === 0}>
        {t('vat.activity.add')}
      </button>
    </section>
  );
...
function TaxCodeRegister({
  ...
  const columns: DataTableColumn<TaxCode>[] = [
    ...
    {
      key: 'actions',
      header: t('vat.code.archive'),
      headerHidden: true,
      align: 'end',
      render: (c) => {
        if (!canManageVat) return null;
        return c.archived === true ? (
          <button
            type="button"
            className="btn btn--ghost btn--sm"
            aria-label={t('vat.code.reactivateLabel', { code: c.code })}
            onClick={() => void reactivate(c.code)}
          >
            {t('vat.code.reactivate')}
          </button>
        ) : (
          <button
            type="button"
            className="btn btn--ghost btn--sm"
            aria-label={t('vat.code.archiveLabel', { code: c.code })}
            onClick={() => setConfirmArchive(c.code)}
          >
            {t('vat.code.archive')}
          </button>
        );
  ...
  return (
    <form className="vat-add-code form-stack" onSubmit={onSubmit}>
      <div className="form-row">
        ...
        <input id={ids.code} value={code} onChange={(e) => setCode(e.target.value)} required />
      </div>
      <div className="form-row">
        <label htmlFor={ids.kind}>{t('vat.code.kindLabel')}</label>
        <select id={ids.kind} value={kind} onChange={(e) => setKind(e.target.value as TaxKind)}>
          {TAX_KINDS.map((k) => (
            <option key={k} value={k}>
              {t(`vat.code.kind.${kindKeySegment(k)}`)}
            </option>
          ))}
        </select>
      </div>
      ...
      <div className="form-row">
        ...
        <input id={ids.formLine} value={formLine} onChange={(e) => setFormLine(e.target.value)} placeholder="303" />
      </div>
      <div className="form-row">
        <label htmlFor={ids.label}>{t('vat.code.label')}</label>
        <input id={ids.label} value={label} onChange={(e) => setLabel(e.target.value)} />
      </div>
      <div className="form-actions">
