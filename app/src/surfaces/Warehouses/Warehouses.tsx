...
export function Warehouses() {
    ...
    },
    {
      ...
      render: (w) => (
        <span className="wh-name-cell">
          ...
          {/* "Standard" is a property, so a quiet tag and never the accent; archived is a state, so
              the Status word (K-22). */}
          {w.isDefault && <span className="wh-tag">{t('warehouses.default')}</span>}
          {!w.active && <Status kind="inactive" label={t('warehouses.archived')} />}
        </span>
      ),
    },
  ];
  ...
  const locationColumns: DataTableColumn<LocationNode>[] = [
    {
      ...
      render: (n) => (
        <span className="wh-name-cell" style={{ paddingLeft: `${n.depth * 16}px` }}>
          <span className="wh-code">{n.code}</span>
          {n.isDefaultForWarehouse && <span className="wh-tag">{t('warehouses.default')}</span>}
        </span>
      ),
    },
  ...
  // K-21: a location row opens its editor; "Als Standard" and "Archivieren" sit behind one overflow.
  const locationActions = (n: LocationNode) => [
    ...(!n.isDefaultForWarehouse && n.active
      ? [
          {
            key: 'default',
            label: t('warehouses.makeDefault'),
            onSelect: () => void write('location_set_default', { locationId: n.id, idempotencyKey: newKey() }),
          },
        ]
      : []),
    ...(n.active
  ...
  return (
    <div className="wh">
      ...
      {failed && (
        <ErrorBanner context="read" message={t('warehouses.error.transport')} onRetry={() => void loadWarehouses()} />
      )}

      {showSkeleton ? (
        <DataTable columns={warehouseColumns} rows={[]} rowKey={(w) => w.id} loading skeletonRows={4} />
      ) : warehouses.length === 0 ? (
        ...
        <div className="wh-split">
          <section className="wh-pane" aria-label={t('warehouses.listLabel')}>
            <DataTable
              ...
              // K-24: the open warehouse wears the selection pill on every cell, no side bar.
              isRowCurrent={(w) => w.id === selectedId}
              rowClassName={(w) => (w.active ? undefined : 'wh-archived')}
            />
          </section>
            ...
            {selected === null ? (
              ...
              <>
                <div className="wh-detail-head">
                  ...
                  {/* K-21: Bearbeiten in the open, the rarer verbs behind one overflow. */}
                  <div className="wh-detail-actions">
                    <button type="button" className="btn btn--secondary btn--sm" onClick={() => openWhEdit(selected)} disabled={!canWrite}>
                      {t('warehouses.edit')}
                    </button>
                    {canWrite && (!selected.isDefault || selected.active) && (
                      <OverflowMenu
                        quiet
                        label={t('warehouses.actionsFor', { code: selected.code })}
                        items={[
                            ...
                            ? [
                                {
                                  ...
                                  label: t('warehouses.makeDefault'),
                                  onSelect: () =>
                                    void write('warehouse_set_default', { warehouseId: selected.id, idempotencyKey: newKey() }),
                                },
                              ]
                            : []),
                          ...(selected.active
                ...
                <div className="wh-subhead">
                  ...
                  <button type="button" className="btn btn--secondary btn--sm" onClick={openLocCreate} disabled={!canWrite || !selected.active}>
                ...
                <DataTable
                  ...
                  onRowClick={canWrite ? (n) => openLocEdit(n) : undefined}
                  rowLabel={(n) => n.code ?? n.name}
                  rowActions={canWrite ? locationActions : undefined}
                  rowActionsLabel={(n) => t('warehouses.locations.actionsFor', { code: n.code ?? n.name })}
                />
        ...
        >
          ...
          <div className="wh-field">
            ...
            <Select
              ...
              onChange={(value) => setLocDraft({ ...locDraft, locationType: value as LocationType | '' })}
              options={[
                { value: '', label: t('warehouses.locations.field.noType') },
                ...LOCATION_TYPES.map((ty) => ({ value: ty, label: t(`warehouses.locations.type.${ty}`) })),
              ]}
              ariaLabel={t('warehouses.locations.field.type')}
            />
          </div>
          {locDrawer === 'create' && (
            <div className="wh-field">
              ...
              <Select
                id="loc-parent"
                value={locDraft.parentId}
                onChange={(value) => setLocDraft({ ...locDraft, parentId: value })}
                options={[
                  { value: '', label: t('warehouses.locations.field.rootParent') },
                  ...activeLocations.map((n) => ({
                    value: n.id,
                    // The tree indent uses a non-breaking space so it survives HTML whitespace collapse.
                    label: `${'\u00A0'.repeat(n.depth * 2)}${n.code ?? n.name}`,
                  })),
                ]}
                ariaLabel={t('warehouses.locations.field.parent')}
              />
            </div>
          )}
          <div className="wh-field">
