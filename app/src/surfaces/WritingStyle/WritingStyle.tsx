 ...
 * (US-E05.5). Runtime state is glyph + label (● / ○), never colour alone (WCAG 2.2 AA).
 *
...
export function WritingStyle() {
  ...
  /** ● lokal / ○ nicht installiert: glyph + label, never colour alone. */
  const statusLine = (
    <p className="voice-runtime-line">
      <span className="voice-runtime-state" aria-label={registered ? t('runtime.status.ready') : t('runtime.status.absent')}>
        <span aria-hidden="true">{registered ? '●' : '○'}</span> {registered ? t('runtime.status.ready') : t('runtime.status.absent')}
      </span>
      ...
      {canWrite && (
        <details className="voice-advanced">
          ...
          <label className="voice-field">
            ...
            <input type="text" value={ggufPath} onChange={(e) => setGgufPath(e.target.value)} placeholder="/pfad/zu/modell.gguf" />
          </label>
          <button
            type="button"
  ...
  return (
    <section className="voice" aria-labelledby="voice-title">
      ...
      {failed && <ErrorBanner message={t('voice.error.transport')} onRetry={() => void load()} />}
            ...
            <div className="voice-profile">
              {stale && (
                <p className="voice-stale">
                  <span aria-hidden="true">⚠</span> {t('voice.stale')}
                </p>
              )}
