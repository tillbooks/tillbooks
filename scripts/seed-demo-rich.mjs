...
function phase4() {
  const lines = [
    ...
    // R4-D2: the Maschinen opening (Dr 1500 3'000'000 / Cr 1509 800'000) is NO LONGER booked as a
    // bare journal line here. It is registered THROUGH the asset module in phase 6 via
    // asset_opening_balance, so the fixed-asset sub-ledger TIES to the 1500/1509 control accounts and
    // asset_reconciliation_check is clean end to end. That verb posts the identical GL (Dr 1500 / Cr
    // 1509 / Cr 2800 for the net book value), so the 2800 Eigenkapital difference still nets to the
    // same 9'000'000 across the two entries: the GL end state is byte-identical, only the mechanism
    // (and the tying asset_transaction row) changed. Formerly a latent gap: the drift blocked any
    // post-2025 period close, but the seed's verify never exercised one.
  ];
...
function phase6() {
  ...
  // R4-D2 (owner answer = Option A): register the opening Maschinen holding THROUGH the asset module
  // so the fixed-asset sub-ledger TIES to the 1500/1509 GL opening balance. asset_opening_balance
  // posts ONE balanced GL entry (Dr 1500 3'000'000 / Cr 1509 800'000 / Cr 2800 2'200'000 for the net
  // book value) AND writes ONE append-only asset_transaction(type=opening) that carries the same
  // cost/accumulated deltas, so the recon control accounts net to zero. This REPLACES the bare
  // 1500/1509 lines that phase 4's set_opening_balances used to book: the GL end state is identical
  // (2800 still nets 9'000'000 across the two opening entries), so no golden report figure moves.
  //
  // Seeded AFTER the depreciation-run loop ON PURPOSE: the golden's monthly runs charged nothing
  // while every phase-6 asset was a draft, so the ledger carries zero fixed-asset depreciation. An
  // opening asset seeded before the loop would suddenly depreciate across ~19 months and move the
  // P&L, the accumulated-depreciation balance and every downstream statement. Seeding it here keeps
  // the depreciation at zero and the golden figures unchanged, while the recon ties from the opening
  // period forward (subLedger == GL on 1500 and 1509 at every cut-off on or after 01.01.).
  const obDate = d('2025-01-01');
  let obSeeded = false;
  if (H.assetCats && H.assetCats.maschinen) {
    const ob = tryCall('asset_create', {
      workspaceId: W.main, categoryId: H.assetCats.maschinen, name: 'Maschinenpark Eröffnungsbestand',
      acquisitionDate: obDate, acquisitionCostRappen: 3000000, depreciationMethod: 'straight_line',
      usefulLifeMonths: 96, idempotencyKey: k('p6', 'obasset', 1),
    }, { date: obDate });
    const obId = ob ? (ob.asset?.id ?? oneId(ob)) : null;
    if (obId) {
      call('asset_opening_balance', {
        workspaceId: W.main, assetId: obId, date: obDate,
        costRappen: 3000000, accumulatedDeprRappen: 800000,
        offsetAccountId: accId(W.main, '2800'),
        description: 'Eröffnungsbestand Maschinen per 01.01.' + obDate.slice(0, 4),
        idempotencyKey: k('p6', 'obseed', 1),
      }, { date: obDate });
      obSeeded = true;
    }
  }
  // The opening asset gates the whole 1500/1509 recon tie. If asset_create was skipped (no Maschinen
  // category) or failed, the sub-ledger silently stays empty and the golden loses 2.2M of NBV, so
  // fail loudly rather than print a reassuring "recon ties" over an absent asset (R4-D2 critic).
  if (!obSeeded) FAILURES.push('phase6: the Maschinen opening asset was NOT seeded; asset_reconciliation_check will drift on 1500/1509');
  console.log('  phase6: ' + Object.keys(H.assetCats).length + ' categories, ' + a + ' assets, ' + r + ' depreciation runs, ' + (obSeeded ? '1 opening-balance asset (recon ties)' : 'NO opening-balance asset (recon WILL drift)'));
}
...
/**
 * R4-D2: prove the fixed-asset sub-ledger TIES to the GL from the opening period forward, so the
 * asset-reconciliation gate a POST-2025 period close must pass is green on the golden. This is the
 * hard check `asset_reconciliation_check` (the one a period close and agents invoke): before this
 * decision the golden carried a 3'000'000 bare Maschinen opening on 1500 with no backing sub-ledger,
 * so the check returned reconciliation_drift and would have BLOCKED any post-opening close. The gap
 * was latent because the seed's verify never exercised the check (mirrors K-65 / K-67: a golden fix
 * is only real once the seed's own verify exercises the flow it unblocks). A drift here is a named
 * FAILURE and exits the run nonzero.
 */
function verifyAssetReconciliation() {
  console.log('\nFixed-asset reconciliation (R4-D2, the post-close gate):');
  // Cut-offs strictly on or after the opening (year(TODAY)-1-01-01): the opening-year end and the
  // first month of the following year. The opening is always seeded (never a future skip), so both
  // are meaningful at any rolled TODAY.
  const periods = [dm('2025-12'), dm('2026-01')];
  for (const period of periods) {
    const res = handleRest('asset_reconciliation_check', { workspaceId: W.main, period }, deps);
    const balanced = res.body && res.body.ok !== false && res.body.status === 'balanced';
    if (balanced) {
      console.log('  asset_reconciliation_check ' + period + ': balanced');
    } else {
      const drift =
        res.body && res.body.accounts
          ? res.body.accounts
              .map((a) => a.accountNumber + ' sub=' + a.subLedgerRappen + ' gl=' + a.glBalanceRappen + ' delta=' + a.deltaRappen)
              .join('; ')
          : (res.body && res.body.error) || 'unknown';
      console.log('  asset_reconciliation_check ' + period + ': DRIFT (' + drift + ')');
      FAILURES.push('verifyAssetReconciliation ' + period + ': ' + drift);
    }
  }
  // The 1500/1509 control accounts specifically must tie (sub-ledger == GL), not merely net across
  // the whole workspace: assert each named account is balanced in the report drill-down.
  const rep = handleRest('asset_reconciliation_report', { workspaceId: W.main, period: dm('2025-12') }, deps);
  const accounts = (rep.body && rep.body.accounts) || [];
  // Assert the tie AND the expected magnitude. A silently-absent opening asset reads sub=0 gl=0, which
  // "ties" vacuously (0 === 0) and passes a tie-only check while 2.2M of NBV has vanished. Pinning the
  // real figures makes the guard catch that regression (R4-D2 critic finding).
  const expectedGl = { '1500': 3000000, '1509': 800000 };
  for (const number of ['1500', '1509']) {
    const row = accounts.find((a) => a.accountNumber === number);
    const tied = row && row.status === 'balanced' && row.subLedgerRappen === row.glBalanceRappen;
    const expected = expectedGl[number];
    const magnitude = row && row.glBalanceRappen === expected && row.subLedgerRappen === expected;
    console.log('  control account ' + number + ': ' + (row ? row.status + ' sub=' + row.subLedgerRappen + ' gl=' + row.glBalanceRappen : 'not present'));
    if (!tied) FAILURES.push('verifyAssetReconciliation account ' + number + ' does not tie: ' + (row ? JSON.stringify(row) : 'missing'));
    if (!magnitude) FAILURES.push('verifyAssetReconciliation account ' + number + ' wrong magnitude, expected sub=gl=' + expected + ': ' + (row ? JSON.stringify(row) : 'missing'));
  }
}
