...
function portDeps(store: ApiDeps['store'], clock: ApiDeps['clock'], ids: ApiDeps['ids'], actor: string, deps: ApiDeps): PortDeps {
  return { store, clock, ids, actor, backupDir: deps.backupDir ?? resolveBackupDir() };
}
...
export function dataActions(h: DataActionHelpers): readonly ActionDef[] {
  ...
  return [
    ...
    depsAction(
      ...
      (deps, input) => verifyBackup(fromDeps(deps), { source: String(input.source ?? '') }),
    ),
    depsAction(
      ...
      depsSchema({}, []),
      (deps) => listRestorableBackups(fromDeps(deps)),
    ),
    depsAction(
      'restore_backup',
      'write',
      ...
      (deps, input) => restoreBackup(fromDeps(deps), input as never),
    ),
    depsAction(
      'get_api_catalog',
      'read',
