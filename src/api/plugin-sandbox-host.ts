...
/** True when `root` is `target` or a directory that CONTAINS it (both resolved absolute). */
...
/**
 * FAIL CLOSED when a derived runtime root would widen the kernel read allowlist over a SENSITIVE tree
 * (security review F6). `computeRuntimeRoots` takes node's grandparent, which is a runtime prefix for a
 * system/Homebrew/nvm node but is `$HOME` for a node at `$HOME/bin/node` (or `$HOME/.local` for
 * `$HOME/.local/bin/node`). The Seatbelt profile then `(allow file-read* (subpath "$HOME"))` and the
 * bubblewrap plan `--ro-bind-try`s it, so the jail's stated denial of "the ledger DB, $HOME" is false
 * while `sandboxStatus()` still reports `active:true`. Rather than silently widen, host construction
 * THROWS, so the platform reports NO usable mechanism and every plugin fails closed (the deny-default
 * direction), and the operator relocates the runtime instead of running a jail that leaks the ledger.
 * A root is refused when it CONTAINS the home directory, the resolved support dir, the resolved ledger
 * path, or the process cwd (the trees the jail exists to keep out). The cwd is one of those trees, so
 * the converse also holds and is honest: a `till` started with its working directory INSIDE the node
 * runtime prefix (an unusual `cd /opt/homebrew; till up`) makes the prefix root contain the cwd and is
 * refused too. That is the fail-closed direction (no jail rather than a jail that reads the cwd), and
 * the operator relocates the runtime or starts `till` from elsewhere.
 */
function assertRuntimeRootsAreConfined(roots: readonly string[]): void {
  const sensitive: string[] = [];
  for (const compute of [homedir, resolveSupportDir, resolveDbPath, process.cwd]) {
    try {
      const p = compute();
      if (typeof p === 'string' && p.length > 0) sensitive.push(p);
    } catch {
      // A resolver that throws (a missing HOME, an unreadable cwd) contributes no constraint rather
      // than failing construction; the remaining sensitive paths still fence the root.
    }
  }
  for (const root of roots) {
    for (const target of sensitive) {
      if (containsPath(root, target)) {
        throw new Error(
          `till_plugin_sandbox: refusing to build a jail whose runtime read root "${root}" contains a ` +
            `sensitive path "${target}" (it would widen the kernel read allowlist over the ledger / $HOME). ` +
            'Relocate the node runtime out of that tree; the host fails closed rather than run a leaky jail.',
        );
      }
    }
  }
...
export function createProcessSandboxHost(config: ProcessSandboxHostConfig): ProcessSandboxHost {
  ...
  // F6: fail closed if a derived root would widen the read allowlist over $HOME / the ledger / cwd.
  assertRuntimeRootsAreConfined(runtimeRoots);
