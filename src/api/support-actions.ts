...
/**
 * M01 (security review F4 + D59 critic): the `machine_scope` diagnostics and feedback verbs are
 * refused to a served subject. `set_diagnostics` / `clear_diagnostics` are ungated on "changing YOUR
 * OWN privacy setting is self-determination", and `prepare_feedback` / `preview_feedback` on the same
 * "reporting a bug is not a privilege" reasoning. On a served instance the diagnostics log and the
 * feedback directory are the MACHINE's, not the caller's: a served subject (a stranger included)
 * clearing the log wipes the instance's G08 record under an unknown identity, and `prepare_feedback`
 * writes a file into the host support dir for any authenticated subject (an unreviewed DoS surface the
 * critic flagged). So these machine-scope verbs are refused for any served subject; a LOCAL operator
 * (the file holder, identitySource absent or `local_client`) keeps the self-service the copy promises.
 */
function refuseServedMachineScope(ctx: WorkspaceContext): Result | undefined {
  if (ctx.identitySource === 'served_subject') {
    return err('permission_denied', { capability: DIAGNOSTICS_READ, role: null, reason: 'machine_scope_host_only' });
  }
  return undefined;
}
...
export function supportActions(h: ActionHelpers): readonly ActionDef[] {
  ...
  return [
    ...
    ctxAction(
      ...
      (ctx, input) => refuseServedMachineScope(ctx) ?? setDiagnostics(depsFor(ctx, supportDir, undefined), input),
    ),
    ctxAction(
      'clear_diagnostics',
      'write',
      ...
      ctxSchema(),
      (ctx) => refuseServedMachineScope(ctx) ?? clearDiagnostics(depsFor(ctx, supportDir, undefined)),
    ),
  ];
}
