  ...
  return [
    depsAction(
      ...
      (deps, input) => {
        ...
        // Thread identitySource so a served subject is scoped away from unprovisioned tenant names.
        return listWorkspaces({ store: deps.store, actor: deps.actor, identitySource: deps.identitySource }, filter);
      },
    ),
    ctxAction(
      'get_workspace',
