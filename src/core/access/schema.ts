...
-- NULL subject and must stay outside the index (a machine with no proxy has no subjects at all). Over
-- lower(subject) (F-11 critic F2): the subject is matched case-insensitively at accept and resolve,
-- so one identity cannot be bound to two case-variants of the same subject.
CREATE UNIQUE INDEX IF NOT EXISTS user_by_subject ON user (lower(subject)) WHERE subject IS NOT NULL;
