...
import { existsSync, mkdirSync, renameSync, rmSync, realpathSync } from 'node:fs';
...
export function readOrBootstrap(deps: LandscapeDeps): LandscapeControlFile {
  ...
  return writeLandscape(
    deps.supportDir,
    // D135: a fresh landscape opens on `main`, the real books, not on `develop`. Defaulting to a
    // synthetic sandbox hid the one environment the owner actually works in. Existing installs keep
    // their stored active pointer; only a first-ever bootstrap is affected.
    { version: 1, environments, active: 'main' },
...
function standardEnvironments(deps: LandscapeDeps): Record<string, Environment> {
  ...
  return {
    main: {
      ...
      // D135: main runs LOCALLY on a local-first desktop install (the process writes its books to
      // resolveDbPath() regardless of tier). `served` was a leftover of the superseded D126 physical
      // model and no code binds to a remote endpoint; a served/hosted deployment declares itself at
      // runtime via TILL_SERVED_MODE, never via this landscape row. Keeping it `served` made a fresh
      // install, now defaulting to main, report the real ledger as read-only to both the Studio and
      // the MCP agent.
...
/**
 * An environment is read-only from a local face only when it is SERVED (a remote/hosted instance the
 * local process cannot write through). A `protected` tier (main) is NOT read-only: it forbids env-wide
 * destructive ops, but ordinary book writes go to the real ledger. Conflating the two (D135) made a
 * local main report read-only to the Studio and the MCP agent even though every post lands. The
 * enforcement of main's protection lives in the env-wide verbs (`env_reset`/`env_delete`/`env_copy`),
 * not in this flag; nothing gates a write on it.
 */
function isReadOnly(env: Environment): boolean {
  return env.runtime_target === 'served';
}
...
/**
 * The real absolute path of `p`, resolving a symlinked ANCESTOR (the deepest existing one) and then
 * appending the not-yet-existing tail. Used so the confinement below compares the target and the
 * environments root under the SAME symlink resolution: macOS `/var` -> `/private/var`, and a `..`
 * routed through a planted symlink under the root, both collapse consistently instead of slipping the
 * lexical prefix check. `resolve()` first so a lexical `..` is collapsed before any realpath.
 */
function realDir(p: string): string {
  ...
  try {
    ...
    // keep the lexical resolution if realpath cannot stat the ancestor
  }
  return tail.length > 0 ? join(cur, ...tail) : cur;
}

/**
 * PATH CONFINEMENT (M01 security review F1, D59 critic). A caller-supplied `dbPath` is confined to
 * the environments root for EVERY create path: synthetic, live AND `policy:'copy'` (the critic found
 * the copy path skipped this and overwrote a host file with a sanitised copy of `main`). Returns the
 * RESOLVED path to build and store from (a trailing-slash `dbPath` is refused rather than left to
 * `EISDIR` mid-build), or a Result rejection. It fences EVERY face, not just the served one (the served
 * fence in `env-actions.ts` already stops a served subject reaching these verbs): a LOCAL agent seat
 * holding `landscape.manage` had the same arbitrary-host-write primitive over stdio.
 */
export function confineDataRoot(
  dbPath: string,
  environmentsRoot: string,
  environments: Readonly<Record<string, Environment>>,
): { resolvedDb: string } | { rejected: Result } {
  if (dbPath.endsWith('/') || dbPath.endsWith(sep) || basename(dbPath).length === 0) {
    return { rejected: err('invalid_input', { field: 'dbPath', reason: 'a data root is a file path, not a directory' }) };
  }
  const resolvedDb = realDir(dbPath);
  const rootCanon = realDir(environmentsRoot);
  const underRoot = resolvedDb === rootCanon || resolvedDb.startsWith(rootCanon + sep);
  if (!underRoot) {
    return { rejected: err('data_root_outside_environments_root', { dbPath, environmentsRoot }) };
  }
  // Case-insensitive (served-security re-critic residual R2): on a case-insensitive filesystem a
  // `till.db-WAL` a caller hands in would alias the operator's own `-wal` sidecar, so the uppercase
  // spelling is refused as a sidecar exactly like the lowercase one.
  if (/(?:-wal|-shm|\.building)$/i.test(resolvedDb)) {
    return { rejected: err('data_root_is_sidecar', { dbPath }) };
  }
  for (const other of Object.values(environments)) {
    const base = realDir(other.db_path);
    if (resolvedDb === `${base}-wal` || resolvedDb === `${base}-shm` || resolvedDb === `${base}.building`) {
      return { rejected: err('data_root_is_sidecar', { dbPath, of: other.name }) };
    }
  }
  if (existsSync(resolvedDb)) {
    return { rejected: err('data_root_exists', { dbPath }) };
  }
  return { resolvedDb };
}
...
export function envCreate(deps: LandscapeDeps, input: CreateInput): Result {
  ...
  // PATH CONFINEMENT (M01 security review F1, D59 critic): confine the data root to the environments
  // root and build from the RESOLVED path, so a trailing slash or a symlinked ancestor cannot slip
  // the check or EISDIR mid-build. The copy path is confined too, in `envCreateCopy` (copy.ts).
  ...
  if (input.confirmed !== true) {
    return ok({
      staged: true,
      plan: { name, policy, codeChannel, dbPath: resolvedDb, dataPolicy: policy, tierRank, runtimeTarget, seed: seedName },
    });
  }
  ...
  // Build/rename use the RESOLVED path (never the raw `dbPath`), so a trailing slash cannot EISDIR.
  const building = resolvedDb + '.building';
  try {
