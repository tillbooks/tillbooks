 ...
 *     honestly rather than pretending a process is running: with no host registered a plugin's
 *     `mcp_tool` is never added to `ACTIONS`, so a call to it answers `unknown_action` (the
 *     `plugin_host_unavailable` code is the SEPARATE case of a registered host whose process is not
 *     running, `plugin-sandbox-host.ts host_call`).
 *
 * THE IFRAME half: the Studio mounts a real `sandbox="allow-scripts"` frame with NO
 * `allow-same-origin`, so the document has an OPAQUE origin and cannot read the shell. That opaque
 * origin is the enforced isolation, and `iframeSandboxDescriptor` is its single source of truth so the
 * engine and the Studio agree on the exact attribute string. It ALSO computes a `connectSrc` from the
 * granted `network:*` scopes, but the Studio does not today turn that into an enforced Content-Security-
 * Policy (the frame is `about:blank`, so nothing loads): NETWORK FENCING IS THE HOST'S RESPONSIBILITY
 * when a host serves a real plugin document, not something this frame enforces on its own. The consent
 * copy (`Extensions/messages.*.json`) states exactly that, and must not be widened to promise a CSP the
 * code does not build.
 *
 ...
 * absent a registered host the plugin's tools are never registered in `ACTIONS`, so a call answers
 * `unknown_action` (the `plugin_host_unavailable` code is the host-registered-but-process-not-running
 * case, not this one).
 *
 ...
 * with no host registered the tool is never added to `ACTIONS`, so the api layer answers `unknown_action`
 * (`plugin_host_unavailable` is what a REGISTERED host returns when the process itself is not up). This
 * is the declare-the-seam-ship-nothing technique the email relay and the registry client already use.
 */
