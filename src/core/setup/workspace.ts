...
export function listWorkspaces(
  deps: { store: SqliteStore; actor?: string; identitySource?: IdentitySource | undefined },
  input: { includeArchived?: boolean } = {},
): Result {
  // M01 (security review D59 critic, allowlist hardening): a SERVED subject must NOT see the NAMES of
  // UNPROVISIONED workspaces. The "no members yet => visible to any D13 actor" branch is a LOCAL fact
  // (the file holder re-finding its solo book), the same reasoning `capabilityFor`'s step 1 rests on;
  // across the served trust boundary it disclosed a transient tenant's name (a fresh local create
  // before its first invite, or a local restore) to every authenticated subject, strangers included.
  // For a served subject the unprovisioned branch is disabled, so it sees ONLY workspaces it is an
  // accepted member of. A LOCAL caller (identitySource absent or `local_client`) is unchanged.
  const served = deps.identitySource === 'served_subject' ? 1 : 0;
  const rows = deps.store.db
    .prepare(
      `SELECT w.id, w.name, w.legal_form, w.base_currency, w.fiscal_year_start, w.archived, w.created_at
         FROM workspace w
        WHERE (? IS NULL
               OR (? = 0 AND NOT EXISTS (SELECT 1 FROM workspace_member m WHERE m.workspace_id = w.id))
               OR EXISTS (SELECT 1
    ...
    .all(deps.actor ?? null, served, deps.actor ?? null, input.includeArchived === true ? 1 : 0) as {
    id: string;
    name: string;
