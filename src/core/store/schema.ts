...
export const ADDITIVE_INDEXES: readonly { name: string; ddl: string }[] = [
  ...
  // index: a local install has no proxy and no subjects. Over `lower(subject)` (F-11 critic F2): the
  // subject is matched case-insensitively at accept and at resolve (`resolveServedActor`), so one
  // identity cannot be bound to two case-variants. It can only FAIL on a database that already holds
  // two identities bound to one subject, which `acceptInvite` (served mode) cannot produce (it binds
  // the subject onto exactly the invited user row, and `user_by_subject` would already have refused a
  // second), so a failure here is a real fault worth surfacing rather than swallowing. The column is
  // additive, so this cannot live in a CREATE beside the table: SCHEMA_SQL runs before ensureColumns
  // widens the row.
  {
    name: 'user_by_subject',
    ddl: `CREATE UNIQUE INDEX IF NOT EXISTS user_by_subject
          ON user (lower(subject)) WHERE subject IS NOT NULL`,
  },
