...
function seedAndInvite(store) {
  // The invite minted here is REDEEMED through the served `till up` process (`withServedUp` below),
  // which runs on the real system clock. Mint on that same clock, not a fixed past date: a fixed
  // 2026-09-05 mint gave the token a 14-day life that had already lapsed by the time the suite ran,
  // so `accept_invite` came back `invite_expired` and these tests failed by the calendar. No assertion
  // in this file reads a timestamp, so the real clock changes nothing that is checked.
  const deps = { store, clock: systemClock, ids: sequenceIdGen(), actor: 'studio' };
