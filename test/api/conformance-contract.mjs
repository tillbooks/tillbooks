...
export const SCENARIOS = {
  ...
  // in-process `minimal` seed (never the heavy Seeblick child process). `env_switch` targets `develop`
  // (present from bootstrap, and non-active since main is active after bootstrap: D135), so it is a real
  // switch. `env_reset`/`env_delete` act on a freshly-created NON-active named env, because reset/delete
  // refuse the active tier (main is active after bootstrap).
  env_switch: (fx) => ({ workspaceId: fx.workspaceId, name: 'develop', confirmed: true, idempotencyKey: 'n00-switch-1' }),
  env_create: (fx) => ({
