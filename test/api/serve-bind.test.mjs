...
async function withServe(env, check) {
  ...
  try {
    ...
    // The child may have ALREADY exited (a refused bind, e.g. the port was taken by a concurrent
    // gate, exits 1 before it ever announces). Waiting on `once(child, 'exit')` in that case blocks
    // forever on an event that already fired, which is the exact shape that wedged this suite. Only
    // signal-and-wait while the child is still live, and register the listener BEFORE the kill so a
    // child that dies between the two is still reaped (there is no await between the check and the
    // listener, so its exit cannot slip through).
