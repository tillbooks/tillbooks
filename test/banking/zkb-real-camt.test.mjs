// A20 camt import hardened against a REAL ZKB (Zürcher Kantonalbank) Datalink export.
//
// WHY THIS EXISTS. Every other camt test in this repo builds its XML by hand (`camtXml(...)`), so it
// proves the parser against OUR idea of camt, never against a bank's. These four fixtures are a real
// ZKB camt.053 export (four consecutive Datalink statements), identity-scrubbed but structurally
// verbatim: the namespace, element order, the ZKB-specific shapes, and every amount are exactly as
// ZKB emitted them. Only names, addresses, IBANs, references, card and VAT numbers were replaced with
// fixed test values; amounts and balances are untouched so the Rappen reconciliation is meaningful.
//
...
//     on real data), and a card debit carrying `CCRD`/`<PAN>`.
//
...
// The scrubbed owner account IBAN, shared by all four statements.
const ZKB_IBAN = 'CH9300762011623852957';

// What each real statement parses to. Amounts are ZKB's, verbatim.
const STATEMENTS = [
  {
    file: 'zkb-camt053-01-20231201.xml',
    seq: '233', opbd: 1837818, clbd: 1840206,
    entry: { cd: 'CRDT', amountMinor: 2388, referenceKind: 'none' },
  },
  {
    file: 'zkb-camt053-02-20240111.xml',
    seq: '7', opbd: 1835116, clbd: 1833393,
    entry: { cd: 'DBIT', amountMinor: 1723, referenceKind: 'qrr', referenceValue: '210000000003139471430009017' },
  },
  {
    file: 'zkb-camt053-03-20240201.xml',
    seq: '22', opbd: 1833393, clbd: 1848659,
    entry: { cd: 'CRDT', amountMinor: 15266, referenceKind: 'none' },
  },
  {
    file: 'zkb-camt053-04-20240226.xml',
    seq: '39', opbd: 1848659, clbd: 1724942,
    entry: { cd: 'DBIT', amountMinor: 123717, referenceKind: 'none' },
  },
];

test('every real ZKB camt.053.001.04 statement parses to the exact Rappen, and reconciles OPBD + net = CLBD', () => {
...
test('ZKB QRR: the QR creditor reference travels under Prtry=QRR, the AddtlRmtInf sibling is ignored', () => {
  const s = ok(parseCamt(read('zkb-camt053-02-20240111.xml'))).statement;
  const [e] = s.entries;
...
test('ZKB extra balance types (CLAV, CCRD) are skipped; only OPBD and CLBD are surfaced', () => {
  ...
  const raw = read('zkb-camt053-01-20231201.xml');
  assert.ok(raw.includes('<Cd>CLAV</Cd>'), 'fixture should still carry the CLAV balance');
  const s = ok(parseCamt(raw)).statement;
  assert.equal(s.openingBalanceMinor, 1837818);
  assert.equal(s.closingBalanceMinor, 1840206);
});

test('ZKB entry+TxDtls AcctSvcrRef: the D81 identity key prefers the entry-level AcctSvcrRef', () => {
  const s = ok(parseCamt(read('zkb-camt053-01-20231201.xml'))).statement;
  // Entry-level AcctSvcrRef is TESTREF0000000001 (scrubbed from ZKB's Z... order ref); the identity
...
// --- money path: importing the real statements is idempotent on ROWS -------------------------------
...
test('importing the four real ZKB statements books one bank_txn each, and a re-import is a no-op', () => {
  const t = zkbAccount();
  const countTxns = () =>
  ...
  assert.equal(countTxns(), STATEMENTS.length, 'one bank_txn per real statement');
  assert.equal(countStatements(), STATEMENTS.length, 'one bank_statement per real file');
  ...
  assert.equal(countTxns(), STATEMENTS.length, 're-importing the real statements doubled the bank_txn rows');
  assert.equal(countStatements(), STATEMENTS.length, 're-importing minted a second bank_statement');
});

test('a real ZKB statement imported into the WRONG account IBAN is refused, and nothing is booked', () => {
  const t = setup();
  const other = ok(
    createBankAccount(t.ctx, {
      name: 'Falsches Konto',
  ...
  const res = importCamt(t.ctx, {
    ...
    xml: read('zkb-camt053-01-20231201.xml'),
