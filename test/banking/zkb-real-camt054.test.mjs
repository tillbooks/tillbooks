...
//
// WHY THIS EXISTS. The sibling `zkb-real-camt.test.mjs` covers four REAL, identity-scrubbed ZKB
// camt.053 statements: single-entry day-end statements. The owner had no real camt.054 (ZKB Datalink
// order type Z54/ZS4/ZQR) to hand, and camt.054 is the shape most likely to expose a parser edge case,
...
//
// THE FIXTURE (`fixtures/zkb-camt054-01-sammelbuchung-qrr.xml`) is SYNTHETIC, not a byte-scrubbed real
// export (its header says so plainly), built to the SIX SPS Cash Management guidelines and the ZKB
// Datalink shape. When a real ZKB Z54 export arrives it should REPLACE the fixture and keep these
// assertions. All parties, IBANs and references are fixed fictitious test values; the amounts are the
// fact under test (1500.00 + 2750.50 + 999.95 == 5250.45).
//
...
// The scrubbed owner account IBAN (the account the notification is FOR), shared with the camt.053 suite.
