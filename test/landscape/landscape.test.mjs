...
test('landscape: first read bootstraps the three standard tiers, pinned first, main protected', () => {
  ...
  try {
    ...
    assert.equal(main.runtimeTarget, 'served');
    assert.equal(main.tierRank, RANK_MAIN);
    assert.equal(res.active, 'develop', 'the default active env is develop (local)');
...
test('landscape: current + status report the active env, drift unknown without a built channel', () => {
  ...
  try {
    ...
    assert.equal(cur.active, 'develop');
    assert.equal(cur.readOnly, false, 'a local develop env is writable');
    const st = envStatus(deps, { name: 'main' });
    assert.equal(st.ok, true);
    assert.equal(st.environment.readOnly, true, 'served + protected main is read-only from a local face');
...
test('landscape: delete refuses a standard tier without force and refuses the active env', () => {
  ...
  try {
    ...
    // The active env is refused even with force (E9b): develop is active by default.
...
// --- switch + read-only main (finding #13) -----------------------------------------------------

test('landscape: switch to main records the pointer and marks the face read-only', () => {
  const dir = tmp();
  try {
    const deps = makeDeps(dir);
    ...
    assert.equal(envCurrent(deps).readOnly, true);
  } finally {
    rmSync(dir, { recursive: true, force: true });
  }
});
